...
async function main() {
  ...
  check(
    'overview: has every expected field',
    expectedFields.every((f) => f in (overview.body ?? {})),
  );
  const realAcademyCount = await prisma.academy.count();
  check(
    'totalAcademies matches a direct DB count',
    overview.body?.totalAcademies === realAcademyCount,
    `api=${overview.body?.totalAcademies} db=${realAcademyCount}`,
  );

  // ── Academy list ─────────────────────────────────────────────────────────
  console.log('\n-- Academy list --');
  const list = await api('/admin/academies', {
    token: adminToken,
  ...
  check(
    'list: returns rows with the expected shape',
    !!list.body?.academies?.[0]?.slug && typeof list.body.academies[0].netRevenueCents === 'number',
  );

  const firstAcademy = list.body?.academies?.[0];
  if (firstAcademy) {
    const bySlug = await api('/admin/academies', {
      token: adminToken,
      params: { search: firstAcademy.slug },
    });
    check(
      'search by slug finds the academy',
      bySlug.body?.academies?.some((a) => a.id === firstAcademy.id),
    );

    const byStatus = await api('/admin/academies', {
      token: adminToken,
      params: { status: firstAcademy.status },
    });
    check(
      'status filter returns only that status',
      byStatus.body?.academies?.every((a) => a.status === firstAcademy.status),
    );
  }

  const noMatch = await api('/admin/academies', {
    token: adminToken,
    params: { search: 'zzzzz_no_such_academy_zzzzz' },
  });
  check(
    'empty search returns an empty (not error) result',
    noMatch.status === 200 && noMatch.body?.academies?.length === 0,
  );

  const overPageSize = await api('/admin/academies', {
    token: adminToken,
  ...
  if (firstAcademy) {
    ...
    check(
      'detail: has staff, featureFlags, revenue fields',
      Array.isArray(detail.body?.staff) &&
        Array.isArray(detail.body?.featureFlags) &&
        typeof detail.body?.netRevenueCents === 'number',
    );
    check('detail: featureFlags include all 5 known keys', detail.body?.featureFlags?.length === 5);
    check(
      'detail: staff excludes STUDENT-role rows',
      detail.body?.staff?.every((s) => s.role !== 'STUDENT'),
    );
  }
  ...
  for (const range of [7, 30, 90]) {
    ...
    check(
      `growth range=${range}: 200, ${range} zero-filled days`,
      growth.status === 200 && growth.body?.length === range,
    );
    const revenue = await api('/admin/analytics/revenue', { token: adminToken, params: { range } });
    check(
      `revenue range=${range}: 200, ${range} zero-filled days`,
      revenue.status === 200 && revenue.body?.length === range,
    );
  }
  const badRange = await api('/admin/analytics/growth', {
    token: adminToken,
    params: { range: 42 },
  });
  ...
  const roleCases = [
    [
      'STUDENT',
      await prisma.user.findFirst({ where: { role: 'STUDENT' }, select: { email: true } }),
    ],
    [
      'TEACHER',
      await prisma.user.findFirst({ where: { role: 'TEACHER' }, select: { email: true } }),
    ],
  ];
  for (const [role, user] of roleCases) {
    if (!user?.email) {
      console.log(`   SKIP  no ${role} in local data`);
      continue;
    }
    ...
    const routes = [
      '/admin/overview',
      '/admin/academies',
      `/admin/academies/${firstAcademy?.id ?? 'x'}`,
  ...
  const ownerMembership = await prisma.academyMembership.findFirst({
    where: { role: 'OWNER' },
  ...
  if (ownerMembership?.user?.email) {
    ...
    check(
      'academy OWNER refused the platform-wide academies list',
