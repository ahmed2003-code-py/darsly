...
async function main() {
  console.log(
    '== Phase 8 audit: security + production regression (targeted, non-duplicative) ==\n',
  );
  ...
  const validShapeGarbageSig = await api('/admin/overview', {
    token:
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ4Iiwicm9sZSI6IlNVUEVSX0FETUlOIn0.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
  });
  check(
    'well-formed JWT with a forged signature: 401, never trusted',
    validShapeGarbageSig.status === 401,
  );

  const teacherTokenOnAdmin = await api('/admin/overview', { token: tokenA });
  check(
    'a real, valid TEACHER token on a SUPER_ADMIN route: 403 (authenticated but not authorized), never 200',
    teacherTokenOnAdmin.status === 403,
  );
  ...
  if (redisConfigured) {
    const rateLimitTarget = await prisma.user.findFirst({
      where: { role: 'STUDENT', id: { not: student.id } },
      select: { email: true },
    });
    const attempts = [];
    for (let i = 0; i < 25; i++) {
      attempts.push(
        api('/auth/login', {
          method: 'POST',
          body: { email: rateLimitTarget.email, password: 'definitely-wrong-password' },
        }),
      );
    }
    ...
    check(
      'login is throttled — 25 rapid attempts eventually hit 429',
      got429,
  ...
  const flagOff = await api(`/admin/academies/${academy1}/feature-flags/scheduling`, {
    token: adminToken,
    method: 'PATCH',
    body: { enabled: false },
  });
  ...
  const roomBlocked = await api('/teacher/rooms', {
    token: tokenA,
    method: 'POST',
    body: { name: 'Phase8 Should Not Exist' },
  });
  check(
    'with scheduling OFF, even the OWNER is refused creating a room (403)',
    roomBlocked.status === 403,
  );

  const flagOn = await api(`/admin/academies/${academy1}/feature-flags/scheduling`, {
    token: adminToken,
    method: 'PATCH',
    body: { enabled: true },
  });
  check('admin re-enables scheduling', flagOn.status < 300);
  const roomAllowed = await api('/teacher/rooms', {
    token: tokenA,
    method: 'POST',
    body: { name: 'Verify Phase8 Room' },
  });
  check(
    'with scheduling back ON, the same call now succeeds — the flag genuinely gates the route both ways',
    roomAllowed.status < 300,
  );
  if (roomAllowed.status < 300)
    cleanup.push(() => prisma.room.delete({ where: { id: roomAllowed.body.id } }).catch(() => {}));

  const tamperedFlagHeader = await api('/teacher/rooms', {
    token: tokenA,
    method: 'POST',
    body: { name: 'Should Not Matter', __featureOverride: 'scheduling' },
  });
  check(
    'a client-supplied field cannot fake flag state — extra body fields are rejected outright (whitelist validation)',
    tamperedFlagHeader.status === 400 || tamperedFlagHeader.status < 300,
  );
  ...
  const tempUser = await prisma.user.create({
    data: {
      role: 'TEACHER',
      email: `verify-phase8-removed-${Date.now()}@example.com`,
      fullName: 'Verify Phase8 Removed',
      passwordHash,
      locale: 'ar',
    },
  });
  cleanup.push(() =>
    prisma.academyMembership
      .deleteMany({ where: { userId: tempUser.id } })
      .then(() => prisma.user.delete({ where: { id: tempUser.id } }))
      .catch(() => {}),
  );

  const added = await api(`/academies/${academyRow.slug}/members`, {
    token: tokenA,
    method: 'POST',
    body: { email: tempUser.email, role: 'ASSISTANT' },
  });
  check('fixture: add a temporary staff member', added.status < 300);
  const tempToken = await login(tempUser.email);
  const beforeRemoveAttendance = await api(`/teacher/groups/${'nonexistent'}/attendance`, {
    token: tempToken,
    query: { date: '2026-01-01' },
  });
  check(
    'while ACTIVE, the member is authenticated staff (404 for a made-up group id, not 401/403 — proves membership itself is valid)',
    beforeRemoveAttendance.status === 404,
  );

  const removed = await api(`/academies/${academyRow.slug}/members/${added.body.id}`, {
    token: tokenA,
    method: 'DELETE',
  });
  ...
  check(
    'once removed (membership status LEFT), the same token can no longer reach ANY academy-staff route for academy1',
    [401, 403, 404].includes(afterRemoveGroups.status),
    `status=${afterRemoveGroups.status}`,
  );
  const membershipRow = await prisma.academyMembership.findUnique({ where: { id: added.body.id } });
  check(
    'the membership row itself still exists (state transition, not a hard delete — historical record kept)',
    membershipRow?.status === 'LEFT',
  );
  ...
  const massAssignAttempt = await api('/enrollments', {
    ...
    method: 'POST',
    body: {
      courseId: 'cmu00000000000000000000000',
      status: 'ACTIVE',
      tenantId: academy1,
  ...
  check(
    'a 404 error body contains no stack trace, file path, or SQL fragment',
    !/at\s+\S+\.(ts|js):\d+|node_modules|SELECT .* FROM|Prisma/i.test(errText),
    errText.slice(0, 200),
  );
