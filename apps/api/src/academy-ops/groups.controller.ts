...
export class GroupsController {
  ...
  @Get()
  @ApiOperation({ summary: '[academy] Groups — OWNER sees all, staff sees only their assigned groups' })
  list(@CurrentAcademy() ctx: AcademyContext, @Query('page') page?: string, @Query('pageSize') pageSize?: string) {
    return this.groups.list(ctx, { page: page ? Number(page) : undefined, pageSize: pageSize ? Number(pageSize) : undefined });
  }
  ...
  update(@CurrentAcademy() ctx: AcademyContext, @Param('groupId') groupId: string, @Body() dto: UpdateGroupDto) {
    return this.groups.update(ctx, groupId, dto);
  }

  @Post(':groupId/members')
  @ApiOperation({ summary: '[academy] Add one or more students to a group' })
  addMembers(@CurrentAcademy() ctx: AcademyContext, @Param('groupId') groupId: string, @Body() dto: AddGroupMembersDto) {
  ...
  removeMember(@CurrentAcademy() ctx: AcademyContext, @Param('groupId') groupId: string, @Param('studentId') studentId: string) {
  ...
  assignStaff(@CurrentAcademy() ctx: AcademyContext, @Param('groupId') groupId: string, @Body() dto: AssignStaffDto) {
  ...
  unassignStaff(@CurrentAcademy() ctx: AcademyContext, @Param('groupId') groupId: string, @Param('userId') userId: string) {
