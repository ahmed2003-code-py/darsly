...
async function main() {
  ...
  {
    const home = await prisma.academyMembership.findFirst({
      where: { isHome: true, deletedAt: null },
    });
    if (!home) {
      check('isHome constraint reachable', false, 'no isHome=true row found to test against');
    } else {
      const otherAcademy = await prisma.academy.findFirst({
        where: { id: { not: home.academyId } },
      });
      let rejected = false;
      try {
  ...
  {
    const nonAdminLogin = await api('/auth/login', {
      method: 'POST',
      body: { email: 'nonexistent@nope.nope', password: 'x' },
    });
  ...
  check(
    'list flags: all 5 known keys present with defaults',
    Array.isArray(initial.body) && initial.body.length === 5,
  );
  ...
  const disable = await api(`/admin/academies/${academy.id}/feature-flags/attendance`, {
    token: adminToken,
    method: 'PATCH',
    body: { enabled: false },
  });
  ...
  const afterDisable = await api(`/admin/academies/${academy.id}/feature-flags`, {
    token: adminToken,
  });
  ...
  const badKey = await api(`/admin/academies/${academy.id}/feature-flags/not-a-real-flag`, {
    token: adminToken,
    method: 'PATCH',
    body: { enabled: false },
  });
  ...
  await api(`/admin/academies/${academy.id}/feature-flags/attendance`, {
    token: adminToken,
    method: 'PATCH',
    body: { enabled: true },
  });
  ...
  if (teacher) {
    ...
    const asTeacher = await api(`/admin/academies/${academy.id}/feature-flags`, {
      token: teacherToken,
    });
    check(
      'teacher (non-admin) is refused the platform-admin route',
  ...
  if (pendingPayment) {
    const before = await prisma.auditLog.count({
      where: { entity: 'Payment', action: 'payment.admin_verify' },
    });
    const verify = await api(`/admin/payments/${pendingPayment.id}/verify`, {
      token: adminToken,
      method: 'POST',
    });
    ...
    const row = await prisma.auditLog.findFirst({
      where: { entity: 'Payment', entityId: pendingPayment.id, action: 'payment.admin_verify' },
    });
  ...
  if (pendingPayout) {
    const before = await prisma.auditLog.count({
      where: { entity: 'PayoutRequest', entityId: pendingPayout.id },
    });
    const proc = await api(`/admin/payouts/${pendingPayout.id}`, {
      token: adminToken,
      method: 'PATCH',
      body: { status: 'REJECTED', note: 'phase1 verification' },
    });
