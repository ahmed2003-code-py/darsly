...
function makeDeps(
  ...
  const prisma: any = {
    ...
    academy: {
      findUnique: jest
        .fn()
        .mockResolvedValue({ id: 'centerA', teacherSharePercent: null, ...academy }),
    },
...
describe('EnrollmentsService — organisation scope is copied from the Course', () => {
  ...
  it('the enrollment mode / approval flag are resolved for the organisation, not the author', async () => {
    const { prisma, svc } = makeDeps(centerCourse, {
      kind: 'CENTER',
  ...
  it('a Center course with a price but no agreed revenue split cannot be quoted / bought', async () => {
    ...
    await expect(svc.quote('c1')).rejects.toMatchObject({
