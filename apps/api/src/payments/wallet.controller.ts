...
export class WalletController {
  ...
  @Get()
  @ApiOperation({ summary: '[academy] Wallet: balance, earnings, pending cash, recent payments + payouts (owner: the organisation; member: own slice)' })
  async wallet(@CurrentUser() user: JwtPayload, @CurrentAcademy() ctx: AcademyContext) {
    const academy = await this.prisma.academy.findUniqueOrThrow({ where: { id: ctx.academyId }, select: { id: true, kind: true, teacherSharePercent: true } });
    const wholeOrg = ctx.role === 'OWNER' || ctx.isPlatformAdmin;
    if (!wholeOrg) return this.memberSlice(user, ctx, academy);

    const academyId = academy.id;
    const [balanceCents, cashOwedCents, earnings, pendingSettlement, pendingCash, cashCollected, fees, teacherShares, payments, payouts, minSetting] = await Promise.all([
      ...
      academy.kind === 'CENTER' ? this.ledger.orgCashOwed(academyId) : this.ledger.teacherCashOwed(academyId),
      ...
      this.prisma.payment.aggregate({ where: { academyId, status: 'PAID', settledAt: null }, _sum: { netCents: true } }),
      // Cash claims waiting for the receiver to confirm them.
      this.prisma.payment.aggregate({ where: { academyId, status: 'PENDING', method: 'CASH' }, _sum: { amountCents: true }, _count: { _all: true } }),
      ...
      this.prisma.payment.aggregate({
        where: { academyId, status: 'PAID', method: 'CASH', cashReceiver: academy.kind === 'CENTER' ? 'CENTER' : 'TEACHER' },
      ...
      this.prisma.ledgerEntry.aggregate({ where: { academyId, account: 'platform:commission', direction: 'CREDIT' }, _sum: { amountCents: true } }),
      // What was credited to teachers out of this Center's sales.
      academy.kind === 'CENTER'
        ? this.prisma.ledgerEntry.aggregate({ where: { academyId, account: { startsWith: 'teacher:' }, direction: 'CREDIT' }, _sum: { amountCents: true } })
      ...
      this.prisma.payoutRequest.findMany({ where: { academyId }, orderBy: { createdAt: 'desc' }, take: 10 }),
  ...
  private async memberSlice(user: JwtPayload, ctx: AcademyContext, academy: { id: string; kind: 'PERSONAL' | 'CENTER' }) {
    const tenantId = user.tenantId;
    if (!tenantId) {
      return { scope: 'MEMBER' as const, kind: academy.kind, balanceCents: 0, cashOwedCents: 0, cashCollectedCents: 0, netCents: 0, earnedHereCents: 0, pendingCashCents: 0, pendingCashCount: 0, recentPayments: [], payouts: [] };
    }
    const account = `teacher:${tenantId}:balance`;
    const [credits, debits, cashOwedCents, earnedHere, pendingCash, cashCollected, payments] = await Promise.all([
      ...
      this.prisma.ledgerEntry.aggregate({ where: { account, academyId: academy.id, direction: 'CREDIT' }, _sum: { amountCents: true } }),
      this.prisma.payment.aggregate({ where: { academyId: academy.id, tenantId, status: 'PENDING', method: 'CASH', cashReceiver: 'TEACHER' }, _sum: { amountCents: true }, _count: { _all: true } }),
      this.prisma.payment.aggregate({ where: { academyId: academy.id, tenantId, status: 'PAID', method: 'CASH', cashReceiver: 'TEACHER' }, _sum: { amountCents: true } }),
      this.prisma.payment.findMany({
        where: { academyId: academy.id, tenantId, status: 'PAID' },
        orderBy: { paidAt: 'desc' },
        take: 10,
        include: { course: { select: { title: true } }, student: { include: { user: { select: { fullName: true } } } } },
      }),
    ]);
    ...
    const mine = await this.prisma.ledgerEntry.findMany({
      where: { account, direction: 'CREDIT', transaction: { paymentId: { in: payments.map((p) => p.id) } } },
