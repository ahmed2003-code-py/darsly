...
export default function AssignmentPage() {
  ...
  const submit = useMutation({
    mutationFn: async () =>
      (await api.post(`/lessons/${lessonId}/assignment/submissions`, { body })).data,
    ...
    <div className="mx-auto max-w-2xl px-6 py-8 sm:px-8">
        ...
        <div>
          ...
          {assignment.dueAt && (
            <p className="text-sm text-outline">
              {t('assess.assign.dueAt')}: {dateShort(assignment.dueAt)}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="card mb-4">
        ...
        <p className="mt-2 text-xs text-outline">
          {t('assess.assign.maxScore')}: {assignment.maxScore}
        </p>
      </div>
      ...
      {graded && (
        <div className="card mb-4 border-2 border-secondary">
          ...
          {mySubmission.feedback && (
            <p className="mt-2 rounded-lg bg-surface-container-low px-3 py-2 text-sm" dir="auto">
              {mySubmission.feedback}
            </p>
          )}
        </div>
      ...
      <div className="card">
        ...
        <textarea
          ...
          placeholder={t('assess.take.answerPlaceholder')}
        />
        ...
        {!locked ? (
          ...
          >
            {submit.isPending
              ? t('common.saving')
              : mySubmission
                ? t('assess.take.resubmit')
                : t('assess.take.submitAssign')}
          </button>
        ) : (
          <p className="mt-3 text-center text-sm text-outline">{t('assess.take.lockedGraded')}</p>
        )}
        {submit.isSuccess && !locked && (
          <p className="mt-2 text-center text-sm text-secondary">{t('assess.take.submitted')}</p>
        )}
      </div>
    </div>
  );
