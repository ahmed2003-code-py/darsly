...
async function probe(
  resource,
  path,
  { asOther, asTeacher, anon = true, method = 'GET', body, leak } = {},
) {
  if (anon) {
  ...
  if (asOther) {
    ...
    check(
      ...
      !bad,
      `HTTP ${r.status}${bad ? ' — RETURNED DATA' : ''}`,
    );
  }
  if (asTeacher) {
...
if (!eA || !eB)
  throw new Error('need active enrolments in two different academies — seed the database');

const A = {
  student: eA.student,
  token: await login(eA.student.user.email),
...
const B = {
  student: eB.student,
  token: await login(eB.student.user.email),
  tenantId: eB.tenantId,
  course: eB.course,
  enrolment: eB,
};

const tA = await prisma.teacherProfile.findUnique({
  where: { id: A.tenantId },
  include: { user: true },
});
const tB = await prisma.teacherProfile.findUnique({
  where: { id: B.tenantId },
  include: { user: true },
});
...
const bLesson = await prisma.lesson.findFirst({
  where: { unit: { course: { tenantId: B.tenantId } } },
});
...
const bMedia = await prisma.academyMedia
  ?.findFirst?.({ where: { academyId: B.tenantId } })
  .catch(() => null);
...
if (bPayment)
  await probe('payment', `/payments/${bPayment.id}`, {
    asOther: A.token,
    asTeacher: teacherA.token,
    leak: (b) => b?.id === bPayment.id,
  });
if (bCert)
  await probe('certificate', `/certificates/${bCert.id}`, {
    asOther: A.token,
    leak: (b) => b?.id === bCert.id,
  });
if (bThread) {
  await probe('chat thread', `/threads/${bThread.id}`, {
    asOther: A.token,
    asTeacher: teacherA.token,
    leak: (b) => b?.id === bThread.id,
  });
  await probe('chat messages', `/threads/${bThread.id}/messages`, {
    asOther: A.token,
    asTeacher: teacherA.token,
    leak: (b) => Array.isArray(b) && b.length > 0,
  });
}
if (bAttempt)
  await probe('quiz attempt', `/quiz-attempts/${bAttempt.id}`, {
    asOther: A.token,
    leak: (b) => b?.id === bAttempt.id,
  });
if (bTopup)
  await probe('wallet top-up', `/wallet/topups/${bTopup.id}`, {
    asOther: A.token,
    leak: (b) => b?.id === bTopup.id,
  });
if (bNote)
  await probe('video note (delete)', `/notes/${bNote.id}`, { asOther: A.token, method: 'DELETE' });
await probe("another student's enrolment (hide)", `/enrollments/${B.enrolment.id}/hide`, {
  asOther: A.token,
  method: 'POST',
});
...
if (bLesson) {
  await probe('lesson (delete)', `/lessons/${bLesson.id}`, {
    asTeacher: teacherA.token,
    method: 'DELETE',
  });
  await probe('lesson quiz (teacher view)', `/teacher/lessons/${bLesson.id}/quiz`, {
    asTeacher: teacherA.token,
    leak: (b) => !!b?.id,
  });
}
if (bUnit)
  await probe('course unit (delete)', `/units/${bUnit.id}`, {
    asTeacher: teacherA.token,
    method: 'DELETE',
  });
await probe("another academy's course (update)", `/teacher/courses/${B.course.id}`, {
  asTeacher: teacherA.token,
  method: 'PATCH',
  body: { title: 'IDOR PROBE — should never land' },
});
await probe("another academy's course (delete)", `/teacher/courses/${B.course.id}`, {
  asTeacher: teacherA.token,
  method: 'DELETE',
});
if (bLive) {
  await probe('live session detail', `/live/${bLive.id}/detail`, {
    asOther: A.token,
    asTeacher: teacherA.token,
    leak: (b) => b?.id === bLive.id,
  });
  await probe('live session (teacher delete)', `/teacher/live/${bLive.id}`, {
    asTeacher: teacherA.token,
    method: 'DELETE',
  });
}
if (bAttachment) {
  await probe('attachment on a GATED lesson', `/files/attachments/${bAttachment.id}`, {
    asOther: A.token,
  });
}
if (bPreviewAttachment) {
  ...
  check(
    'attachment on a FREE PREVIEW lesson',
    'stays reachable by design',
    r.status < 300,
    `HTTP ${r.status}`,
  );
}
if (bMedia)
  await probe('academy media', `/academy/media/${bMedia.id}`, {
    asTeacher: teacherA.token,
    leak: (b) => b?.id === bMedia.id,
  });
if (bPayout)
  await probe("another academy's payout", `/teacher/payouts/${bPayout.id}`, {
    asTeacher: teacherA.token,
    leak: (b) => b?.id === bPayout.id,
  });
...
if (bAcademy) {
  ...
  for (const p of ['console', 'settings', 'members', 'manage/courses']) {
    await probe(`academy ${p}`, `/academies/${bAcademy.slug}/${p}`, {
      asTeacher: teacherA.token,
      asOther: A.token,
      leak: (b) => !!b && !b.message,
    });
...
console.log('— list scoping —');
{
  ...
  check(
    'teacher course list',
    "contains only this academy's courses",
    otherTenant === 0,
    `${otherTenant} foreign courses`,
  );
  ...
  check(
    ...
    "contains only this academy's students",
    foreignStudents === 0,
    `${foreignStudents} foreign`,
  );
}
...
console.log('— confirming no write landed —');
{
  ...
  check(
    "another academy's course",
    'title unchanged by the update probe',
    course?.title !== 'IDOR PROBE — should never land',
    course?.title ?? 'gone',
  );
  check(
    "another academy's course",
    'still exists after the delete probe',
    !!course && !course.deletedAt,
    course?.deletedAt ? 'soft-deleted' : 'present',
  );
  if (bLesson) {
    const l = await prisma.lesson.findUnique({ where: { id: bLesson.id } });
    check(
      "another academy's lesson",
      'survived the delete probe',
      !!l && !l.deletedAt,
      l?.deletedAt ? 'soft-deleted' : 'present',
    );
  }
  if (bNote) {
    ...
    check(
      ...
      'survived the delete probe',
      !!n && !n.deletedAt,
      n?.deletedAt ? 'soft-deleted' : 'present',
    );
  }
  const enr = await prisma.enrollment.findUnique({ where: { id: B.enrolment.id } });
  check(
    "another student's enrolment",
    'not hidden by the probe',
    !enr?.hiddenAt,
    enr?.hiddenAt ? 'HIDDEN' : 'untouched',
  );
}
