...
const center = { id: 'c1', name: 'El Shehab', slug: 'el-shehab', kind: 'CENTER', logoUrl: null, brandTokens: null, colorPrimary: '#0f766e', colorAccent: '#f59e0b', owner: { fullName: 'Owner' } };
const teacher = { id: 't1', name: 'Mr Khaled', slug: 'khaled', kind: 'PERSONAL', logoUrl: null, brandTokens: { palette: { background: '#0b0f19', ink: '#e5e7eb', primary: '#818cf8', accent: '#f472b6' } }, colorPrimary: '#818cf8', colorAccent: '#f472b6', owner: { fullName: 'Khaled' } };
const skin = { key: 'theme-midnight-pro', nameAr: 'ميدنايت برو', nameEn: 'Midnight Pro', rarity: 'LEGENDARY', config: { accent: '#60a5fa', accentDark: '#93c5fd', secondaryDark: '#fbbf24', surfaces: { background: '#0b1020', surface: '#121a2e', ink: '#e6ecff', line: '#93c5fd' }, pattern: 'grid' } };
const tint = { key: 'theme-mint', nameAr: 'نعناع', nameEn: 'Mint', rarity: 'RARE', config: { accent: '#10b981', secondary: '#0ea5e9', wash: '#ecfdf5' } };
...
describe('AdminThemeService.catalog', () => {
  it('lists every preset, every live Academy (Centers and teachers) and every active store THEME, all resolved to the same token shape', async () => {
    ...
    expect(cat.cosmetics.map((c) => c.id)).toEqual(['cosmetic:theme-midnight-pro', 'cosmetic:theme-mint']);
    ...
    expect(prisma.cosmeticItem.findMany.mock.calls[0][0].where).toMatchObject({ category: 'THEME', isActive: true });
  });
...
describe('AdminThemeService.resolve / set / get', () => {
  it('resolves every namespace, and a bare legacy id as a preset', async () => {
    ...
    expect(prisma.academy.findFirst.mock.calls[0][0].where).toMatchObject({ id: 'c1', deletedAt: null });
    expect(prisma.cosmeticItem.findFirst.mock.calls[0][0].where).toMatchObject({ key: 'theme-midnight-pro', category: 'THEME', isActive: true });
  });
  ...
  it('refuses anything that does not exist: unknown preset, deleted academy, retired item, foreign namespace', async () => {
    ...
    const svc = new AdminThemeService(prisma);
    for (const id of ['preset:nope', 'nope', 'academy:gone', 'cosmetic:theme-retired', 'user:u1', 'academy:']) {
  ...
  it('set persists only the normalised id, for the caller only; the response carries the resolved look', async () => {
    ...
    expect(prisma.user.update).toHaveBeenCalledWith({ where: { id: 'me' }, data: { adminThemePreference: { themeId: 'preset:midnight' } } });
  ...
  it('set(null) clears', async () => {
    ...
    expect(prisma.user.update.mock.calls[0][0].data.adminThemePreference).not.toEqual(expect.objectContaining({ themeId: expect.anything() }));
  ...
  it('get resolves the stored choice; a choice that stopped existing reads as nothing chosen', async () => {
    const prisma = makePrisma();
    prisma.user.findUnique.mockResolvedValueOnce({ adminThemePreference: { themeId: 'academy:c1' } });
    ...
    prisma.user.findUnique.mockResolvedValueOnce({ adminThemePreference: { themeId: { $ne: null } } });
    expect(await svc.get('me')).toEqual({ themeId: null, theme: null });
  });
});
...
describe('token derivation', () => {
  it('a cosmetic config never reaches the tokens unfiltered — bad hexes fall back, every slot is a triple', () => {
    const palette = paletteFromCosmetic({ accent: 'javascript:alert(1)', surfaces: { background: 'nope' } } as any);
  ...
  it('a skin prefers its dark-ground accent; a tint uses the plain one', () => {
    expect(paletteFromCosmetic(skin.config as any)).toMatchObject({ primary: '#93c5fd', accent: '#fbbf24', background: '#0b1020', ink: '#e6ecff' });
    expect(paletteFromCosmetic(tint.config as any)).toMatchObject({ primary: '#10b981', accent: '#0ea5e9', background: '#ecfdf5' });
  });
});
