...
const s3 = new S3Client({
  region: /\.r2\.cloudflarestorage\.com/i.test(endpoint ?? '')
    ? 'auto'
...
async function copyFiles() {
  ...
  for await (const file of walk(ROOT)) {
    ...
    if (DRY) {
      console.log(`  would put ${key} (${size} B)`);
      copied++;
      bytes += size;
      continue;
    }
  ...
  console.log(
    `files: ${copied} copied (${(bytes / 1e6).toFixed(1)} MB), ${skipped} already there, ${failed} failed`,
  );
  return failed;
}
...
async function backfillProofs(prisma: PrismaClient) {
  ...
  const move = async (kind: 'payments' | 'topups', id: string, dataUrl: string) => {
    ...
    if (DRY) {
      console.log(`  would move ${kind}/${id} (${body.length} B) → ${key}`);
      return key;
    }
    await s3.send(
      new PutObjectCommand({
  ...
  for (const p of await prisma.payment.findMany({
    where: { proofImageUrl: { startsWith: 'data:' } },
    ...
    try {
      ...
      failed++;
      console.error(`  FAILED payment ${p.id}: ${(e as Error).message}`);
    }
  }
  for (const t of await prisma.walletTopup.findMany({
    where: { proofImageUrl: { startsWith: 'data:' } },
    select: { id: true, proofImageUrl: true },
  })) {
    try {
      ...
      if (!DRY)
        await prisma.walletTopup.update({ where: { id: t.id }, data: { proofImageUrl: key } });
      moved++;
    } catch (e) {
      failed++;
      console.error(`  FAILED topup ${t.id}: ${(e as Error).message}`);
    }
  }
  skipped =
    (await prisma.payment.count({ where: { proofImageUrl: { startsWith: 'payment-proofs/' } } })) +
    (await prisma.walletTopup.count({
      where: { proofImageUrl: { startsWith: 'payment-proofs/' } },
    }));
  console.log(
    `proofs: ${moved} moved out of Postgres, ${skipped} already objects, ${failed} failed`,
  );
  return failed;
}
...
async function verify() {
  ...
  for await (const file of walk(ROOT)) {
    ...
    if (remote === null) {
      ...
      sizeMismatch++;
      console.error(`  SIZE ${key}: local ${size} remote ${remote}`);
    } else {
      ok++;
      bytes += size;
    }
  ...
  console.log(
    `verify: ${ok} ok (${(bytes / 1e6).toFixed(1)} MB), ${missing} missing, ${sizeMismatch} size mismatch`,
  );
...
(async () => {
  if (!process.env.S3_ACCESS_KEY || !process.env.S3_SECRET_KEY) {
    console.error('S3_ACCESS_KEY / S3_SECRET_KEY are not set');
    process.exit(2);
  }
  ...
  try {
    ...
    console.log(
      failures
        ? `DONE WITH ${failures} PROBLEM(S) — do not switch STORAGE_DRIVER yet`
        : 'DONE — safe to set STORAGE_DRIVER=s3 and redeploy',
    );
