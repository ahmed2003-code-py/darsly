...
| Model | الغرض |
|---|---|
| `Subject` | المادة: `code` (مفتاح ثابت للكتالوج، بيخلّي إعادة الزرع تحدّث نفس الصفوف مش تكرّرها)، `track` = `GENERAL`/`LANGUAGES`/`BOTH`. الكتالوج ٣٣ مادة (التاريخ والجغرافيا والفلسفة وعلم النفس صف واحد `BOTH` لكل منها — بيتدرّسوا بالعربي في النظامين) |
...
| `LiveSession` → `LiveBooking` | جلسات مباشرة + حجز مقاعد (بسعة اختيارية). ومعاها الفصل نفسه: `status` (SCHEDULED/LIVE/ENDED)، `roomName`/`roomUrl` (غرفة Daily خاصة)، `startedAt`/`endedAt`، و`joinUrl` القديم للتوافق |
| `LiveAttendance` | مين كان في الغرفة فعلاً: صف واحد لكل (جلسة، مستخدم)، `joinedAt`/`lastSeenAt`/`leftAt`/`durationSeconds` |
| `LiveChatMessage` | شات الفصل. منفصل عن `ChatThread` عمداً — ده محادثة بين اتنين بس (migration `one_chat_per_pair`) وده أوضة فيها فصل. السوكيت مشترك، الموديل لأ |
...
### 2.9 الجلسات المباشرة والفصل المباشر (live)
...
**الفصل المباشر (Daily):** الجلسة المجدولة بتتحوّل لفصل فيديو حقيقي جوه درسلي.

- **المفتاح على السيرفر بس.** `DAILY_API_KEY` موجود في `live/daily.service.ts`
  ومش بيخرج منه أبداً. البراوزر بياخد **رابط غرفة + توكن قصير العمر** متخصّص
  لشخص واحد في غرفة واحدة. الغرفة `private`، فالرابط لوحده بيفتح باب مقفول.
- **الغرفة بتتعمل وقت البدء، مش وقت الجدولة** — حصة ممكن متتمش عمرها ما تحجز
  غرفة، والـ`exp` بيتحسب من ميعاد بدء معروف بدل تخمين قبلها بأسابيع.
- **مين المدرّس بيتقرّر هنا:** `is_owner` (اللي بيسمح بالكتم والإخراج) بيتحدّد من
  عضوية الأكاديمية اللي الـguard تحقّق منها، مش من أي حاجة الطلب بيطلبها.
- **دورة الحياة:** `SCHEDULED → LIVE → ENDED`. الـ`ENDED` بتتحسب كمان من الساعة
  وقت القراءة (`effectiveStatus`)، فجلسة محدش أنهاها مبتفضلش «شغّالة» للأبد
  مستنية كرون.
- **فصل واحد للمدرّس في نفس الوقت**، بيتحجز بـ`updateMany` شرطية فضغطتين في نفس
  اللحظة مش هيكسبوا الاتنين. ولو Daily فشلت، الحالة بترجع `SCHEDULED` بدل ما
  تفضل `LIVE` من غير غرفة.
  > ملاحظة: `maxConcurrentSessions` **مش** المستخدمة هنا — هي بتحكم عدد أجهزة
  > الطالب (مكافحة مشاركة الحسابات في `token.service.ts`)، ومالهاش علاقة بالبث.
- **التوافق:** جلسة قديمة فيها `joinUrl` خارجي (زوم/ميت) بتفضل شغّالة زي ما هي؛
  الفصل الداخلي بيشتغل بس لما المدرّس يدوس «ابدأ».

**الحضور (`LiveAttendance`)** — صف واحد لكل شخص لكل جلسة (`@@unique`)، فالريفريش
والـreconnect والتابين المفتوحين بيمدّدوا نفس الصف مش بيعملوا صفوف جديدة. المدة
بتتجمّع من **نبضات كل ٣٠ ثانية**، مش من `leftAt - joinedAt`: اللابتوب المقفول
والنفق والبطارية الفاضية كلهم شكلهم واحد من هنا، وفجوة أطول من ٩٠ ثانية بتتحسب
غياب. يعني الرقم معناه «وقت في الغرفة» مش «وقت من أول ما دخل».
  > **حدّ معروف:** ده heartbeat مش webhooks. لو التاب اتقفل فجأة، آخر ≤٣٠ ثانية
  > ممكن متتحسبش، والـ`leftAt` ممكن يفضل فاضي لحد ما المدرّس ينهي الجلسة (وقتها
  > بيتقفل لكل اللي جوه). مقبول لإن الرقم بيقلّ مش بيزيد — والبديل (webhooks)
  > محتاج تحقّق توقيع وendpoint عام، وده اللي بند ١٥ سمح بتأجيله.

**بعد الحصة: تسجيل ← نص ← ملخّص.** تلات مراحل، كل واحدة بحالتها
(`LivePipelineStatus`) على `LiveSession` نفسها:

- **التسجيل** بيبدأ من المتصفّح (توكن المالك هو اللي بيسمح بيه) والسيرفر بيتسجّل
  عنده الـ`recordingId`. **الرابط مش بيتخزّن أبداً** — بيتعمل عند الطلب وبينتهي
  لوحده، لأن رابط تسجيل مخزّن في صف داتابيز هو رابط عام دائم أول ما الصف يتقرا
  غلط. والطالب مبيشوفش التسجيل إلا لما المدرّس يشارك الحصة.
- **الترانسكريبت** بيبدأ أول ما المدرّس يدخل الغرفة، **مش** مربوط بالتسجيل:
  الملخّص بيتكتب من الكلام، والمدرّس اللي شرح من غير ما يسجّل لسه عايز ملخّص.
...
- **السبب بيتقال صح، مش بيتخمّن.** نص فاضي ليه معنيين مختلفين تماماً — «محدش
  ...
  وبرضو السيرفر بيسأل الحساب نفسه (`DailyService.transcriptionAvailable`) عشان
  المدرّس اللي قفل التاب مبيبلّغش حاجة. أي واحد فيهم بيدّي
  `TRANSCRIPTION_UNAVAILABLE` بدل `NO_TRANSCRIPT`. ولو مقدرناش نسأل أصلاً،
  بنقول `NO_TRANSCRIPT` — «مش عارفين» متتحوّلش لاتّهام للحساب.
- **الملخّص** بيشتغل على طابور `AiJob` **الموجود** (نفس الـleasing والretry) عبر
  `LIVE_SUMMARY` handler، وبيستخدم `AiClient.completeStructured` يعني الـschema
  مفروض من OpenAI نفسها — مفيش JSON حر يتفك ويتصلّح.
  - **القاعدة اللي الميزة كلها قايمة عليها:** النص هو المصدر الوحيد. الـprompt
    بيقول كده صراحة، والschema فيها مكان لـ«مفيش» في كل قسم ممكن يغري النموذج
    إنه يخترع. اتجرّب على نص فيه واجب وعلى نفس النص من غير واجب: طلّع الواجب
    بالظبط في الأولى و`[]` في التانية.
  - **Idempotent**: لو الحالة `READY` أو `PROCESSING` مبيتعملش job تاني —
    ضغطتين من المدرّس أو webhook مكرّر مش هيصرفوا نداءين للنموذج.
  - النص بيتخزّن عندنا أول مرة، فإعادة التوليد مش معتمدة على إن المزوّد لسه
    ماسك نسخته.
- **الرؤية**: الملخّص ملك المدرّس لحد ما يشاركه. الطالب اللي متشاركش معاه بيتقاله
  «مفيش ملخّص» مش «ممنوع» — مش محتاج يعرف إن فيه حاجة مدرّسه لسه بيراجعها.
...
- **متعدّد النسخ (multi-instance)**: الإنتاج بيشغّل أكتر من نسخة (replica) ورا
  الـload balancer. `RedisIoAdapter` (`redis/redis-io.adapter.ts`) بينشر
  الـrooms والبثّ بين النسخ عن طريق `@socket.io/redis-adapter` — من غيره رسالة
  مبعوتة من مستخدم واصل لنسخة معيّنة ما توصلش لمستقبِل واصل لنسخة تانية (اتأكدت
  فعلياً على الإنتاج قبل الإصلاح). فشل Redis مؤقت بيرجّع السلوك لبثّ داخل
  النسخة الواحدة بس (fail-open) بدل ما يقطع أي اتصال — التفاصيل في
  [docs/DEPLOYMENT.md §3c](./DEPLOYMENT.md).
...
1. **ThrottlerGuard** — حد معدّل عام: 120 طلب / 60 ثانية، وحدود أضيق على
   `login`/`forgot-password` (20 و5 على التوالي). العدّاد بقى موزّع عبر Redis
   (`redis/redis-throttler-storage.service.ts`) بدل الميموري المحلية — من غيره
   كل نسخة (replica) كانت بتعدّ لوحدها، فالحد الفعلي بيتضاعف بعدد النسخ (اتأكد
   فعلياً: ٢١ محاولة اتقسّمت على نسختين ما رجّعوش 429 خالص). فشل Redis المؤقت
   بيخلّي الحد يوقف عن التطبيق (fail-open) بدل ما يرفض كل الطلبات — سياسة
   متعمّدة، التفاصيل في [docs/DEPLOYMENT.md §3c](./DEPLOYMENT.md).
