...
> **تحديث (بعد B.6):** B.6 اتعمله push واتنشر (`0d6dbff`)، والـ production smoke عدّى 15/15 على Cloudflare بـ TURN.

### Checkpoint B.7 — UX بمستوى premium، والتحقق من البيانات، وموثوقية معالجة الميديا — **متعمله push ومتنزّل (7079621، 5e0e05d)**

**قواعد الجلسة (مصدر واحد):**
- `LIVE_SESSION_RULES` + `validateLiveSession` في `@darsly/shared-types`، والـ API والفورم الاتنين بيقروا منها.
- العنوان 2–160، والوصف ≤ 1000، والمدة **5–720 دقيقة** (الحد الأدنى الحقيقي كان 5 في الـ DTO، مش 10)، والسعة 1–100,000 أو فاضية.
- **جديد:** الموعد ميبقاش في الماضي (بسماح 5 دقايق).
- الـ API بيرفض بـ `fields: [{field, code, params}]`، والفورم بيحط كل رسالة تحت الحقل بتاعها.

**الفصل:**
- lobby للمدرس فيه preview للكاميرا، ومؤشر للميكروفون، واختيار الأجهزة، ورسايل واضحة لكل مشكلة في الأجهزة. والطالب بيدخل يستمع من غير ما المتصفح يطلب منه أي إذن.
- shell غامق فيه top bar، و stage بالأولوية (الشاشة، وبعدها المدرس/المتكلم)، وأوضاع تلقائي/تركيز/شبكة، و pin محلي.
- لوحة جانبية واحدة (المشاركون/الشات)، وطلبات التحدث أول حاجة فيها، وكمان كارت يطلع للمدرس على الـ stage.
- dock مقسّم لمجموعات، و"إنهاء الفصل" منفصل وليه تأكيد.
- **عدد المشاركين بقى من الاتصالات المفتوحة بالغرفة**، مش من الـ attendance (الـ lobby كانت بتتحسب قبل كده).

**التسجيل:**
- أوقات لكل مرحلة (`claimedAt`، `captureStartedAt`، `handedAt`، `readyAt`).
- الطلب اللي محدش استلمه خلال دقيقتين، أو اللي الفصل خلص قبل ما يبدأ، **بيتقفل كـ "لم يبدأ"**، ومبقاش بيفضل "جاري التجهيز" للأبد. ده بالظبط اللي حصل في production، لأنه مفيش recorder service.
- الـ finalize والـ sync بقوا بيشتغلوا على الـ API كمان (ffmpeg بس، من غير Chrome).

**النص والملخص:** اتفصلوا لمرحلتين.
- ملخص حصة Cloudflare من غير نص **بيترفض قبل ما يتعمل job** (`TRANSCRIPT_NOT_READY`).
- والصفحة بتقول "في انتظار النص"، أو "مفيش نص"، بدل `NO_TRANSCRIPT`.

**الاختبارات:**
- اختبارات الـ live في الـ API: **289/289** (منهم integration على Postgres حقيقي).
- web **88/88**.
- typecheck + route check.
- E2E على app الـ Cloudflare الحقيقي: الفصل **26/26**، النهاية **16/16**، الشبكة **8/8**، والتسجيل لحد HLS READY (3 failures في سكريبتات الاختبار نفسها مش في المنتج).
- الصور في مجلد الـ scratchpad `b7-shots`.

**لسه:**
- ARABIC BENCHMARK PENDING INPUT. الـ harness بقى بيقيس كمان المصطلحات الإنجليزي والأرقام والتشكيل.
- أجهزة موبايل حقيقية.
- مشاهدة التسجيل جوه التطبيق = Checkpoint C.

### Checkpoint B.8 — فورم أسهل، وتفاعلات وأصوات، وشات محفوظ، ونص الحصة — **IMPLEMENTED محلياً، مش متعمله push**

**الفورم:**
- العنوان والوصف، وبعدهم "إمتى؟" بـ chips (دلوقتي / النهارده / بكرة) وتاريخ ووقت بنظام 12 ساعة.
- المدة chips (30/45/ساعة/90/ساعتين/مخصص)، والسعة (بدون حد / حد أقصى).
- الـ joinUrl الخارجي في "خيارات متقدمة"، وسطر في الآخر بيقرا الموعد: "اليوم · الوقت · المدة".
- نفس قواعد B.7 بالظبط (`validateLiveSession`).

**التفاعلات والأصوات:**
- 6 تفاعلات (`live:react` → `live:reaction`) لكل اللي في الغرفة. بتطير في الركن مع اسم اللي بعتها.
- حد أقصى 5 كل 5 ثواني لكل شخص (`ReactionLimiter`)، ومسموح بس للي جوه `live:{id}`.
- أصوات خفيفة (WebAudio، مفيش ملفات) لما حد يدخل أو يخرج، أو يرفع إيده (للمدرس)، أو تيجي رسالة. وفيه زرار كتم بيتحفظ في المتصفح.

**الشات في سجل الحصة:** الرسايل بتظهر في صفحة الحصة بعد ما تخلص، للمدرس والطلاب الحاجزين (آخر 5، و"عرض الكل").

**نص الحصة (Cloudflare) — مقفول افتراضياً:**
- ليه مكانش شغال: Cloudflare بينقل ميديا بس، ومفيش recorder service على Railway، ومفيش مزوّد STT.
- الحل: صفحة المدرس بتخلط الميكروفون بتاعه مع صوت الطلاب المسموح لهم يتكلموا (AudioContext → MediaRecorder، 32kbps).
  - بترفعها قطع مستقلة كل 3 دقايق على `POST /teacher/live/:id/audio/:seq`، و seq = ثانية بداية القطعة (مبيتكررش لو المدرس عمل reload).
  - القطع الساكتة مبتترفعش.
  - Safari بيبعت MP4، والامتداد بيفضل `.m4a`.
- بتتخزن private في `source/live-audio/...` (جدول `LiveAudioSegment`).
- لما الحصة تخلص بيتعمل job `LIVE_TRANSCRIBE`:
  - بيستنى 45 ثانية لآخر قطعة.
  - بيفرّغ كل قطعة (`gpt-4o-mini-transcribe`، `ar`) وبيحفظ نص كل قطعة على الصف بتاعها، فالـ retry مبيدفعش مرتين.
  - بيجمع النص في `transcriptText` = READY، وبيمسح الصوت.
  - التكلفة بتتحسب في `AI_MONTHLY_BUDGET_CENTS`.
- الطلاب بيشوفوا "التفريغ النصي مفعّل" في الـ top bar.
- `LIVE_TRANSCRIPTION_ENABLED=false` افتراضياً. تشغيله محتاج موافقة (مكالمات مدفوعة) + `AI_ACADEMY_ENABLED` + worker.

**الاختبارات:**
- API **2465/2465** (164 suite).
- web **92/92**.
- typecheck + route check.
- E2E على app الـ Cloudflare الحقيقي **20/20**:
  - الفورم؛
  - التفاعلات والـ limit؛
  - الشات في السجل؛
  - قطعة WebM حقيقية اترفعت وقت النهاية؛
  - مفيش أي مكالمة مدفوعة.
