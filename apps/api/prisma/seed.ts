/**
 * Demo seed — Academy SaaS model. WIPES all data, then generates a rich dataset:
...
async function main() {
  ...
  // Platform receiving accounts + payout minimum
  ...
  for (let i = 0; i < totalStudents; i++) {
    ...
    const user = await prisma.user.create({
      data: { role: 'STUDENT', email: `student${i + 1}@darsly.app`, passwordHash: hash, fullName: name },
    });
    const profile = await prisma.studentProfile.create({
      data: { userId: user.id, gradeId: pick(grades), currentStreak: rand(12), longestStreak: rand(30) },
    });
  ...
  for (let ti = 0; ti < TEACHERS.length; ti++) {
    ...
    const tUser = await prisma.user.create({
      data: { role: 'TEACHER', email: `teacher${ti + 1}@darsly.app`, passwordHash: hash, fullName: T.name },
    });
    const tp = await prisma.teacherProfile.create({
      data: { userId: tUser.id, slug: T.slug, bio: T.tagline, subjectId: subjects[T.subject], status: 'APPROVED', verifiedAt: new Date(), commissionPercent: 20 },
    });
    // Academy.id === TeacherProfile.id (identity-preserving) → tenantId works everywhere.
    const academy = await prisma.academy.create({
      data: {
        id: tp.id, slug: T.slug, name: T.name.replace('أ. ', 'أكاديمية '), status: 'ACTIVE',
        ownerUserId: tUser.id, tagline: T.tagline, colorPrimary: T.color, colorAccent: T.color, feeType: 'PERCENT', feeValue: 20,
      },
    });
    ...
    // 4 courses with units + lessons
    const courses: { id: string; priceCents: number }[] = [];
    for (let ci = 0; ci < COURSE_TEMPLATES.length; ci++) {
      const CT = COURSE_TEMPLATES[ci];
      const course = await prisma.course.create({
        data: {
          tenantId: academy.id, title: `${subjName} — ${CT.suffix}`,
          description: `كورس ${CT.suffix} في ${subjName} مع ${T.name}.`,
          subjectId: subjects[T.subject], gradeId: grades[(ti + ci) % grades.length], status: 'PUBLISHED',
          pricingModel: CT.model as any, priceCents: CT.price, defaultViewsCap: 3,
        },
      });
      for (let u = 0; u < 2; u++) {
        ...
        for (let l = 0; l < 4; l++) {
          await prisma.lesson.create({
            data: { unitId: unit.id, title: `الدرس ${u * 4 + l + 1}`, type: 'VIDEO', sortOrder: l, durationSec: 480 + rand(600), isFreePreview: u === 0 && l === 0 },
          });
        }
      }
    ...
    // 12 students for this academy (home), enrol each in ~2 courses; ~65% pay.
      ...
      for (const c of chosen) {
        ...
        if (Math.random() < 0.65 && c.priceCents > 0) {
          ...
          const payment = await prisma.payment.create({
            data: {
              studentId: st.id, courseId: c.id, enrollmentId: enr.id, tenantId: academy.id,
              amountCents: total, feeCents: fee, netCents: net, status: 'PAID', gateway: 'manual',
              method: pick(['INSTAPAY', 'VODAFONE_CASH']) as any, paidAt: new Date(),
            },
          });
          await prisma.ledgerTransaction.create({
            data: {
              description: `enrollment payment ${payment.id}`, paymentId: payment.id,
              entries: {
                create: [
    ...
    // Cross-academy demo: first 6 students of the NEXT academy also join this one.
    ...
    for (const st of cross) {
      await prisma.academyMembership.upsert({
        ...
        update: {},
        create: { userId: st.userId, academyId: academy.id, role: 'STUDENT', status: 'ACTIVE', isHome: false, joinedAt: new Date() },
      });
      const c = pick(courses);
      await prisma.enrollment.upsert({
        where: { studentId_courseId: { studentId: st.id, courseId: c.id } },
        update: {},
        create: { studentId: st.id, courseId: c.id, tenantId: academy.id, status: 'ACTIVE', approvedAt: new Date() },
      });
    }

    console.log(`✓ ${academy.name}: ${courses.length} courses, 12 students`);
  }

  const [uCount, aCount, cCount, eCount] = await Promise.all([
    prisma.user.count(), prisma.academy.count(), prisma.course.count(), prisma.enrollment.count(),
  ]);
  console.log(`\n✅ seed done — users=${uCount} academies=${aCount} courses=${cCount} enrolments=${eCount} payments=${paymentsCreated}`);
  console.log(`   password for EVERYONE: ${PASSWORD}`);
  console.log(`   admin: admin@darsly.app · teachers: teacher1..5@darsly.app · students: student1..${totalStudents}@darsly.app`);
}
