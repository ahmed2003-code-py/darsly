...
async function main() {
  ...
  check(
    'roster: has real students with expected shape',
    roster.body?.students?.[0]?.fullName !== undefined &&
      typeof roster.body?.students?.[0]?.isActive === 'boolean',
  );
  ...
  const sharedStudent = roster.body?.students?.find((s) =>
    rosterB.body?.students?.some((b) => b.id === s.id),
  );
  if (sharedStudent) {
    const inB = rosterB.body.students.find((b) => b.id === sharedStudent.id);
    check(
      'a student in both rosters shows academy-specific enrollment counts, not a shared total',
      sharedStudent.enrollmentsCount >= 1 && inB.enrollmentsCount >= 1,
    );
  }
  ...
  const createGroup = await api('/teacher/groups', {
    token: tokenA,
    method: 'POST',
    body: { name: 'Verify Phase3 Group', description: 'test fixture' },
  });
  ...
  const dupMembers = await api(`/teacher/groups/${groupId}/members`, {
    token: tokenA,
    method: 'POST',
    body: { studentIds: [students[0].id] },
  });
  ...
  if (foreignStudent) {
    const badAdd = await api(`/teacher/groups/${groupId}/members`, {
      token: tokenA,
      method: 'POST',
      body: { studentIds: [foreignStudent.id] },
    });
  ...
  console.log('\n-- DB constraints --');
  {
    ...
    try {
      await prisma.groupMembership.create({
        data: { groupId, studentId: students[0].id, academyId: academy1 },
      });
    } catch (e) {
  ...
  const bBeforeAssign = await api(`/teacher/groups/${groupId}`, {
    token: tokenB,
    headers: { 'X-Academy-Id': academy1 },
  });
  check(
    'teacherB (staff of academy1, group.manage capability, NOT assigned to this group) is refused: 403',
    bBeforeAssign.status === 403,
  );

  const assign = await api(`/teacher/groups/${groupId}/assignments`, {
    token: tokenA,
    method: 'POST',
  ...
  const bAfterAssign = await api(`/teacher/groups/${groupId}`, {
    token: tokenB,
    headers: { 'X-Academy-Id': academy1 },
  });
  ...
  const bTriesToAssignSomeoneElse = await api(`/teacher/groups/${groupId}/assignments`, {
    ...
    method: 'POST',
    headers: { 'X-Academy-Id': academy1 },
    body: { userId: teacherC.user.id, role: 'ASSISTANT' },
  });
  check(
    'teacherB (assigned TEACHER, not OWNER) cannot assign staff to the group: 400',
    bTriesToAssignSomeoneElse.status === 400,
  );

  // ── Cross-academy isolation ──────────────────────────────────────────────
  console.log('\n-- Cross-academy isolation --');
  const cNoMembership = await api(`/teacher/groups/${groupId}`, {
    token: tokenC,
    headers: { 'X-Academy-Id': academy1 },
  });
  check(
    'teacherC (OWNER of a wholly separate academy, no membership in academy1) refused: 401/403/404',
  ...
  check(
    "IDOR: teacherC in their OWN academy context naming academy1's groupId gets 404, not the group",
    cOwnAcademyForeignGroup.status === 404,
  );

  // ── Attendance ────────────────────────────────────────────────────────────
  console.log('\n-- Attendance --');
  const today = new Date().toISOString().slice(0, 10);
  const session = await api(`/teacher/groups/${groupId}/attendance?date=${today}`, {
    token: tokenA,
  });
  check(
    'get attendance session (none marked yet): 200, full roster, all null',
    session.status === 200 && session.body?.students?.every((s) => s.status === null),
  );
  ...
  check(
    'marked statuses reflected back',
    mark.body?.students?.find((s) => s.studentId === students[0].id)?.status === 'ABSENT',
  );

  const remark = await api(`/teacher/groups/${groupId}/attendance`, {
    token: tokenA,
    method: 'POST',
    body: { date: today, records: [{ studentId: students[0].id, status: 'LATE' }] },
  });
  check(
    're-marking the same date updates in place (no duplicate session)',
    remark.status < 300 &&
      remark.body?.students?.find((s) => s.studentId === students[0].id)?.status === 'LATE',
  );

  const sessionsForGroup = await prisma.attendanceSession.count({
    where: { groupId, date: new Date(today) },
  });
  check(
    'exactly one AttendanceSession exists for (group, date) despite two mark calls',
    sessionsForGroup === 1,
  );
  ...
  const badStudentMark = await api(`/teacher/groups/${groupId}/attendance`, {
    ...
    body: {
      date: today,
      records: [{ studentId: foreignStudent?.id ?? 'nonexistent', status: 'PRESENT' }],
    },
  });
  ...
  check(
    "student attendance history: 200, includes today's LATE",
    history.status === 200 && history.body?.some((h) => h.status === 'LATE'),
  );

  const teacherBHistory = await api(`/teacher/students/${students[0].id}/attendance`, {
    token: tokenB,
    headers: { 'X-Academy-Id': academy1 },
  });
  check(
    "teacherB (assigned to this group) can see this student's history too",
    teacherBHistory.status === 200 && teacherBHistory.body?.length > 0,
  );
  ...
  check(
    'needs-attention: 200, has all three sections',
    needsAttention.status === 200 &&
      ['repeatedAbsences', 'inactiveStudents', 'staleGroups'].every(
        (k) => k in (needsAttention.body ?? {}),
      ),
  );
  ...
  if (studentUser) {
    ...
    for (const [label, req] of [
      ...
      [
        'mark attendance',
        () =>
          api(`/teacher/groups/${groupId}/attendance`, {
            token: studentToken,
            method: 'POST',
  ...
  const disableGroups = await api(`/admin/academies/${academy1}/feature-flags/groups`, {
    token: adminToken,
    method: 'PATCH',
    body: { enabled: false },
  });
  ...
  const reenable = await api(`/admin/academies/${academy1}/feature-flags/groups`, {
    token: adminToken,
    method: 'PATCH',
    body: { enabled: true },
  });
  ...
  // ── Soft delete ───────────────────────────────────────────────────────────
  console.log('\n-- Soft delete --');
  const archive = await api(`/teacher/groups/${groupId}`, {
    token: tokenA,
    method: 'PATCH',
  ...
  check(
    'an ARCHIVED group is still readable (archived, not deleted)',
    stillVisible.status === 200 && stillVisible.body?.status === 'ARCHIVED',
  );
