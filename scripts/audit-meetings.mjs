...
const note = (n, d) => { console.log(`   BLOCKED  ${n}  (${d})`); blocked++; };
...
function connect(token) {
  return new Promise((resolve) => {
    const s = io(WS, { auth: { token }, transports: ['websocket'], reconnection: false, timeout: 6000 });
...
try {
  ...
  const sessions = await prisma.liveSession.findMany({
    where: { deletedAt: null }, include: { teacher: { include: { user: true } }, bookings: true }, take: 50,
  });
  ...
  const bookedStudent = sessA.bookings[0]
    ? await prisma.studentProfile.findUnique({ where: { id: sessA.bookings[0].studentId }, include: { user: true } })
    : null;
  const bookedTok = bookedStudent ? await login(bookedStudent.user.email) : null;

  const outsiderUser = await prisma.user.create({
    data: { email: `${tag}-outsider@test.invalid`, fullName: 'Meeting Outsider', passwordHash: await argon2.hash(PASSWORD), role: 'STUDENT', isActive: true },
  });
  ...
  console.log(`session A ${sessA.id.slice(0, 10)}  academy ${sessA.tenantId.slice(0, 10)}  teacher ${sessA.teacher.user.email}`);
  console.log(`session B ${sessB.id.slice(0, 10)}  academy ${sessB.tenantId.slice(0, 10)}  teacher ${teacherB.profile.user.email}`);
  console.log(`booked student: ${bookedStudent?.user.email ?? '(none booked — booking probes will be limited)'}\n`);
  ...
  console.log('=== 1. JOIN AUTHORIZATION (REST) ===');
  {
    ...
    const teacherJoinAsStudentRoute = await api(`/live/${sessA.id}/join`, { token: teacherA.token });
    check("session A's own teacher cannot use the STUDENT join route", teacherJoinAsStudentRoute.status >= 400,
    ...
    check('a nonexistent session id is refused, not 500', wrongId.status >= 400 && wrongId.status < 500, `HTTP ${wrongId.status}`);
  }

  // ── 2. cross-teacher / cross-academy control ─────────────────────────────
  console.log('\n=== 2. CROSS-TEACHER AND CROSS-ACADEMY CONTROL ===');
  {
    const start = await api(`/teacher/live/${sessB.id}/start`, { method: 'POST', token: teacherA.token });
    check("teacher A cannot start teacher B's session", start.status >= 400, `HTTP ${start.status}`);

    const end = await api(`/teacher/live/${sessB.id}/end`, { method: 'POST', token: teacherA.token });
    check("teacher A cannot end teacher B's session", end.status >= 400, `HTTP ${end.status}`);

    const cancel = await api(`/teacher/live/${sessB.id}`, { method: 'DELETE', token: teacherA.token });
    check("teacher A cannot cancel teacher B's session", cancel.status >= 400, `HTTP ${cancel.status}`);

    const attendance = await api(`/teacher/live/${sessB.id}/attendance`, { token: teacherA.token });
    check("teacher A cannot read teacher B's attendance", attendance.status >= 400, `HTTP ${attendance.status}`);

    const bookings = await api(`/teacher/live/${sessB.id}/bookings`, { token: teacherA.token });
    check("teacher A cannot list teacher B's bookings", bookings.status >= 400, `HTTP ${bookings.status}`);

    const rec = await api(`/teacher/live/${sessB.id}/recording/start`, { method: 'POST', token: teacherA.token, body: { recordingId: 'x' } });
    check("teacher A cannot mark teacher B's session recording", rec.status >= 400, `HTTP ${rec.status}`);

    const summary = await api(`/teacher/live/${sessB.id}/summary`, { method: 'POST', token: teacherA.token });
    check("teacher A cannot queue a summary for teacher B's session", summary.status >= 400, `HTTP ${summary.status}`);

    const vis = await api(`/teacher/live/${sessB.id}/summary/visibility`, { method: 'PATCH', token: teacherA.token, body: { visible: true } });
    check("teacher A cannot toggle teacher B's summary visibility", vis.status >= 400, `HTTP ${vis.status}`);
  }
  ...
  console.log('\n=== 3. CLASSROOM CONTENT (detail / chat / recording) ===');
  {
    ...
    check("an outsider cannot read another academy's session detail", detail.status >= 400, `HTTP ${detail.status}`);

    const chatRead = await api(`/live/${sessB.id}/chat`, { token: outsiderTok });
    check("an outsider cannot read another academy's classroom chat", chatRead.status >= 400, `HTTP ${chatRead.status}`);

    const chatSend = await api(`/live/${sessB.id}/chat`, { method: 'POST', token: outsiderTok, body: { body: `${tag} injected` } });
    check("an outsider cannot send into another academy's classroom chat", chatSend.status >= 400, `HTTP ${chatSend.status}`);
    const landed = await prisma.chatMessage.count({ where: { body: { contains: tag } } }).catch(() => 0);
    ...
    check("an outsider cannot mint a link to another academy's recording", recording.status >= 400, `HTTP ${recording.status}`);
    ...
    const attBefore = await prisma.liveAttendance.count({ where: { sessionId: sessB.id, userId: outsiderUser.id } });
    const heartbeat = await api(`/live/${sessB.id}/heartbeat`, { method: 'POST', token: outsiderTok });
    const attAfter = await prisma.liveAttendance.count({ where: { sessionId: sessB.id, userId: outsiderUser.id } });
    check("an outsider's heartbeat is a genuine no-op for another academy's session",
  ...
  console.log('\n=== 4. TEACHER-ONLY CONTROLS, ATTEMPTED BY A STUDENT ===');
  {
    ...
    const start = await api(`/teacher/live/${sessA.id}/start`, { method: 'POST', token: asStudent });
    ...
    check("a student cannot read the teacher's attendance view", attendance.status >= 400, `HTTP ${attendance.status}`);
    const vis = await api(`/teacher/live/${sessA.id}/summary/visibility`, { method: 'PATCH', token: asStudent, body: { visible: true } });
  ...
  console.log('\n=== 5. MEETING ID TAMPERING ===');
  {
    for (const bad of [FOREIGN_ID, "' OR '1'='1", '../../../etc/passwd', '<script>1</script>', '0']) {
      const r = await api(`/live/${encodeURIComponent(bad)}/join`, { token: outsiderTok });
      check(`malformed id "${bad.slice(0, 20)}" is refused, not 500`, r.status >= 400 && r.status < 500, `HTTP ${r.status}`);
    }
  }
  ...
  if (bookedTok) {
    ...
    check('two simultaneous heartbeats both resolve without a server error', a.status < 500 && b.status < 500, `${a.status}, ${b.status}`);

    const [j1, j2] = await Promise.all([
      api(`/live/${sessA.id}/join`, { token: bookedTok }),
      api(`/live/${sessA.id}/join`, { token: bookedTok }),
    ]);
    check('two simultaneous joins by the same booked student both resolve cleanly', j1.status < 500 && j2.status < 500, `${j1.status}, ${j2.status}`);
    if (j1.status >= 400 && j2.status >= 400) {
      note('legitimate join outcome', `the booked-student join itself returns ${j1.status} — see the Daily-credentials note below`);
    }
  } else {
  ...
  console.log('\n=== 7. SOCKET.IO CONSISTENCY WITH REST ===');
  {
    ...
    const heard = [];
    outsiderSocket.onAny((name) => { if (String(name).startsWith('live:')) heard.push(name); });
    ...
    check('the socket does not emit anything from a room the outsider was refused', heard.length === 0, `${heard.length} event(s)`);
    ...
    if (bookedTok) {
      ...
      check('a genuinely booked student can join and leave the socket room without error', before, before ? 'connected' : 'never connected');
      ownSocket.close();
    }

    const anonSocket = await connect(null);
    check('an anonymous socket cannot even reach the live:join handler (rejected at handshake)', !anonSocket.connected, anonSocket.connected ? 'connected!' : 'refused');
    anonSocket.close();
  }

  // ── 8. the one thing genuinely blocked ───────────────────────────────────
  console.log('\n=== 8. WHAT COULD NOT BE TESTED ===');
  note('a legitimately booked student receiving a real Daily meeting token',
  ...
  note('WebRTC/SDP/ICE signaling itself',
