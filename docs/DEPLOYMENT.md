...
- **التفريغ (Transcription) لازم يتفعّل على حساب Daily نفسه — ومش مفعّل حالياً.**
  ده إعداد على مستوى الدومين مش على مستوى الغرفة: لما الحساب مفيهوش مزوّد تفريغ،
  `enable_transcription` بيرجع `None` من `GET https://api.daily.co/v1/`، وإنشاء
  الغرفة بـ`enable_transcription_storage` **بينجح عادي** ومفيش أي غلط بيظهر —
  الفشل بيحصل بعدين جوّه الحصة (`transcription-error`) والملف اللي بيتخزّن
  بيبقى فيه سطر `WEBVTT` وبس.
  - **تتفعّل إزاي:** [dashboard.daily.co](https://dashboard.daily.co) →
    Transcription، وتوصيل مزوّد (Deepgram). من غير كده مفيش ملخّص لأي حصة.
