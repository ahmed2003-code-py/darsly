  ...
  return (
    <div>
      ...
      <p className="mt-2 text-xs text-outline">
        {t('groups.pickerHint', { count: selected.size })}
      </p>
    </div>
  );
}
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {data.groups.map((g) => (
            ...
            >
              ...
              {g.description && (
                <p className="mb-3 line-clamp-2 text-sm text-on-surface-variant">{g.description}</p>
              )}
              <div className="mt-auto flex items-center justify-between border-t border-outline-variant/50 pt-3 text-sm">
                <span className="text-on-surface-variant">
                  {t('groups.studentsCount', { count: g.studentsCount })}
                </span>
                <span className="text-outline">
                  {g.staff.map((s) => s.name).join(', ') || t('groups.noStaff')}
                </span>
              </div>
            </Link>
          ))}
        ...
        <div className="grid gap-3">
          {data.students.map((s) => (
            <div key={s.id} className="card flex flex-wrap items-center gap-4 p-4">
              ...
              <Badge tone={s.isActive ? 'teal' : 'neutral'}>
                {s.isActive ? t('groups.active') : t('groups.inactive')}
              </Badge>
              <span className="text-xs text-outline">
                {s.lastActivityAt
                  ? t('groups.lastActive', { date: dateShort(s.lastActivityAt) })
                  : t('groups.noActivityYet')}
              </span>
            </div>
          ))}
        </div>
...
function AttentionTab() {
  ...
  const nothing =
    !data?.repeatedAbsences.length && !data?.inactiveStudents.length && !data?.staleGroups.length;
  ...
  return (
    <div className="grid gap-6">
            ...
            {data.inactiveStudents.map((s) => (
              <div key={s.studentId} className="card flex items-center justify-between p-4">
                ...
                <span className="text-sm text-on-surface-variant">
                  {s.lastActivityAt ? dateShort(s.lastActivityAt) : t('groups.noActivityYet')}
                </span>
              </div>
            ))}
          </div>
      ...
      {!!data?.staleGroups.length && (
        <section>
          <h3 className="mb-3 font-heading font-bold text-on-surface-variant">
            {t('groups.staleGroups')}
          </h3>
          <div className="grid gap-2">
            {data.staleGroups.map((g) => (
              <Link
                key={g.groupId}
                to={`/teacher/groups/${g.groupId}`}
                className="card card-hover flex items-center justify-between p-4"
              >
                <span className="font-bold">{g.name}</span>
                <span className="text-sm text-on-surface-variant">
                  {g.lastSessionAt ? dateShort(g.lastSessionAt) : t('groups.neverTaken')}
                </span>
              </Link>
            ))}
          </div>
