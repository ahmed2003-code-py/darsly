...
async function login(email) {
  ...
  if (r.status >= 300) throw new Error(`login ${email} failed: ${r.status} ${JSON.stringify(r.body).slice(0, 120)}`);
...
const students = await prisma.studentProfile.findMany({
  take: 2, include: { user: { select: { email: true } } },
  where: { user: { isActive: true } },
});
const teacher = await prisma.teacherProfile.findFirst({
  where: { status: 'APPROVED' }, include: { user: { select: { email: true } } },
});
if (students.length < 2 || !teacher) throw new Error('seed did not provide two students and a teacher');
...
console.log('\n=== AUTHENTICATION BOUNDARIES ===');
{
  ...
  const forged = jwt.default.sign({ sub: A.userId, role: 'SUPER_ADMIN' }, 'attacker-chosen-secret-aaaaaaaaaaaaaaa', { expiresIn: '1h' });
  const wrongSig = await req('/auth/me', { token: forged });
  check('token signed with the wrong secret is refused', wrongSig.status === 401, `got ${wrongSig.status}`);

  // alg:none — the classic algorithm-confusion attempt.
  const header = Buffer.from(JSON.stringify({ alg: 'none', typ: 'JWT' })).toString('base64url');
  const payload = Buffer.from(JSON.stringify({ sub: A.userId, role: 'SUPER_ADMIN', exp: Math.floor(Date.now() / 1000) + 3600 })).toString('base64url');
  const none = await req('/auth/me', { token: `${header}.${payload}.` });
  check('alg:none token is refused', none.status === 401, `got ${none.status}`);

  const expired = jwt.default.sign({ sub: A.userId, role: 'STUDENT' }, process.env.JWT_ACCESS_SECRET ?? 'x', { expiresIn: '-1s' });
...
console.log('\n=== USER ENUMERATION ===');
{
  const unknown = await req('/auth/login', { method: 'POST', body: { email: 'nobody-here@test.invalid', password: 'whatever' } });
  const known = await req('/auth/login', { method: 'POST', body: { email: A.user.email, password: 'definitely-wrong' } });
  check('unknown and known emails fail alike',
    unknown.status === known.status && JSON.stringify(unknown.body?.message) === JSON.stringify(known.body?.message),
...
console.log('\n=== AUTHORIZATION / PRIVILEGE ===');
{
  ...
  check('student cannot reach an admin route', asStudent.status === 403 || asStudent.status === 401, `got ${asStudent.status}`);

  const asTeacher = await req('/admin/payment-events', { token: tokenT });
  check('teacher cannot reach an admin route', asTeacher.status === 403 || asTeacher.status === 401, `got ${asTeacher.status}`);

  const anonAdmin = await req('/admin/payment-events');
  check('anonymous cannot reach an admin route', anonAdmin.status === 401, `got ${anonAdmin.status}`);

  const ingest = await req('/payment-events', { method: 'POST', body: { provider: 'VODAFONE_CASH', amountCents: 999999 }, headers: { 'x-listener-key': 'wrong-key-entirely' } });
  check('transfer ingestion refuses a wrong listener key', ingest.status === 401, `got ${ingest.status}`);
}
...
console.log('\n=== CROSS-ACCOUNT (IDOR spot-check) ===');
{
  ...
  if (bPayment) {
    ...
    check("student A cannot read student B's payment", cross.status === 403 || cross.status === 404, `got ${cross.status}`);
  } else {
  ...
  if (bEnrolment) {
    const cross = await req(`/enrollments/${bEnrolment.id}/hide`, { method: 'POST', token: tokenA });
    check("student A cannot act on student B's enrolment", cross.status === 403 || cross.status === 404, `got ${cross.status}`);
  }
}
...
console.log('\n=== MALFORMED INPUT (no 5xx allowed) ===');
{
  const cases = [
    ...
    ['huge string', '/auth/login', { email: 'x'.repeat(20000) + '@t.io', password: 'y'.repeat(20000) }],
  ...
  const badJson = await fetch(`${API}/auth/login`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"email":' });
...
console.log('\n=== LATENCY BASELINE (30 samples each, warm) ===');
{
  const endpoints = [
    ['GET  /health', '/health', {}],
    ['POST /auth/login', '/auth/login', { method: 'POST', body: { email: A.user.email, password: PASS } }],
  ...
  for (const [label, path, opts] of endpoints) {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      times.push(r.ms); status = r.status;
    }
    times.sort((a, b) => a - b);
    const at = (p) => times[Math.min(times.length - 1, Math.floor(times.length * p))].toFixed(1);
    console.log(`   ${label.padEnd(28)} ${String(times.length).padStart(3)}  ${at(.5).padStart(6)}ms ${at(.95).padStart(6)}ms ${at(.99).padStart(6)}ms ${times[times.length-1].toFixed(1).padStart(6)}ms  ${status}`);
  }
}
