...
function makePrisma(
  academy: { id: string; kind: 'PERSONAL' | 'CENTER'; ownerUserId?: string } = {
    id: 'teacherT',
    kind: 'PERSONAL',
  },
) {
  ...
  const prisma: any = {
    ...
    payoutMethodSaved: {
      findMany: jest.fn((args: any) =>
        Promise.resolve([...methods.values()].filter((m) => m.academyId === args.where.academyId)),
      ),
      findFirst: jest.fn((args: any) =>
        Promise.resolve(
          [...methods.values()].find((m) =>
            Object.entries(args.where).every(([k, v]) => m[k] === v),
          ) ?? null,
        ),
      ),
      count: jest.fn((args: any) =>
        Promise.resolve(
          [...methods.values()].filter((m) => m.academyId === args.where.academyId).length,
        ),
      ),
      create: jest.fn(async ({ data }: any) => {
        const row = { id: `m${methods.size + 1}`, ...data };
    ...
    payoutRequest: {
      ...
      create: jest.fn(async ({ data }: any) => {
        const row = { id: `po${payouts.size + 1}`, status: 'REQUESTED', ...data };
        payouts.set(row.id, row);
        return row;
      }),
      findMany: jest.fn((args: any) =>
        Promise.resolve(
          [...payouts.values()].filter((p) =>
            args.where?.academyId ? p.academyId === args.where.academyId : true,
          ),
        ),
      ),
      findUnique: jest.fn(({ where, include }: any) => {
        ...
        return Promise.resolve({
          ...row,
          teacher: { userId: 'owner1' },
          academy: { id: row.academyId, kind: academy.kind, ownerUserId: 'owner1' },
        });
      }),
  ...
  const ledger: any = {
    orgBalance: jest.fn().mockResolvedValue(0),
...
describe('PayoutsService — organisation-scoped balance, not always the teacher', () => {
  ...
  it("a Center payout draws on the Center's own balance, not the requester's personal one", async () => {
    ...
    expect(ledger.orgBalance).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'centerA', kind: 'CENTER' }),
      expect.anything(),
    );
  });
  ...
  it("payout cannot exceed the organisation's available balance", async () => {
    ...
    await expect(svc.request('centerA', 60_000, methodId)).rejects.toMatchObject({
      response: { code: 'PAYOUT_EXCEEDS_BALANCE' },
    });
  });

  it('a payout method from a different organisation cannot be used (cross-scope 404, existence hidden)', async () => {
    const { svc } = makePrisma({ id: 'centerA', kind: 'CENTER' });
    await expect(svc.request('centerA', 60_000, 'method-from-center-b')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it('a payout already finalized cannot be processed again (no double settlement)', async () => {
    ...
    await expect(svc.process(payout.id, 'COMPLETED' as any, 'admin1')).rejects.toBeInstanceOf(
      BadRequestException,
    );
