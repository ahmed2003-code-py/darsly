...
function makeDeps() {
  ...
  const access: any = {
    assertGroupAccess: jest.fn(async (c: any, groupId: string) => {
      const group = await prisma.group.findFirst({ where: { id: groupId, academyId: c.academyId } });
      if (!group) throw new NotFoundException('Group not found');
      if (c.role !== 'OWNER') {
        const assigned = await prisma.groupAssignment.findFirst({ where: { groupId, userId: c.userId } });
...
describe('GroupsService', () => {
  ...
  describe('addMembers', () => {
    it('rejects a student who is not enrolled in this academy', async () => {
      ...
      const svc = new GroupsService(prisma, access, audit);
      await expect(svc.addMembers(ctx({ role: 'OWNER' }), 'g1', { studentIds: ['s1', 's2'] })).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('refuses a teacher not assigned to the group before even checking students', async () => {
      ...
      const svc = new GroupsService(prisma, access, audit);
      await expect(svc.addMembers(ctx(), 'g1', { studentIds: ['s1'] })).rejects.toBeInstanceOf(ForbiddenException);
  ...
  describe('assignStaff', () => {
    it('refuses a non-OWNER even with group.manage and even if assigned to the group', async () => {
      ...
      prisma.group.findFirst.mockResolvedValue({ id: 'g1', academyId: 'a1' });
      const svc = new GroupsService(prisma, access, audit);
      await expect(svc.assignStaff(ctx({ role: 'TEACHER' }), 'g1', { userId: 'u2', role: 'ASSISTANT' })).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('404s a cross-academy group id for assignment too (checked independently of OWNER-only rule)', async () => {
      ...
      prisma.group.findFirst.mockResolvedValue(null);
      const svc = new GroupsService(prisma, access, audit);
      await expect(svc.assignStaff(ctx({ role: 'OWNER' }), 'g1', { userId: 'u2', role: 'TEACHER' })).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('refuses assigning a user who is not active staff of this academy', async () => {
      ...
      const svc = new GroupsService(prisma, access, audit);
      await expect(svc.assignStaff(ctx({ role: 'OWNER' }), 'g1', { userId: 'stranger', role: 'TEACHER' })).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('OWNER assigning legitimate staff succeeds', async () => {
      ...
      prisma.groupAssignment.upsert.mockResolvedValue({ id: 'ga1', groupId: 'g1', userId: 'u2', role: 'TEACHER' });
      const svc = new GroupsService(prisma, access, audit);
      const result = await svc.assignStaff(ctx({ role: 'OWNER' }), 'g1', { userId: 'u2', role: 'TEACHER' });
      expect(result.userId).toBe('u2');
      expect(audit.log).toHaveBeenCalledWith(expect.objectContaining({ action: 'group.assignment.set', academyId: 'a1' }));
    });
  });
});
