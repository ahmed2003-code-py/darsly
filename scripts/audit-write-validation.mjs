...
try {
...

  {
    ...
    check(
      'non-numeric durationMin is rejected, not coerced',
      r3.status === 400,
      `HTTP ${r3.status}`,
    );
    ...
    check(
      'durationMin past the 720-minute ceiling is rejected',
      r5.status === 400,
      `HTTP ${r5.status}`,
    );
    ...
    const r8 = await create({
      ...base,
      tenantId: 'someone-elses-academy',
      isOwner: true,
      teacherId: 'not-me',
    });
    check(
      'unrecognized fields (tenantId/isOwner/teacherId) are rejected outright, not silently dropped',
      r8.status === 400,
      `HTTP ${r8.status}`,
    );
    ...
    const r10 = await create({
      ...base,
      title: weirdTitle,
      startsAt: new Date(Date.now() + 7200_000).toISOString(),
    });
    check(
      'a title with quotes/HTML/SQL punctuation is accepted as literal content',
      r10.status === 201 || r10.status === 200,
      `HTTP ${r10.status}`,
    );
    if (r10.body?.id) {
      ...
      check(
        '…and stored byte-for-byte, not mangled or executed',
        row?.title === weirdTitle,
        row?.title,
      );
    }
  }
  ...
  console.log('\n=== 2. ADD MEMBER — ROLE INJECTION ===');
  {
    const addMember = (body) =>
      api(`/academies/${academy.slug}/members`, { method: 'POST', token: tTok, body });
    const r1 = await addMember({ email: `${tag}-nope@test.invalid`, role: 'SUPER_ADMIN' });
    check(
      'role:"SUPER_ADMIN" is rejected (not in the DTO\'s allowed set)',
      r1.status === 400,
      `HTTP ${r1.status}`,
    );
    ...
    const r3 = await addMember({
      email: `${tag}-nope3@test.invalid`,
      role: 'TEACHER',
    ...
    check(
      'extra fields riding along with a valid role are rejected outright',
      r3.status === 400,
      `HTTP ${r3.status}`,
    );
  }
  ...
  {
    const patch = (body) =>
      api(`/academies/${academy.slug}/settings`, { method: 'PATCH', token: tTok, body });
    ...
    check(
      'maxConcurrentSessions past its Max(10) is rejected',
      r2.status === 400,
      `HTTP ${r2.status}`,
    );
    ...
    check(
      'unrecognized settings fields (id/ownerId/verified) are rejected outright',
      r3.status === 400,
      `HTTP ${r3.status}`,
    );
  }
  ...
  console.log('\n=== 4. PUBLIC TEACHER SEARCH — PAGINATION & OUTPUT SHAPE ===');
  {
    ...
    check(
      'a negative page is rejected, not silently clamped to page 1',
      r1.status === 400,
      `HTTP ${r1.status}`,
    );
    ...
    check(
      'an absurd page number does not 500 (past-Max rejected or empty page)',
      r2.status === 400 || (r2.status === 200 && Array.isArray(r2.body?.items ?? r2.body)),
      `HTTP ${r2.status}`,
    );
    ...
    check(
      'a non-numeric page is rejected, not parsed as NaN into a query',
      r4.status === 400,
      `HTTP ${r4.status}`,
    );
    ...
    if (Array.isArray(rows) && rows.length) {
      const leaked = rows.some(
        (t) => 'passwordHash' in (t ?? {}) || 'passwordHash' in (t?.user ?? {}),
      );
  ...
  console.log('\n=== 5. CONCURRENT BOOKING — DOUBLE-CLICK ===');
  {
    ...
    if (target) {
      await prisma.liveBooking.deleteMany({
        where: { sessionId: target.id, studentId: student.id },
      });
      ...
      check(
        'neither concurrent booking request 500s',
        a.status < 500 && b.status < 500,
        `${a.status}, ${b.status}`,
      );
      const count = await prisma.liveBooking.count({
        where: { sessionId: target.id, studentId: student.id },
      });
      ...
      const winners = [a, b].filter(
        (r) => r.status >= 200 && r.status < 300 && !r.body?.alreadyBooked,
      ).length;
      const echoes = [a, b].filter(
        (r) => r.status >= 200 && r.status < 300 && r.body?.alreadyBooked === true,
      ).length;
      check(
        'exactly one request created the booking and the other was told alreadyBooked:true',
        winners === 1 && echoes === 1,
        `winners=${winners} alreadyBooked=${echoes}`,
      );
      await prisma.liveBooking.deleteMany({
        where: { sessionId: target.id, studentId: student.id },
      });
