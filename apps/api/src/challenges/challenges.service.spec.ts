...
function makeCtx(opts: { challenge?: Partial<typeof CHALLENGE> } = {}) {
  ...
  const prisma: any = {
    ...
    challengeQuestion: {
      ...
      findFirst: jest.fn((args: any) =>
        Promise.resolve(QUESTIONS.find((q) => q.id === args.where.id) ?? null),
      ),
      ...
      findMany: jest.fn((args: any) =>
        Promise.resolve(
          [...attempts.values()].filter(
            (a) => a.studentId === args.where.studentId && a.challengeId === args.where.challengeId,
          ),
        ),
      ),
      findFirst: jest.fn((args: any) => {
        ...
        if (args.where.studentId && a.studentId !== args.where.studentId)
          return Promise.resolve(null);
        if (args.where.challengeId && a.challengeId !== args.where.challengeId)
          return Promise.resolve(null);
        return Promise.resolve(a);
      }),
      create: jest.fn((args: any) => {
        const row = {
          id: `att${attempts.size + 1}`,
          status: 'IN_PROGRESS',
          startedAt: new Date(),
          deadlineAt: null,
          score: 0,
    ...
    challengeAnswer: {
      count: jest.fn((args: any) =>
        Promise.resolve(
          [...answers.values()].filter((a) => a.attemptId === args.where.attemptId).length,
        ),
      ),
      findMany: jest.fn((args: any) => {
        ...
        if (isCorrectFilter !== undefined)
          rows = rows.filter((a) => a.isCorrect === isCorrectFilter);
      ...
      findUnique: jest.fn((args: any) =>
        Promise.resolve(
          answers.get(
            `${args.where.attemptId_questionId.attemptId}:${args.where.attemptId_questionId.questionId}`,
          ) ?? null,
        ),
      ),
      create: jest.fn((args: any) => {
  ...
  const progress: any = {
    touchActivity: jest
      .fn()
      .mockResolvedValue({ rolled: false, currentStreak: 1, freezeUsed: false }),
  };
...
describe('ChallengesService — attempt lifecycle & anti-cheat', () => {
  ...
  it('refuses to start once maxAttempts is used up', async () => {
    ...
    prisma.challengeAttempt.findMany.mockResolvedValueOnce([
      { id: 'old', status: 'COMPLETED', studentId: 's1', challengeId: 'ch1' },
    ]);
  ...
  it('a wrong answer scores 0 XP and does not advance beyond what it earned', async () => {
    const { svc } = makeCtx();
    const state = await svc.startAttempt('u1', 'ch1');
    const res = await svc.answer('u1', 'ch1', state.attemptId, {
      questionId: 'q1',
  ...
  it('resubmitting the same question is idempotent — returns the stored result, never rescored', async () => {
    ...
    const state = await svc.startAttempt('u1', 'ch1');
    const first = await svc.answer('u1', 'ch1', state.attemptId, {
      questionId: 'q1',
      selectedOptionIds: ['o1'],
    });
    const sizeAfterFirst = answers.size;
    const second = await svc.answer('u1', 'ch1', state.attemptId, {
      questionId: 'q1',
      selectedOptionIds: ['o2'],
    }); // even a different answer
  ...
  it('refuses an answer for a question that is not the current one (no skipping ahead)', async () => {
    ...
    const state = await svc.startAttempt('u1', 'ch1');
    await expect(
      svc.answer('u1', 'ch1', state.attemptId, { questionId: 'q2', selectedOptionIds: ['o2'] }),
  ...
  it('a perfect score fires CHALLENGE_PERFECT; a partial score does not', async () => {
    ...
    await perfect.svc.answer('u1', 'ch1', s1.attemptId, {
      questionId: 'q1',
      selectedOptionIds: ['o1'],
    });
    await perfect.svc.answer('u1', 'ch1', s1.attemptId, {
    ...
    expect(perfect.gamification.record.mock.calls.map((c: any[]) => c[0].type)).toContain(
      'CHALLENGE_PERFECT',
    );

    const partial = makeCtx();
    const s2 = await partial.svc.startAttempt('u1', 'ch1');
    await partial.svc.answer('u1', 'ch1', s2.attemptId, {
      questionId: 'q1',
    ...
    expect(partial.gamification.record.mock.calls.map((c: any[]) => c[0].type)).not.toContain(
      'CHALLENGE_PERFECT',
    );
  });
  ...
  it('retryMistakes builds a new attempt containing only the wrong questions', async () => {
    const { svc } = makeCtx();
    const state = await svc.startAttempt('u1', 'ch1');
    await svc.answer('u1', 'ch1', state.attemptId, {
      questionId: 'q1',
      selectedOptionIds: ['WRONG'],
    }); // wrong
  ...
  it('retryMistakes refuses a perfect attempt — nothing to retry', async () => {
    ...
    await svc.answer('u1', 'ch1', state.attemptId, { questionId: 'q2', selectedOptionIds: ['o2'] });
    await svc.complete('u1', 'ch1', state.attemptId);
    await expect(svc.retryMistakes('u1', 'ch1', state.attemptId)).rejects.toThrow(
      BadRequestException,
    );
  });
});
