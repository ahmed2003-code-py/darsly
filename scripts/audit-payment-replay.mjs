...
if (!KEY) {
  console.error('REFUSED: PAYMENT_LISTENER_KEY is not set — the ingest route needs it.');
  process.exit(2);
}
...
const ingest = (event) =>
  api('/payment-events', { method: 'POST', body: event, headers: { 'x-listener-key': KEY } });
...
async function newStudentWithPendingPayment(priceCents, senderNumber, payerName) {
  ...
  if (sub.status >= 300)
    throw new Error(`submit failed: ${sub.status} ${JSON.stringify(sub.body).slice(0, 180)}`);
...
const ledgerFor = async (paymentId) => {
  const entries = await prisma.ledgerEntry.findMany({
    where: { transaction: { description: { contains: paymentId } } },
  });
  return entries;
};
...
try {
  ...
  console.log('\n=== 1. THE SAME SMS DELIVERED TWICE ===');
  {
    ...
    const events = await prisma.paymentEvent.count({
      where: { dedupeKey: { contains: `${tag}-sms-1` } },
    });
    ...
    check(
      'the first delivery is accepted',
      first.status < 300,
      `HTTP ${first.status} ${first.body?.status ?? ''}`,
    );
    check(
      'the second is recognised as a duplicate',
  ...
  console.log('\n=== 2. THE SAME SMS DELIVERED SIMULTANEOUSLY ===');
  {
    ...
    const events = await prisma.paymentEvent.count({
      where: { dedupeKey: { contains: `${tag}-sms-2` } },
    });
    const enrolments = await prisma.enrollment.count({
      where: { studentId: s.student.id, status: 'ACTIVE' },
    });
    ...
    check(
      'none returned a server error',
      [a, b, c].every((r) => r.status < 500),
  ...
  console.log('\n=== 3. REPLAY WITH A FRESH EVENT ID (same transfer, same reference) ===');
  {
    ...
    check(
      'the replay did not double-book the ledger',
      entries.length <= 4,
      `${entries.length} entries`,
    );
    check(
      'the replay was not silently accepted as a second settlement',
      replay.body?.status !== 'MATCHED' || entries.length <= 4,
      `${replay.body?.status}`,
    );
  }
  ...
  console.log('\n=== 4. AN SMS FOR THE WRONG AMOUNT ===');
  {
    ...
    check(
      'a short transfer does not match the payment',
      r.body?.status !== 'MATCHED',
      `${r.body?.status}`,
    );
  ...
  console.log('\n=== 5. AN OUTGOING TRANSFER MUST NOT CREDIT ANYONE ===');
  {
    ...
    const r = await ingest({
      ...
      reference: sender,
      occurredAt: new Date().toISOString(),
      externalId: `${tag}-sms-5`,
      rawMessage: outgoing,
    });
    const pay = await settledCount(s.payment.id);
    check(
      'money leaving the account does not settle a payment',
      !pay.settledAt,
      `${r.body?.status}, status ${pay.status}`,
    );
  }
  ...
  console.log('\n=== 6. INGESTION AUTH AND VALIDATION ===');
  {
    ...
    const badKey = await api('/payment-events', {
      method: 'POST',
      body: good,
      headers: { 'x-listener-key': 'x'.repeat(KEY.length) },
    });
    check(
      'a wrong key of the same length is refused',
      badKey.status === 401,
      `HTTP ${badKey.status}`,
    );
    ...
    const malformed = [
      ...
      [
        'unexpected field',
        { provider: 'VODAFONE_CASH', amountCents: 1000, matchedPaymentId: 'anything' },
      ],
    ];
    for (const [label, body] of malformed) {
      const r = await ingest(body);
      check(
        `${label} is a clean 4xx, not a 500`,
  ...
  for (const id of made.students) {
    await prisma.paymentEvent
      .deleteMany({ where: { dedupeKey: { contains: tag } } })
      .catch(() => {});
