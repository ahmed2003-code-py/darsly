...
import {
  useActiveAcademyRate,
  usePlatformAttendance,
  usePlatformFinancial,
} from '../../lib/analytics';
    ...
    <section className="card mb-6 p-5">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="card">
            <p className="font-heading text-2xl font-extrabold tabular-nums">
              {financial.data?.paymentConversion.convertedPct ?? '—'}%
            </p>
            <p className="text-xs text-outline">{t('admin.platformAnalytics.conversion')}</p>
          </div>
          <div className="card">
            <p className="font-heading text-2xl font-extrabold tabular-nums">
              {financial.data?.paymentConversion.pending ?? 0}
            </p>
      ...
      {loading ? (
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          <div>
            ...
            <BarChart
              data={thinned(revenue.data ?? []).map((p) => ({
...
export default function AdminOverviewPage() {
  ...
  const money = [
    // The single filled accent moment on the page — the headline number.
    {
      ...
      icon: 'trending_up',
      card: 'bg-primary text-on-primary border-transparent',
      iconBg: 'bg-white/15',
    },
    {
      label: t('admin.commission'),
      value: egp(data.commissionCents),
  ...
  const counts = [
    {
      label: t('admin.academiesTotal'),
      value: data.totalAcademies,
      icon: 'apartment',
      to: '/admin/academies',
    },
    {
      label: t('admin.academiesActive'),
      value: data.activeAcademies,
      icon: 'domain_verification',
      to: '/admin/academies?status=ACTIVE',
    },
    { label: t('admin.students'), value: data.students, icon: 'school', to: undefined },
    {
      label: t('admin.teachers'),
      value: data.teachersApproved,
      icon: 'groups',
      to: '/admin/teachers',
    },
    {
      label: t('admin.pendingTeachers'),
      value: data.teachersPending,
      icon: 'pending_actions',
      to: '/admin/teachers?status=PENDING',
      highlight: data.teachersPending > 0,
    },
    ...
    {
      label: t('admin.totalEnrollments'),
    ...
    },
    {
      ...
      icon: 'payments',
      to: '/admin/payouts',
      highlight: data.pendingPayouts > 0,
    },
    {
      label: t('admin.studioTile'),
      value: t('admin.studioTileValue'),
      icon: 'rate_review',
      to: '/admin/academy-studio',
    },
  ];
    ...
    <div className="page">
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {counts.map((c) => {
          const inner = (
            <div
              className={`card card-hover flex items-center gap-4 ${c.highlight ? 'ring-2 ring-primary' : ''}`}
            >
          ...
          return c.to ? (
            <Link key={c.icon} to={c.to}>
              {inner}
            </Link>
          ) : (
            <div key={c.icon}>{inner}</div>
          );
        })}
      </div>
