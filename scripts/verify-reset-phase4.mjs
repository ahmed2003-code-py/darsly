...
async function main() {
  ...
  const student = await prisma.user.findFirst({ where: { role: 'STUDENT' }, select: { email: true, studentProfile: { select: { id: true } } } });
  ...
  const cA = (await api('/admin/centers', { token: tokAdmin, method: 'POST', body: { name: `Scope A ${tag}`, adminName: 'Center Admin', adminEmail: tA.owner.email } })).body;
  const cB = (await api('/admin/centers', { token: tokAdmin, method: 'POST', body: { name: `Scope B ${tag}`, adminName: 'Center Admin', adminEmail: tB.owner.email } })).body;
  check('setup: both Centers created', !!cA?.id && !!cB?.id, JSON.stringify(cA?.message ?? cA?.code ?? ''));
  ...
  check('setup: teacher A is OWNER of Center A and TEACHER in Center B', (await api(`/invitation-links/${link.token}/accept`, { token: tokA, method: 'POST' })).status < 300);

  // ── 1. Personal course: tenantId == academyId ──
  const pc = await api('/teacher/courses', { token: tokA, method: 'POST', body: { title: `Personal ${tag}`, priceCents: 1000, subjectId: subjA } });
  ...
  check('OWNER activates a subject', (await api(`/academies/${cA.slug}/subjects/${subjA}`, { token: tokA, method: 'PUT', body: { isActive: true }, headers: H(cA.id) })).status < 300);
  check('master Subject row untouched (no duplicate created)', (await prisma.subject.count({ where: { id: subjA } })) === 1);
  const cc = await api('/teacher/courses', { token: tokA, method: 'POST', body: { title: `CenterA ${tag}`, priceCents: 0, subjectId: subjA }, headers: H(cA.id) });
  ...
  await api(`/academies/${cB.slug}/subjects/${subjA}`, { token: tokB, method: 'PUT', body: { isActive: true }, headers: H(cB.id) });
  const subjB = (await prisma.teacherSubject.findFirst({ where: { tenantId: tB.id } })).subjectId;
  await api(`/academies/${cB.slug}/subjects/${subjB}`, { token: tokB, method: 'PUT', body: { isActive: true }, headers: H(cB.id) });
  const aInB = await api('/teacher/courses', { token: tokA, method: 'POST', body: { title: `A-in-B ${tag}`, priceCents: 0, subjectId: subjA }, headers: H(cB.id) });
  check('teacher A authors a course inside Center B', aInB.status === 201 && aInB.body.tenantId === tA.id && aInB.body.academyId === cB.id, String(aInB.status));
  cleanup.courseIds.push(aInB.body.id);
  const bInB = await api('/teacher/courses', { token: tokB, method: 'POST', body: { title: `B-in-B ${tag}`, priceCents: 0, subjectId: subjB }, headers: H(cB.id) });
  cleanup.courseIds.push(bInB.body.id);
  const aListB = (await api('/teacher/courses', { token: tokA, headers: H(cB.id) })).body;
  check('TEACHER member in Center B sees only their own course there', aListB.some((c) => c.id === aInB.body.id) && !aListB.some((c) => c.id === bInB.body.id));
  const bListB = (await api('/teacher/courses', { token: tokB, headers: H(cB.id) })).body;
  check('OWNER of Center B sees every course offered there', bListB.some((c) => c.id === aInB.body.id) && bListB.some((c) => c.id === bInB.body.id));
  check("teacher A cannot edit a colleague's course in Center B", (await api(`/teacher/courses/${bInB.body.id}`, { token: tokA, method: 'PATCH', body: { title: 'hijack' }, headers: H(cB.id) })).status === 404);
  check('personal course does not leak into any Center list', !aListB.some((c) => c.id === pc.body.id) && !bListB.some((c) => c.id === pc.body.id));

  // ── 4. Cross-Center isolation ──
  for (const [label, method, path, body] of [['read', 'GET', `/teacher/courses/${bInB.body.id}`, undefined], ['update', 'PATCH', `/teacher/courses/${bInB.body.id}`, { title: 'Hijack attempt' }], ['delete', 'DELETE', `/teacher/courses/${bInB.body.id}`, undefined]]) {
    check(`Center A context cannot ${label} a Center B course`, (await api(path, { token: tokA, method, body, headers: H(cA.id) })).status === 404);
  }
  ...
  check('teacher B cannot even select Center A as a workspace', (await api(`/academies/${cA.slug}/me`, { token: tokB, headers: H(cA.id) })).status === 404);

  // ── 5. Enrollment scope ──
  for (const id of [cc.body.id, aInB.body.id, pc.body.id]) await api(`/teacher/courses/${id}`, { token: tokA, method: 'PATCH', body: { status: 'PUBLISHED' }, headers: id === cc.body.id ? H(cA.id) : id === aInB.body.id ? H(cB.id) : undefined }).catch(() => {});
  ...
  for (const id of [cc.body.id, aInB.body.id]) {
    if ((await hasLesson(id)) === 0) {
      const unit = await prisma.courseUnit.create({ data: { courseId: id, title: 'u', sortOrder: 0 } });
  ...
  const eB = await api('/enrollments', { token: tokS, method: 'POST', body: { courseId: aInB.body.id } });
  check('student enrols in Center A and Center B courses', eA.status < 300 && eB.status < 300, `${eA.status}/${eB.status} ${JSON.stringify(eA.body?.code ?? '')}`);
  const rowA = await prisma.enrollment.findUnique({ where: { studentId_courseId: { studentId: student.studentProfile.id, courseId: cc.body.id } } });
  const rowB = await prisma.enrollment.findUnique({ where: { studentId_courseId: { studentId: student.studentProfile.id, courseId: aInB.body.id } } });
  check('Enrollment.academyId copied from the Course; tenantId stays the author', rowA?.academyId === cA.id && rowA?.tenantId === tA.id && rowB?.academyId === cB.id && rowB?.tenantId === tA.id);
  ...
  check('Center A staff list shows only Center A enrolments', listA.some((e) => e.id === rowA.id) && !listA.some((e) => e.id === rowB.id));
  check('Center B staff list shows only Center B enrolments', listB.some((e) => e.id === rowB.id) && !listB.some((e) => e.id === rowA.id));
  check('Center A cannot revoke a Center B enrolment', (await api(`/teacher/enrollments/${rowB.id}/revoke`, { token: tokA, method: 'PATCH', body: {}, headers: H(cA.id) })).status === 404);
  const roster = (await api('/teacher/roster', { token: tokA, headers: H(cA.id) })).body;
  const rosterIds = JSON.stringify(roster ?? '');
  check('Center A roster includes the student (organisation scope)', rosterIds.includes(student.studentProfile.id));
  const rosterB = JSON.stringify((await api('/teacher/roster', { token: tokB, headers: H(cB.id) })).body ?? '');
  check('Center B roster also includes the student — same learner, two organisations', rosterB.includes(student.studentProfile.id));
  const rosterPersonal = JSON.stringify((await api('/teacher/roster', { token: tokB })).body ?? '');
  check("teacher B's PERSONAL roster does not pick up Center enrolments", !rosterPersonal.includes(rowB.id));
  ...
  check('teacher payment queue is organisation-scoped (no personal payments inside Center A)', teacherQueueA.status === 404 || (Array.isArray(teacherQueueA.body) && teacherQueueA.body.every((p) => p.academyId === cA.id || p.academyId === undefined && p.tenantId === cA.id)), String(teacherQueueA.status));

  // ── 7. Subject isolation & admin counts ──
  check('teacher B (not a member of Center A) cannot toggle Center A subjects', (await api(`/academies/${cA.slug}/subjects/${subjB}`, { token: tokB, method: 'PUT', body: { isActive: true }, headers: H(cA.id) })).status === 404);
  check('student cannot toggle subjects', (await api(`/academies/${cA.slug}/subjects/${subjA}`, { token: tokS, method: 'PUT', body: { isActive: false }, headers: H(cA.id) })).status === 404);
  check('deactivating keeps the row and the master subject', (await api(`/academies/${cA.slug}/subjects/${subjA}`, { token: tokA, method: 'PUT', body: { isActive: false }, headers: H(cA.id) })).body?.isActive === false && (await prisma.subject.count({ where: { id: subjA } })) === 1);
  const detailA = (await api(`/admin/academies/${cA.id}`, { token: tokAdmin })).body;
  check('admin Center detail counts courses by organisation scope', detailA?.coursesCount === 1 && detailA?.enrollmentsCount === 1, `${detailA?.coursesCount}/${detailA?.enrollmentsCount}`);
  ...
  // ── 9. Regression ──
  check('teacher A still resolves their PERSONAL workspace with no header', (await api(`/academies/${tA.slug}/me`, { token: tokA })).body?.role === 'OWNER');
  const personalList = (await api('/teacher/courses', { token: tokA })).body;
  check('PERSONAL course list unchanged (only personal courses)', personalList.some((c) => c.id === pc.body.id) && !personalList.some((c) => c.id === cc.body.id));
}
