...
export function useCreateRoom() {
  ...
  return useMutation({
    mutationFn: async (dto: { name: string; location?: string; capacity?: number }) =>
      (await api.post('/teacher/rooms', dto)).data,
...
export function useSchedule(slug: string | undefined, from: string, to: string) {
  return useQuery<ScheduleSession[]>({
    ...
    queryFn: async () =>
      (await api.get(`/academies/${slug}/schedule`, { params: { from, to } })).data,
    enabled: !!slug,
  });
}
...
export function useCancelSession() {
  ...
  return {
    ...update,
    mutate: (sessionId: string) => update.mutate({ sessionId, status: 'CANCELLED' }),
  };
}
