...
export const TECHNICAL_DESIGN: DesignSpec = {
  ...
  motion: {
    ...
    scrollFx: ['sticky-headings', 'counters'],
  },
  decoration: {
    backdrop: 'grid-lines',
    accents: ['corner-brackets', 'rule-lines'],
    dividers: 'notch',
    imageTreatment: 'grid-overlay',
  },
};
...
export const WARM_DESIGN: DesignSpec = {
  ...
  decoration: {
    backdrop: 'aurora',
    accents: ['underline-swash', 'blob'],
    dividers: 'hairline',
    imageTreatment: 'mask-arch',
  },
};
...
export function buildComposition(opts: ComposeFixtureOptions): SiteDocument {
  ...
  const blocks: SiteBlock[] = [
    ...
    },
    {
      type: 'about',
      id: 'blk-about',
      section: at('about', 'about.side-by-side'),
    ...
    },
    {
      type: 'toolkit',
      id: 'blk-toolkit',
      section: at('toolkit', 'toolkit.skill-matrix'),
    ...
    },
    {
      type: 'credentials',
      id: 'blk-credentials',
      section: at('credentials', 'credentials.cards'),
    ...
    },
    {
      type: 'stats',
      id: 'blk-stats',
      section: at('stats', 'stats.big-numbers'),
    ...
    },
    {
      type: 'courses',
      id: 'blk-courses',
      section: at('courses', 'courses.bento'),
    ...
    },
    {
      type: 'reviews',
      id: 'blk-reviews',
      section: at('reviews', 'reviews.wall'),
    ...
    },
    {
      type: 'faq',
      id: 'blk-faq',
      section: at('faq', 'faq.two-column'),
      heading: lt('الأسئلة الشائعة', 'FAQ'),
      items: p.faq,
    },
    {
      type: 'contact',
      id: 'blk-contact',
      section: at('contact', 'contact.split-cta'),
  ...
  if (hasGallery) {
    blocks.splice(6, 0, {
      type: 'gallery',
      id: 'blk-gallery',
      section: at('gallery', 'gallery.mosaic'),
  ...
  if (rich) {
    blocks.splice(3, 0, {
      type: 'timeline',
      id: 'blk-timeline',
      section: at('timeline', 'timeline.rail'),
      heading: lt('المسيرة', 'Journey'),
      items: [
        {
          marker: lt('٢٠١٦', '2016'),
          title: lt('بداية التدريس', 'Started teaching'),
          body: lt('أول مجموعة من عشرة طلاب.', 'A first group of ten students.'),
        },
        {
          marker: lt('٢٠٢٠', '2020'),
          title: lt('التحول للأونلاين', 'Moved online'),
          body: lt(
            'منهج كامل مسجل ومتابعة أسبوعية.',
            'A full recorded syllabus with weekly follow-up.',
          ),
        },
        {
          marker: lt('٢٠٢٤', '2024'),
          title: lt('أكاديمية مستقلة', 'An academy of my own'),
          body: lt('فريق من ثلاثة مدرسين.', 'A team of three teachers.'),
        },
      ],
    });
    blocks.splice(5, 0, {
      type: 'process',
      id: 'blk-process',
      section: at('process', 'process.numbered'),
      heading: lt('الطريقة', 'How it works'),
      steps: [
        {
          title: lt('تحديد المستوى', 'Placement'),
          body: lt('اختبار قصير قبل البداية.', 'A short check before we begin.'),
        },
        {
          title: lt('الحصص', 'The lessons'),
          body: lt('حصتان أسبوعياً مع تسجيل.', 'Twice a week, with recordings.'),
        },
        {
          title: lt('المتابعة', 'Follow-up'),
          body: lt('واجب ومراجعة كل أسبوع.', 'Homework and a review every week.'),
        },
      ],
    });
    blocks.splice(8, 0, {
      type: 'quote',
      id: 'blk-quote',
      section: at('quote', 'quote.statement'),
      text: lt(
        'الفهم أولاً، والتمرين بعده يبقى سهل.',
        'Understanding first — the practice then takes care of itself.',
      ),
