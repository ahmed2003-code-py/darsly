...
export class AcademySubjectsController {
  ...
  async setAll(
    ...
    @Body() dto: SetSubjectOfferedDto,
  ) {
    const res = await this.subjects.setAllOffered(ctx.academyId, dto.isActive);
    await this.audit.log({
      actorUserId: user.sub,
      action: dto.isActive ? 'academy.subject.activateAll' : 'academy.subject.deactivateAll',
      entity: 'AcademySubject',
  ...
  async set(
    ...
    await this.audit.log({
      actorUserId: user.sub,
      action: dto.isActive ? 'academy.subject.activate' : 'academy.subject.deactivate',
      entity: 'AcademySubject',
      entityId: subjectId,
      academyId: ctx.academyId,
