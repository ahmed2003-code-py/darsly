...
| ⭐ `REDIS_URL` | **إجباري في الإنتاج** — بيوصّل الحدّ من معدّل الطلبات (login/forgot-password) وSocket.IO بين النسختين (replicas) الشغالين على Railway. من غيره الحدّ بيتضاعف (٢٠ محاولة/دقيقة بتبقى فعلياً ~٤٠ لأن كل نسخة بتعدّ لوحدها)، ورسالة الشات على نسخة ممكن متوصلش لمستخدم على النسخة التانية — الاتنين اتأكدوا فعلياً على الإنتاج. **الإقلاع بيفشل في الإنتاج لو ناقص** (`common/config.validation.ts`). شوف قسم 3c |
...
## 3c) Redis — توزيع الحدّ من المعدّل وSocket.IO بين النسختين

**ليه ده مهم:** الإنتاج بيشغّل **نسختين (replicas)** من الـAPI ورا الـload balancer بتاع Railway. الحدّ من معدّل الطلبات (throttler) الافتراضي في NestJS بيعدّ في الميموري — يعني كل نسخة بتعدّ لوحدها. اتأكد فعلياً على الإنتاج: ٢١ محاولة تسجيل دخول من IP واحد ما رجّعوش ولا 429 واحد، لأن كل نسخة شافت أقل من ٢٠. وSocket.IO بالمثل: رسالة شات مبعوتة من مستخدم واصل لنسخة A ما كانتش توصل لمستقبِل واصل لنسخة B — نفس المشكلة، اتأكدت.

الحل: Redis واحد مشترك بين النسختين. بيتستخدم في حاجتين بس:
1. **عدّاد الحدّ من المعدّل** (`RedisThrottlerStorageService`) — بدل الميموري المحلية.
2. **Socket.IO Redis adapter** (`@socket.io/redis-adapter`) — بينشر الـrooms والبثّ بين النسختين عن طريق pub/sub.

**خطوات Railway بالظبط:**

1. في مشروع Railway، **+ New** → **Database** → **Add Redis**. Railway بيدّيلك خدمة Redis جاهزة بمتغيّر `REDIS_URL` (أو `REDIS_PRIVATE_URL` لو حابب تستخدم الشبكة الداخلية).
2. على خدمة الـAPI، ضيف المتغيّر:
   ```
   REDIS_URL=${{Redis.REDIS_URL}}
   ```
   (لو استخدمت الشبكة الداخلية: `${{Redis.REDIS_PRIVATE_URL}}` — أرخص وأسرع لأنه مش عابر الإنترنت.)
3. Redeploy. الإقلاع بيفشل فوراً في الإنتاج لو `REDIS_URL` مش موجود — دي حماية متعمّدة (`common/config.validation.ts`)، مش باگ.
4. **تحقّق:** افتح لوج أي نسخة بعد الإقلاع، لازم تلاقي:
   ```
   [RedisService] Redis connected
   [RedisIoAdapter] Socket.IO Redis adapter connected — rooms now fan out across replicas.
   ```
   لو شفت `Redis initial connection failed` أو `continuing without it` — المتغيّر غلط أو الخدمة لسه بتقوم؛ التطبيق بيفضل شغّال (مش هيقع) لكن الحدّ من المعدّل والـSocket.IO بيرجعوا لسلوك النسخة الواحدة القديم لحد ما Redis يرجع.

**سياسة الفشل (Fail-open) — مقصودة، لا تتفاجأ منها:** لو Redis وقع مؤقتاً بعد ما كان شغّال، الحدّ من المعدّل بيوقف عن التطبيق (بيسمح لكل الطلبات) بدل ما يرفض كل حاجة، وSocket.IO بيرجع لبثّ داخل النسخة الواحدة بدل ما يقطع الاتصالات. الاختيار ده متعمّد: عطل Redis مؤقت أهون بكتير من قطع تسجيل الدخول لكل المستخدمين. argon2 على الباسورد وموديل الحساب نفسه هما خط الدفاع الأساسي ضد الـbrute force — الحدّ من المعدّل طبقة إضافية فوقهم، مش الوحيدة. التفاصيل في `apps/api/src/redis/`.

**محلياً:** مش لازم Redis عشان تشتغل — نسخة واحدة محلية، فالسلوك في الميموري صح أصلاً. لو حابب تجرّب المسار الموزّع فعلياً:
```bash
docker compose up -d redis   # redis:7-alpine على :6379
```
وحطّ `REDIS_URL=redis://localhost:6379` في `.env`.
...
- **التفريغ (Transcription) بيتفعّل بمتغيّر واحد: `DEEPGRAM_API_KEY`.** ده إعداد
  ...
  - **اللغة بتتبعت من السيرفر** (`TeacherProfile.language`، عربي افتراضياً) مع
    `nova-3` (Nova-2 مفيهوش عربي أصلاً). من غيرها Deepgram بيفترض إنجليزي وبيطلّع نص فاضي لحصة عربي.
...
- [ ] خدمة Redis مضافة و`REDIS_URL` مضبوط — **إجباري مع أكتر من نسخة** (قسم 3c). تأكّد من
      `Redis connected` و`Socket.IO Redis adapter connected` في لوج كل نسخة.
...
**تحقّق من Redis تحديداً (لو حطّيته لأول مرة):** افتح لوج كل نسخة وتأكد من `Redis connected`
و`Socket.IO Redis adapter connected`. جرّب ٢١ محاولة تسجيل دخول بباسورد غلط سريعة — المفروض
429 يظهر قريب من المحاولة ٢١، مش ٤٠+ (لو ظهر متأخّر كده، يبقى النسختين لسه بيعدّوا لوحدهم).
