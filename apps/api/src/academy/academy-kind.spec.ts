...
function renamePrisma(kind: 'PERSONAL' | 'CENTER') {
  return {
    ...
    teacherProfile: { findFirst: jest.fn().mockResolvedValue(null), updateMany: jest.fn().mockResolvedValue({ count: 1 }) },
  } as any;
}
...
describe('AcademyService.updateSettings — slug sync by kind', () => {
  it('a PERSONAL academy rename moves the teacher slug with it (existing behaviour)', async () => {
    ...
    expect(prisma.teacherProfile.updateMany).toHaveBeenCalledWith({ where: { userId: 'owner' }, data: { slug: 'new-name' } });
  });
  ...
  it('a CENTER cannot take a slug a teacher already holds', async () => {
    ...
    await expect(new AcademyService(prisma).updateSettings('c1', { slug: 'ahmed' } as any)).rejects.toBeInstanceOf(ConflictException);
  });
});
...
describe('AuthService slug derivation at teacher signup', () => {
  it('skips a candidate held by an Academy (Center) even when no teacher has it', async () => {
    ...
    expect(prisma.academy.findUnique).toHaveBeenCalledWith({ where: { slug: 'ahmed' }, select: { id: true } });
  });
});
...
describe('STAFF identity', () => {
  const row = (over: Record<string, unknown>) => ({
    id: 'm', userId: 'u', academyId: 'c', role: 'OWNER', status: 'ACTIVE', permissions: [], deletedAt: null,
  ...
  it('a STAFF OWNER gets a full OWNER context with no TeacherProfile at all', async () => {
    const prisma = { academyMembership: { findFirst: jest.fn().mockResolvedValue(row({})) } } as any;
  ...
  it('a suspended Center refuses its admin on the next request, without touching identities', async () => {
    const prisma = { academyMembership: { findFirst: jest.fn().mockResolvedValue(row({ academy: { status: 'SUSPENDED', deletedAt: null } })) } } as any;
  ...
  it('a STAFF admin has no context in a PERSONAL teacher academy they are not a member of', async () => {
    ...
    expect(await new AcademyService(prisma).buildContext('u', 'personal-of-teacher', 'STAFF')).toBeNull();
  });
  ...
  it('addMember still refuses STAFF for TEACHER/ASSISTANT roles (authorship needs a teacher)', async () => {
    const prisma = {
      user: { findUnique: jest.fn().mockResolvedValue({ id: 's', role: 'STAFF', isActive: true, teacherProfile: null }) },
      academyMembership: { findUnique: jest.fn(), upsert: jest.fn() },
    } as any;
    await expect(new AcademyService(prisma).addMember('c', { email: 's@x', role: 'TEACHER' }))
