...
try {
  ...
  const txn = await prisma.ledgerTransaction.create({
    data: { description: `${tag} seed earnings` },
  });
  await prisma.ledgerEntry.createMany({
    data: [
      ...
      {
        transactionId: txn.id,
        account: `teacher:${teacher.id}:balance`,
        direction: 'CREDIT',
        amountCents: BALANCE,
        tenantId: teacher.id,
      },
  ...
  const method = await prisma.payoutMethodSaved.create({
    data: {
      ...
      details: { walletPhone: '01000000000' },
      isDefault: true,
    },
  });
  ...
  if (lr.status >= 300)
    throw new Error(`teacher login failed: ${lr.status} ${JSON.stringify(lr.body).slice(0, 140)}`);
  ...
  console.log('=== TWO SIMULTANEOUS WITHDRAWALS OF THE FULL BALANCE ===');
  {
    const [a, b] = await Promise.all([
      api('/teacher/payouts', {
        method: 'POST',
        token,
        body: { amountCents: BALANCE, methodId: method.id },
      }),
      api('/teacher/payouts', {
        method: 'POST',
        token,
    ...
    check(
      'exactly one withdrawal was accepted',
      ok === 1,
      `${ok} accepted (${a.status}, ${b.status})`,
    );
    check(
      'neither request produced a server error',
      a.status < 500 && b.status < 500,
      `${a.status}, ${b.status}`,
    );
    check(
      'pending payouts do not exceed the balance',
      pending <= BALANCE,
      `${money(pending)} pending of ${money(BALANCE)}`,
    );
    if (ok !== 1)
      console.log(
        `      bodies: ${JSON.stringify(a.body).slice(0, 160)} | ${JSON.stringify(b.body).slice(0, 160)}`,
      );
  }
  ...
  console.log('\n=== THREE SIMULTANEOUS WITHDRAWALS OF THE REMAINING BALANCE ===');
  {
    ...
    const pending = await pendingTotal();
    check(
      'exactly one of three was accepted',
      ok === 1,
      `${ok} accepted (${a.status}, ${b.status}, ${c.status})`,
    );
    check(
      'no server errors',
  ...
  console.log('\n=== OVER-WITHDRAWAL AND MALFORMED AMOUNTS ===');
  {
    ...
    for (const [label, amountCents] of cases) {
      const r = await api('/teacher/payouts', {
        method: 'POST',
        token,
        body: { amountCents, methodId: method.id },
      });
  ...
  console.log('\n=== CROSS-TENANT PAYOUT ===');
  {
    ...
    if (!otherMethod) {
      ...
      const r = await api('/teacher/payouts', {
        method: 'POST',
        token,
        body: { amountCents: 1000, methodId: otherMethod.id },
      });
      check("another academy's payout method cannot be used", r.status >= 400, `HTTP ${r.status}`);
      const landed = await prisma.payoutRequest.count({
        where: { tenantId: otherMethod.tenantId, amountCents: 1000 },
      });
  ...
  if (teacher) {
    ...
    await prisma.ledgerEntry
      .deleteMany({ where: { account: { contains: `teacher:${teacher.id}:` } } })
      .catch(() => {});
