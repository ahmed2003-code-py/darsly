...
function makeDeps() {
  const prisma: any = {
    ...
    room: {
      findFirst: jest.fn().mockResolvedValue({ id: 'r1', academyId: 'a1', status: 'ACTIVE' }),
    },
    groupAssignment: {
      findFirst: jest.fn().mockResolvedValue({ id: 'ga' }),
      findMany: jest.fn().mockResolvedValue([]),
    },
  ...
  return {
    ...
    access,
    academy,
    svc: new SessionsService(prisma, access, audit, academy),
  };
}
...
describe('teaching mode + location — data on the occurrence, never a role', () => {
  it('PHYSICAL with a room: location defaults to CENTER', async () => {
    ...
    expect(prisma.groupSession.create.mock.calls[0][0].data).toMatchObject({
      mode: 'PHYSICAL',
      locationType: 'CENTER',
      joinUrl: null,
    });
  ...
  it('PHYSICAL without a room needs a location type', async () => {
    const { svc } = makeDeps();
    await expect(svc.create(ctx(), 'g1', { ...T })).rejects.toMatchObject({
  ...
  it("PHYSICAL at the student's place: no room, note kept, no join link stored", async () => {
    ...
    await svc.create(ctx(), 'g1', {
      ...
      locationNote: 'Nasr City',
      joinUrl: 'https://x.test',
    });
    expect(prisma.groupSession.create.mock.calls[0][0].data).toMatchObject({
      mode: 'PHYSICAL',
      locationType: 'STUDENT',
  ...
  it('a room with a non-CENTER location is contradictory', async () => {
    const { svc } = makeDeps();
    await expect(
      svc.create(ctx(), 'g1', { ...T, roomId: 'r1', locationType: 'TEACHER' }),
    ).rejects.toMatchObject({ response: { code: 'ROOM_NEEDS_CENTER_LOCATION' } });
  });
  it('ONLINE needs online access (join link)', async () => {
    const { svc } = makeDeps();
    await expect(svc.create(ctx(), 'g1', { ...T, mode: 'ONLINE' })).rejects.toMatchObject({
      response: { code: 'ONLINE_ACCESS_REQUIRED' },
    });
  });
  ...
  it('ONLINE with a physical location or room is refused', async () => {
    ...
    ).rejects.toMatchObject({ response: { code: 'ONLINE_HAS_LOCATION' } });
    await expect(
      svc.create(ctx(), 'g1', { ...T, mode: 'ONLINE', joinUrl: 'https://m.test', roomId: 'r1' }),
    ).rejects.toMatchObject({ response: { code: 'ONLINE_HAS_LOCATION' } });
  });
  ...
  it('HYBRID needs both a location and online access', async () => {
    const { svc, prisma } = makeDeps();
    await expect(
      svc.create(ctx(), 'g1', { ...T, mode: 'HYBRID', joinUrl: 'https://m.test' }),
    ).rejects.toMatchObject({ response: { code: 'LOCATION_REQUIRED' } });
    await expect(
      svc.create(ctx(), 'g1', { ...T, mode: 'HYBRID', roomId: 'r1' }),
    ).rejects.toMatchObject({ response: { code: 'ONLINE_ACCESS_REQUIRED' } });
    await svc.create(ctx(), 'g1', {
      ...T,
      mode: 'HYBRID',
      roomId: 'r1',
...
describe('teacher assignment — validated, never trusted', () => {
  ...
  it('an ineligible teacher (STAFF / student / unapproved / other Center) is refused before any conflict check', async () => {
    ...
    await expect(
      svc.create(ctx(), 'g1', { ...T, roomId: 'r1', teacherUserId: 'foreign' }),
  ...
  it('an OWNER who is a teacher may take their own group unassigned, but still passes the identity check', async () => {
    ...
    await svc.create(ctx({ userId: 'owner1' }), 'g1', {
      ...T,
      roomId: 'r1',
...
describe('conflicts across both tables', () => {
  it('a live stream of the same teacher in the window blocks a physical slot', async () => {
    ...
    prisma.liveSession.findMany.mockResolvedValue([
      { id: 'L', academyId: 'a1', startsAt: new Date('2026-11-01T10:30:00Z'), durationMin: 60 },
    ]);
    const err = await svc
      .create(ctx(), 'g1', { ...T, roomId: 'r1', teacherUserId: 'tA' })
      .catch((e) => e);
  ...
  it('a non-overlapping live stream does not conflict', async () => {
    const { svc, prisma } = makeDeps();
    prisma.liveSession.findMany.mockResolvedValue([
      { id: 'L', academyId: 'a1', startsAt: new Date('2026-11-01T08:00:00Z'), durationMin: 60 },
    ]);
    await expect(
      svc.create(ctx(), 'g1', { ...T, roomId: 'r1', teacherUserId: 'tA' }),
...
describe('scope', () => {
  it('a session of another academy 404s on update — the query is clamped to ctx.academyId', async () => {
    ...
    await expect(svc.update(ctx(), 'foreign', { startAt: T.startAt })).rejects.toBeInstanceOf(
      NotFoundException,
    );
    expect(prisma.groupSession.findFirst.mock.calls[0][0].where).toMatchObject({
      id: 'foreign',
      academyId: 'a1',
    });
  });
  it('Center schedule: every query is clamped to the academy and live streams are role-scoped', async () => {
    ...
    await svc.schedule(
      ctx({ role: 'TEACHER', userId: 'tA' }),
      new Date('2026-11-01'),
      new Date('2026-11-08'),
    );
    expect(prisma.groupSession.findMany.mock.calls[0][0].where).toMatchObject({
      academyId: 'a1',
      groupId: { in: ['g1'] },
    });
  ...
  it('My Schedule is keyed on the teacher across every academy — both tables, nobody else', async () => {
    ...
    prisma.liveSession.findMany.mockResolvedValue([
      {
        ...
        joinUrl: null,
        academyId: 'centerB',
        academy: { id: 'centerB', name: 'B', slug: 'b', kind: 'CENTER' },
      },
    ]);
    ...
    expect(prisma.liveSession.findMany.mock.calls[0][0].where).toMatchObject({
      teacherUserId: 'tA',
    });
    expect(out.map((e) => [e.kind, e.academyId])).toEqual([
      ['LIVE', 'centerB'],
      ['GROUP', 'a1'],
    ]);
  });
