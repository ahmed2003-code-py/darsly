...
export class SessionsService {
  ...
  private async precheckOverlap(
    ...
    if (conflict) {
      ...
      throw new ScheduleConflictError(
        code,
        `${code.replace('_', ' ').toLowerCase()} — another session overlaps this time`,
        ownId,
      );
    }
  }
  ...
  private async precheckLiveOverlap(
    teacherUserId: string,
    startAt: Date,
    endAt: Date,
    academyId: string,
    ...
    const clash = candidates.find(
      (c) => new Date(c.startsAt.getTime() + c.durationMin * 60_000) > startAt,
    );
    if (clash) {
      throw new ScheduleConflictError(
        'TEACHER_CONFLICT',
        'teacher conflict — a live session overlaps this time',
        clash.academyId === academyId ? clash.id : undefined,
      );
    }
  }
  ...
  private async resolveDelivery(
    ...
    const locationType =
      input.locationType !== undefined
        ? input.locationType
        : (existing?.locationType ?? (roomId ? 'CENTER' : null));
    const locationNote =
      input.locationNote !== undefined ? input.locationNote : (existing?.locationNote ?? null);
    const joinUrl = input.joinUrl !== undefined ? input.joinUrl : (existing?.joinUrl ?? null);

    if (mode === 'ONLINE') {
      if (locationType || roomId)
        throw new BadRequestException({
          message: 'An online session has no physical location',
          code: 'ONLINE_HAS_LOCATION',
        });
    } else {
      if (!locationType)
        throw new BadRequestException({
          message: 'A physical or hybrid session needs a location type',
          code: 'LOCATION_REQUIRED',
        });
      if (roomId && locationType !== 'CENTER')
        throw new BadRequestException({
    ...
    if (mode !== 'PHYSICAL' && !joinUrl) {
      ...
      const covered =
        live && new Date(live.startsAt.getTime() + live.durationMin * 60_000) > startAt;
      if (!covered)
        throw new BadRequestException({
          message: 'An online or hybrid session needs a join link or a live session for this group',
          code: 'ONLINE_ACCESS_REQUIRED',
        });
    }
    return {
      mode,
      locationType: mode === 'ONLINE' ? null : locationType,
      locationNote: mode === 'ONLINE' ? null : locationNote,
      joinUrl: mode === 'PHYSICAL' ? null : joinUrl,
    };
  }
  ...
  private async resolveAndValidate(
    ...
    const teacherUserId =
      input.teacherUserId !== undefined
        ? input.teacherUserId
        : (existing?.teacherUserId ?? undefined);
  ...
  async create(ctx: AcademyContext, groupId: string, dto: CreateSessionInput) {
    ...
    const { startAt, endAt, roomId, teacherUserId, mode, locationType, locationNote, joinUrl } =
      await this.resolveAndValidate(ctx, groupId, dto);
  ...
  async schedule(ctx: AcademyContext, from: Date, to: Date) {
    ...
    if (ctx.role === 'TEACHER' || ctx.role === 'ASSISTANT') {
      const assigned = await this.prisma.groupAssignment.findMany({
    ...
    return [...groupSessions.map((g) => ({ kind: 'GROUP' as const, ...g })), ...live].sort(
  ...
  liveAsEvent(l: {
    ...
    teacherUser?: { id: string; fullName: string } | null;
  }) {
    return {
      kind: 'LIVE' as const,
      id: l.id,
