...
const blankQuestion = (type: ChallengeQuestionRow['type']): ChallengeQuestionRow => {
  ...
  if (type === 'TRUE_FALSE') {
    return {
      ...
      type,
      options: [
        { id: 'true', text: i18n.t('assess.true') },
        { id: 'false', text: i18n.t('assess.false') },
      ],
...
export default function ChallengeBuilderPage() {
  ...
  const applyType = (ty: 'PRACTICE' | 'RANKED') => {
    setType(ty);
    if (ty === 'RANKED') {
      setScoring('SPEED_BASED');
      setQuestionTimeSec('20');
      setMaxAttempts('1');
      setLeaderboardEnabled(true);
      setAnswerReveal('AFTER_SUBMISSION');
  ...
  const myGrades: Grade[] = (grades ?? []).filter(
    ...
    <div className="page">
      ...
      <PageHeader
        ...
        action={
          challenge && challenge.status !== 'DRAFT' ? (
            <button className="btn-ghost" onClick={() => unpublish.mutate()}>
              {t('challenges.teacher.unpublish')}
            </button>
          ) : undefined
        }
      />
      ...
      <div className="mb-6 flex flex-wrap gap-1 rounded-full bg-surface-container-high p-1">
        {STEPS.map((s, i) => (
          ...
          >
            <span
              className={`grid h-5 w-5 place-items-center rounded-full text-[11px] ${step === s ? 'bg-primary text-on-primary' : 'bg-surface-container-high'}`}
            >
              {i + 1}
            </span>
        ...
        <div className="card max-w-4xl space-y-5">
          <label className="block">
            <span className="mb-1 block text-sm font-bold">
              {t('challenges.teacher.basic.title')}
            </span>
            <input
              className="input"
              dir="auto"
              value={title}
              placeholder={t('challenges.teacher.basic.titlePh')}
              onChange={(e) => setTitle(e.target.value)}
            />
          </label>
          <label className="block">
            <span className="mb-1 block text-sm font-bold">
              {t('challenges.teacher.basic.description')}
            </span>
            <textarea
              className="input min-h-[4rem]"
              dir="auto"
              value={description}
              placeholder={t('challenges.teacher.basic.descriptionPh')}
          ...
          <div>
            <span className="mb-2 block text-sm font-bold">
              {t('challenges.teacher.basic.type')}
            </span>
              ...
              {(['PRACTICE', 'RANKED'] as const).map((ty) => (
                ...
                >
                  <span className="block font-heading font-bold">
                    {t(`challenges.teacher.basic.type${ty === 'PRACTICE' ? 'Practice' : 'Ranked'}`)}
                  </span>
                  <span className="mt-1 block text-xs text-on-surface-variant">
                    {t(
                      `challenges.teacher.basic.type${ty === 'PRACTICE' ? 'Practice' : 'Ranked'}Hint`,
                    )}
                  </span>
                </button>
              ))}
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              <span className="mb-2 block text-sm font-bold">
                {t('challenges.teacher.basic.difficulty')}
              </span>
            ...
            <div>
              <span className="mb-2 block text-sm font-bold">
                {t('challenges.teacher.basic.coverIcon')}
              </span>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <label className="block">
              <span className="mb-1 block text-sm font-bold">
                {t('challenges.teacher.basic.course')}
              </span>
              <select
                className="input"
                value={courseId}
            ...
            <label className="block">
              <span className="mb-1 block text-sm font-bold">
                {t('challenges.teacher.basic.subject')}
              </span>
              <select
                className="input"
                value={subjectId}
            ...
            <label className="block">
              <span className="mb-1 block text-sm font-bold">
                {t('challenges.teacher.basic.grade')}
              </span>
              <select
                className="input"
                value={gradeId}
              ...
              >
                ...
                {myGrades.map((g) => (
                  <option key={g.id} value={g.id}>
                    {i18n.language === 'ar' ? g.nameAr : g.nameEn}
                  </option>
                ))}
              </select>
            </label>
            <label className="block">
              <span className="mb-1 block text-sm font-bold">
                {t('challenges.teacher.basic.topic')}
              </span>
              <input
                className="input"
                dir="auto"
      ...
      {step === 'settings' && (
        <div className="card max-w-4xl space-y-4">
          <div>
            <span className="mb-2 block text-sm font-bold">
              {t('challenges.teacher.settings.maxAttempts')}
            </span>
            <div className="flex gap-2">
          ...
          >
            <span
              className={`material-symbols-outlined text-[18px] transition-transform ${showAdvanced ? 'rotate-90' : ''}`}
            >
              chevron_left
            </span>
          ...
          {showAdvanced && (
            ...
            >
              <div className="grid gap-5 sm:grid-cols-2">
                <label className="block">
                  <span className="mb-1 block text-sm font-bold">
                    {t('challenges.teacher.settings.duration')}
                  </span>
                  <input
                    className="input"
                    inputMode="numeric"
                    placeholder={t('challenges.teacher.settings.durationNone')}
                    value={durationMin}
                    onChange={(e) => setDurationMin(e.target.value.replace(/\D/g, '').slice(0, 4))}
                  />
                  <span className="mt-1 block text-xs text-outline">
                    {t('challenges.teacher.settings.durationHint')}
                  </span>
                </label>
                <label className="block">
                  <span className="mb-1 block text-sm font-bold">
                    {t('challenges.teacher.settings.questionTime')}
                  </span>
                  <input
                    className="input"
                    inputMode="numeric"
                    placeholder={t('challenges.teacher.settings.questionTimeNone')}
                    value={questionTimeSec}
                    onChange={(e) =>
                      setQuestionTimeSec(e.target.value.replace(/\D/g, '').slice(0, 4))
                    }
                  />
                  <span className="mt-1 block text-xs text-outline">
                    {t('challenges.teacher.settings.questionTimeHint')}
                  </span>
                </label>
              </div>
              <div>
                <span className="mb-2 block text-sm font-bold">
                  {t('challenges.teacher.settings.scoring')}
                </span>
                <div className="grid gap-3 sm:grid-cols-2">
                  {(['STANDARD', 'SPEED_BASED'] as const).map((sc) => (
                    ...
                    >
                      <span className="block font-heading font-bold">
                        {t(
                          `challenges.teacher.settings.scoring${sc === 'STANDARD' ? 'Standard' : 'Speed'}`,
                        )}
                      </span>
                      <span className="mt-1 block text-xs text-on-surface-variant">
                        {t(
                          `challenges.teacher.settings.scoring${sc === 'STANDARD' ? 'Standard' : 'Speed'}Hint`,
                        )}
                      </span>
                    </button>
                  ))}
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <label className="block">
                  <span className="mb-1 block text-sm font-bold">
                    {t('challenges.teacher.settings.answerReveal')}
                  </span>
                  <select
                    className="input"
                    value={answerReveal}
                    onChange={(e) => setAnswerReveal(e.target.value)}
                  >
                    <option value="IMMEDIATE">
                      {t('challenges.teacher.settings.revealImmediate')}
                    </option>
                    <option value="AFTER_SUBMISSION">
                      {t('challenges.teacher.settings.revealAfterSubmission')}
                    </option>
                    <option value="AFTER_CLOSE">
                      {t('challenges.teacher.settings.revealAfterClose')}
                    </option>
                ...
                <label className="block">
                  <span className="mb-1 block text-sm font-bold">
                    {t('challenges.teacher.settings.randomize')}
                  </span>
                  <select
                    className="input"
                    value={randomize}
                  ...
                  >
                    ...
                    <option value="QUESTIONS">
                      {t('challenges.teacher.settings.randomizeQuestions')}
                    </option>
                    <option value="ANSWERS">
                      {t('challenges.teacher.settings.randomizeAnswers')}
                    </option>
        ...
        <div className="space-y-4">
            ...
            <div key={i} className="card">
              <div className="mb-3 flex items-center justify-between">
                <span className="flex items-center gap-2">
                  <span className="flex h-7 w-7 items-center justify-center rounded-full bg-primary-fixed text-sm font-bold text-on-primary-fixed">
                    {i + 1}
                  </span>
                  <select
                    ...
                    onChange={(e) =>
                      setQuestions((qs) =>
                        qs.map((qq, j) => (j === i ? blankQuestion(e.target.value as any) : qq)),
                      )
                    }
                  >
                ...
                <button
                  disabled={!isDraft}
                  className="text-error/70 hover:text-error disabled:opacity-30"
              ...
              <textarea
                className="input mb-3 min-h-[3rem]"
                dir="auto"
                disabled={!isDraft}
                placeholder={t('challenges.teacher.q.promptPh')}
                value={q.prompt}
                ...
                {q.options.map((o) => {
                  ...
                  return (
                    <label key={o.id} className="flex items-center gap-2">
                      ...
                      />
                      {q.type === 'TRUE_FALSE' ? (
                        <span className="flex-1 select-none rounded-lg bg-surface-container-low px-3 py-2 text-sm font-semibold">
                          {o.text}
                        </span>
                      ) : (
                        <input
                          ...
                          onChange={(e) =>
                            setQ(i, {
                              options: q.options.map((oo) =>
                                oo.id === o.id ? { ...oo, text: e.target.value } : oo,
                              ),
                            })
                          }
                        />
              ...
              <label className="mt-3 block">
                <span className="mb-1 block text-sm font-semibold text-on-surface-variant">
                  {t('challenges.teacher.q.explanation')}
                </span>
                <textarea
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                <label className="text-sm">
                  <span className="mb-1 block font-semibold text-on-surface-variant">
                    {t('challenges.teacher.q.points')}
                  </span>
                  <input
                    ...
                    onChange={(e) =>
                      setQ(i, { points: Number(e.target.value.replace(/\D/g, '')) || 0 })
                    }
                ...
                <label className="text-sm">
                  <span className="mb-1 block font-semibold text-on-surface-variant">
                    {t('challenges.teacher.q.timeLimit')}
                  </span>
                  <input
                    className="input py-1 text-sm"
                    inputMode="numeric"
                    ...
                    onChange={(e) => {
                      ...
                      setQ(i, { timeLimitSec: v ? Number(v) : null });
                    }}
                  />
                </label>
                <label className="text-sm">
                  <span className="mb-1 block font-semibold text-on-surface-variant">
                    {t('challenges.teacher.q.topic')}
                  </span>
                  <input
                    className="input py-1 text-sm"
                    dir="auto"
                    disabled={!isDraft}
                    value={q.topic ?? ''}
                    onChange={(e) => setQ(i, { topic: e.target.value || null })}
                  />
                </label>
                <label className="text-sm">
                  <span className="mb-1 block font-semibold text-on-surface-variant">
                    {t('challenges.teacher.q.difficulty')}
                  </span>
                  <select
                    className="input py-1 text-sm"
                    disabled={!isDraft}
          ...
          {isDraft && (
            <div className="flex flex-wrap gap-2">
              <button
                className="btn-ghost"
                onClick={() => setQuestions((qs) => [...qs, blankQuestion('MCQ')])}
              >
                {t('challenges.teacher.q.addMcq')}
              </button>
              <button
                className="btn-ghost"
                onClick={() => setQuestions((qs) => [...qs, blankQuestion('TRUE_FALSE')])}
              >
                {t('challenges.teacher.q.addTrueFalse')}
              </button>
            </div>
          )}
      ...
      {step === 'preview' && (
        <div className="max-w-md">
          <p className="mb-3 text-sm text-on-surface-variant">
            {t('challenges.teacher.preview.hint')}
          </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-on-surface-variant">
                  {t('challenges.card.questions', { count: questions.length })}
                </p>
              </div>
            </div>
            ...
            {questions[0] && (
              <div className="rounded-xl border border-outline-variant p-4">
                <p className="mb-3 text-xs font-bold uppercase text-outline">
                  {t('challenges.play.question', { n: 1, total: questions.length })}
                </p>
                <p className="mb-3 font-semibold" dir="auto">
                  {questions[0].prompt || '—'}
                </p>
                <div className="space-y-2">
          ...
          {challenge?.status === 'DRAFT' ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm text-on-surface-variant">
                {t('challenges.teacher.publishStep.ready')}
              </p>
            </div>
          )}
        </div>
...
function SaveBar({
  ...
  mutation: { isPending: boolean; isSuccess: boolean; error: unknown; mutate: () => void };
  disabled?: boolean;
}) {
  const { t } = useTranslation();
  return (
    <div className="border-t border-outline-variant/50 pt-4">
      ...
      {mutation.isSuccess && (
        <span className="ms-3 text-sm text-secondary">{t('common.saved')}</span>
      )}
