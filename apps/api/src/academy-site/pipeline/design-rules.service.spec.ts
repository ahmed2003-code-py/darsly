...
describe('DesignRulesService.validatePlan', () => {
  ...
  it('downgrades a display heading on a minimal, academic direction', () => {
    ...
    const { tokens, verdicts } = rules.validatePlan(
      plan({ designDNA: 'academic_precise' }),
      signals(),
    );
  ...
  it('stays quiet when there is imagery to work with', () => {
    expect(
      codes(rules.validatePlan(plan(), signals({ hasCover: false, galleryCount: 3 })).verdicts),
    ).not.toContain('no-media');
    expect(
      codes(rules.validatePlan(plan(), signals({ hasCover: true, galleryCount: 0 })).verdicts),
    ).not.toContain('no-media');
  });
...
describe('DesignRulesService.check — the assembled document', () => {
  ...
  it('stays quiet for a primary that carries its label', () => {
    expect(codes(rules.check(doc({ primary: '#0B3D2E', accent: '#E3B341' })))).not.toContain(
      'button-contrast-low',
    );
  });
  ...
  it('does not warn when primary and accent are literally the same colour', () => {
    ...
    expect(codes(rules.check(doc({ primary: '#3B82F6', accent: '#3B82F6' })))).not.toContain(
      'accent-indistinct',
    );
  });
