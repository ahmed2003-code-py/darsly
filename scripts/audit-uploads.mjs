...
try {
  ...
  const avatar = (mime, buf) =>
    api('/me/avatar', { method: 'POST', token: sToken, body: { dataUrl: dataUrl(mime, buf) } });
  {
    const good = await avatar('image/png', PNG);
    check(
      'a real PNG is accepted',
      good.status < 300,
      `HTTP ${good.status} ${JSON.stringify(good.body).slice(0, 90)}`,
    );
    ...
    console.log(
      `   note: PNG signature + appended markup -> HTTP ${poly.status} (signature is genuine; nosniff governs how it is served)`,
    );

    const oversized = await avatar('image/png', Buffer.concat([PNG, Buffer.alloc(400 * 1024)]));
    check(
      'an oversized avatar is refused',
      oversized.status >= 400 && oversized.status < 500,
  ...
  console.log('\n=== 2. COURSE THUMBNAIL (data URL or link) ===');
  {
    ...
    await prisma.course
      .update({ where: { id: course.id }, data: { thumbnailUrl: course.thumbnailUrl } })
      .catch(() => {});
  }
  ...
  {
    const anon = await api('/me/avatar', {
      method: 'POST',
      body: { dataUrl: dataUrl('image/png', PNG) },
    });
    ...
    const asStudent = await api('/academy/media', {
      method: 'POST',
      token: sToken,
      body: { kind: 'LOGO', dataUrl: dataUrl('image/png', PNG) },
    });
    check(
      'a student cannot upload academy media',
      asStudent.status >= 400,
    ...
    if (otherCourse) {
      const r = await api(`/teacher/courses/${otherCourse.id}`, {
        method: 'PATCH',
        token: tToken,
        body: { thumbnailUrl: dataUrl('image/png', PNG) },
      });
      ...
      check(
        "a teacher cannot set another academy's thumbnail",
        r.status >= 400,
        `HTTP ${r.status}`,
        true,
      );
  ...
  console.log('\n=== 4. STORAGE PATH CONTROL ===');
  {
    for (const [label, extra] of [
      ...
      const r = await api('/me/avatar', {
        method: 'POST',
        token: sToken,
        body: { dataUrl: dataUrl('image/png', PNG), ...extra },
      });
