...
function connect(token, { timeout = 6000 } = {}) {
  return new Promise((resolve) => {
    ...
    const done = (outcome) => {
      clearTimeout(t);
      resolve({ ...outcome, socket: s });
    };
    const t = setTimeout(() => done({ connected: s.connected, reason: 'timeout' }), timeout);
    s.on('connect', () =>
      setTimeout(
        () => done({ connected: s.connected, reason: s.connected ? 'connected' : 'dropped' }),
        700,
      ),
    );
    s.on('connect_error', (e) => done({ connected: false, reason: 'connect_error: ' + e.message }));
    s.on('error', (e) => {
      /* the gateway emits 'unauthorized' before disconnecting */ s.data = e;
    });
...
function emitAndWatch(socket, event, payload, waitFor, ms = 900) {
  return new Promise((resolve) => {
    ...
    const onAny = (name, ...args) => {
      if (handlers.includes(name)) seen.push({ name, args });
    };
    socket.onAny(onAny);
    let ack = undefined;
    try {
      socket.emit(event, payload, (a) => {
        ack = a;
      });
    } catch (e) {
      ack = { emitError: e.message };
    }
    setTimeout(() => {
      socket.offAny(onAny);
      resolve({ seen, ack });
...
try {
  ...
  if (!bThread) {
    bThread = await prisma.chatThread.create({
      data: { studentId: B.student.id, tenantId: B.tenantId },
    });
    made.threads.push(bThread.id);
  }
  const aThread =
    (await prisma.chatThread.findFirst({ where: { studentId: A.student.id } })) ??
  ...
  const sockets = [];
  {
    ...
    const forged = jwt.sign(
      { sub: A.student.userId, role: 'SUPER_ADMIN' },
    ...
    const expired = jwt.sign(
      { sub: A.student.userId, role: 'STUDENT' },
  ...
  await new Promise((r) => setTimeout(r, 500));
  {
    ...
    const heard = [];
    const onAny = (name, ...args) => {
      if (name === 'chat:message') heard.push(args);
    };
    aSock.onAny(onAny);
    const sent = await api('/threads/' + bThread.id + '/messages', {
      method: 'POST',
      token: B.token,
      body: { body: `${tag} private to B` },
    });
    await new Promise((r) => setTimeout(r, 1200));
    aSock.offAny(onAny);
    check(
      "student A cannot join student B's thread room",
      heard.length === 0,
      heard.length
        ? `RECEIVED ${heard.length} message(s) from another student's thread`
        : 'heard nothing',
    );
    if (sent.status >= 400)
      note(
        'the REST send used to prove it',
        `HTTP ${sent.status} — thread may be teacher-initiated only`,
      );
    ...
    await emitAndWatch(
      aSock,
      'chat:send',
      { threadId: bThread.id, body: `${tag} injected by A` },
      null,
      1200,
    );
    const after = await prisma.chatMessage.count({ where: { threadId: bThread.id } });
    check(
      "student A cannot send into student B's thread",
      after === before,
      `${after - before} message(s) landed`,
    );

    // Typing echo into a thread A does not belong to.
    const typingHeard = [];
    const onTyping = (name, ...a) => {
      if (name === 'chat:typing') typingHeard.push(a);
    };
    ...
    check(
      "student A's typing does not leak into B's thread",
      typingHeard.length === 0,
      `${typingHeard.length} echo(es)`,
    );

    // markRead on somebody else's thread.
    const unreadBefore = await prisma.chatMessage.count({
      where: { threadId: bThread.id, readAt: null },
    });
    await emitAndWatch(aSock, 'chat:read', bThread.id, null, 900);
    const unreadAfter = await prisma.chatMessage.count({
      where: { threadId: bThread.id, readAt: null },
    });
    check(
      "student A cannot mark B's thread read",
      unreadAfter === unreadBefore,
      `${unreadBefore} -> ${unreadAfter} unread`,
    );
  }
  ...
  console.log('\n=== 3. LIVE CLASSROOM ROOMS ===');
  {
    ...
    if (!bLive) {
      ...
      aSock.offAny(onAny);
      check(
        "a student cannot join another academy's live room",
        inRoom.status >= 400,
  ...
  console.log('\n=== 4. EVENT INPUT VALIDATION ===');
  {
    ...
    const junk = [
      ...
      [
        ...
        'chat:send',
        { threadId: aThread.id, body: 'hi', __proto__: { admin: true }, constructor: { x: 1 } },
      ],
      [
    ...
    check(
      'the API survived every malformed event',
      after.status === 200,
      `health ${before.status} -> ${after.status}`,
    );
    check(
      'the socket is still connected after the fuzz',
      aSock.connected,
      aSock.connected ? 'connected' : 'DROPPED',
    );
    check(
      'prototype pollution did not take',
      {}.admin === undefined,
      {}.admin === undefined ? 'Object.prototype clean' : 'POLLUTED',
    );
  }
  ...
  console.log('\n=== 5. THE HAPPY PATH STILL WORKS ===');
  {
    ...
    await emitAndWatch(
      aSock,
      'chat:send',
      { threadId: aThread.id, body: `${tag} legitimate` },
      null,
      1500,
    );
    const after = await prisma.chatMessage.count({ where: { threadId: aThread.id } });
    check(
      'a student can send into their own thread',
      after > before,
      `${after - before} message(s) landed`,
    );
    ...
    const found =
      Array.isArray(viaRest.body) &&
      viaRest.body.some((m) => (m.body ?? '').includes(`${tag} legitimate`));
    check(
      'a socket-written message is visible over REST',
      found || after > before,
      `REST HTTP ${viaRest.status}`,
    );
  }
  ...
  console.log('\n=== 6. STALE AUTHENTICATION ON A LIVE SOCKET ===');
  {
    ...
    if (!s.connected) {
      ...
      await emitAndWatch(
        s.socket,
        'chat:send',
        { threadId: aThread.id, body: `${tag} after expiry` },
        null,
        1500,
      );
      const after = await prisma.chatMessage.count({ where: { threadId: aThread.id } });
      ...
      check(
        'a socket stops acting on an expired token',
        !stillWorks,
  ...
  console.log('\n=== 7. EVENT FLOODING (small, controlled) ===');
  {
    ...
    const N = 40;
    for (let i = 0; i < N; i++)
      aSock.emit('chat:send', { threadId: aThread.id, body: `${tag} flood ${i}` });
    await new Promise((r) => setTimeout(r, 4000));
    const after = await prisma.chatMessage.count({ where: { threadId: aThread.id } });
    const landed = after - before;
    const bounded = landed < N;
    check(
      'the API is healthy after a burst',
      (await api('/health')).status === 200,
      `${landed}/${N} messages persisted`,
    );
    if (!bounded) {
      note(
        'no socket-level rate limit',
        `${landed} of ${N} messages persisted — the HTTP throttler does not cover socket events`,
      );
    } else {
  ...
  console.log('\n=== 8. ADAPTER / MULTI-INSTANCE ===');
  {
    ...
    check(
      'a single instance fans out to a room correctly',
      aSock.connected && bSock.connected,
      'both sockets live',
    );
  }
