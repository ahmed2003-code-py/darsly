...
export const CATALOG: CatalogSeed[] = [
  // ── Themes ────────────────────────────────────────────────────────────────
  theme('theme-paper', 'COMMON', 'ورق وحبر', 'Paper', 'مظهر درسلي الأساسي — هادي وواضح.',
    "Darsly's own look — calm and clear.", '#4a32c9', '#8d7bf0', 0, 10, { isStarter: true }),
  theme('theme-ocean', 'COMMON', 'محيط', 'Ocean', 'أزرق هادي بيريّح العين في المذاكرة الطويلة.',
    'A calm blue that holds up over a long session.', '#0f6f9c', '#3fb3e0', 120, 20),
  theme('theme-forest', 'COMMON', 'غابة', 'Forest', 'أخضر طبيعي، مريح ومركّز.',
    'Natural green — settled and focused.', '#1f7a4d', '#3fc286', 120, 30),
  theme('theme-sunset', 'RARE', 'غروب', 'Sunset', 'دفء برتقالي على أطراف بنفسجية.',
    'Warm orange over violet edges.', '#c2410c', '#fb923c', 250, 40),
  theme('theme-midnight', 'EPIC', 'منتصف الليل', 'Midnight', 'أناقة داكنة لمحبّي المذاكرة بالليل.',
    'Dark and quiet, for people who study late.', '#4338ca', '#a5b4fc', 500, 50, { requiredLevel: 3 }),
  theme('theme-cyber', 'EPIC', 'سايبر', 'Cyber', 'طاقة عالية وحواف حادّة.',
    'High energy, sharp edges.', '#0d9488', '#2dd4bf', 500, 60, { requiredLevel: 4 }),
  theme('theme-aurora', 'LEGENDARY', 'شفق', 'Aurora', 'ألوان متدرّجة هادية — أرقى مظهر في الاستوديو.',
    'The most considered look in the Studio.', '#7c3aed', '#c4b5fd', 900, 70, { requiredLevel: 6 }),
  theme('theme-scholar', 'LEGENDARY', 'العالِم', 'Scholar', 'بيتفتح لما تخلّص أول دورة كاملة.',
    'Unlocked by finishing your first course.', '#92400e', '#fbbf24', 0, 80,
    { requiredAchievement: 'first_course' }),

  // ── Accents ───────────────────────────────────────────────────────────────
  accent('accent-default', 'COMMON', 'اللون الأساسي', 'Default', '#4a32c9', 0, 10, { isStarter: true }),
  accent('accent-violet', 'COMMON', 'بنفسجي', 'Violet', '#7c3aed', 80, 20),
  accent('accent-teal', 'COMMON', 'فيروزي', 'Teal', '#0d9488', 80, 30),
  accent('accent-rose', 'RARE', 'وردي', 'Rose', '#be123c', 180, 40),
  accent('accent-amber', 'RARE', 'كهرماني', 'Amber', '#b45309', 180, 50),
  accent('accent-fire', 'EPIC', 'لهب', 'Fire', '#dc2626', 0, 60,
    { requiredAchievement: 'streak_7', descAr: 'بيتفتح مع سلسلة ٧ أيام.', descEn: 'Unlocked by a seven-day streak.' }),

  // ── Buttons ───────────────────────────────────────────────────────────────
  style('BUTTON_STYLE', 'button-classic', 'classic', 'COMMON', 'كلاسيك', 'Classic',
    'شكل الأزرار الأساسي.', 'The default button shape.', 0, 10, { isStarter: true }),
  style('BUTTON_STYLE', 'button-rounded', 'rounded', 'COMMON', 'مدوّر', 'Rounded',
    'حواف أنعم شوية.', 'A softer corner.', 60, 20),
  style('BUTTON_STYLE', 'button-pill', 'pill', 'RARE', 'كبسولة', 'Pill',
    'أزرار دايرية بالكامل.', 'Fully rounded buttons.', 150, 30),
  style('BUTTON_STYLE', 'button-sharp', 'sharp', 'RARE', 'حاد', 'Sharp',
    'حواف مستقيمة وحاسمة.', 'Straight, decisive edges.', 150, 40),
  style('BUTTON_STYLE', 'button-elevated', 'elevated', 'EPIC', 'بارز', 'Elevated',
    'ظل خفيف بيدّي إحساس بالعمق.', 'A quiet shadow that lifts them.', 300, 50, { requiredLevel: 3 }),

  // ── Cards ─────────────────────────────────────────────────────────────────
  style('CARD_STYLE', 'card-minimal', 'minimal', 'COMMON', 'بسيط', 'Minimal',
  ...
  style('CARD_STYLE', 'card-elevated', 'elevated', 'RARE', 'مرتفع', 'Elevated',
  ...
  style('CARD_STYLE', 'card-glass', 'glass', 'EPIC', 'زجاجي', 'Glass',
    'شفافية خفيفة — مستخدمة بحساب.', 'A little translucency, used sparingly.', 300, 50, { requiredLevel: 4 }),

  // ── Navigation ────────────────────────────────────────────────────────────
  style('NAV_STYLE', 'nav-classic', 'classic', 'COMMON', 'كلاسيك', 'Classic',
  ...
  style('NAV_STYLE', 'nav-floating', 'floating', 'EPIC', 'عائم', 'Floating',
    'القايمة بتبان كإنها طايرة فوق الصفحة.', 'The navigation floats above the page.', 300, 30, { requiredLevel: 3 }),

  // ── Avatars ───────────────────────────────────────────────────────────────
  style('AVATAR', 'avatar-initial', 'initial', 'COMMON', 'الحرف الأول', 'Initial',
  ...
  style('AVATAR', 'avatar-bloom', 'bloom', 'EPIC', 'تفتّح', 'Bloom',
  ...
  // ── Frames ────────────────────────────────────────────────────────────────
  style('FRAME', 'frame-none', 'none', 'COMMON', 'بدون إطار', 'No frame',
    'من غير إطار.', 'No frame at all.', 0, 10, { isStarter: true }),
  style('FRAME', 'frame-bronze', 'bronze', 'COMMON', 'برونزي', 'Bronze',
    'إطار برونزي.', 'A bronze ring.', 100, 20),
  style('FRAME', 'frame-silver', 'silver', 'RARE', 'فضّي', 'Silver',
    'إطار فضّي.', 'A silver ring.', 220, 30, { requiredLevel: 2 }),
  style('FRAME', 'frame-gold', 'gold', 'EPIC', 'ذهبي', 'Gold',
    'إطار ذهبي.', 'A gold ring.', 450, 40, { requiredLevel: 4 }),
  style('FRAME', 'frame-diamond', 'diamond', 'LEGENDARY', 'ألماس', 'Diamond',
    'إطار ألماسي.', 'A diamond ring.', 800, 50, { requiredLevel: 7 }),
  style('FRAME', 'frame-fire', 'fire', 'EPIC', 'لهب', 'Fire',
    'بيتفتح مع سلسلة ٣٠ يوم.', 'Unlocked by a thirty-day streak.', 0, 60,
    { requiredAchievement: 'streak_30' }),
  style('FRAME', 'frame-scholar', 'scholar', 'LEGENDARY', 'العالِم', 'Scholar',
