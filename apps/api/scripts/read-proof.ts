...
if (!file) { console.error('usage: npm run proof:read -- <image>'); process.exit(1); }
const mime = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp' }[extname(file).toLowerCase()];
if (!mime) { console.error('image must be .png, .jpg or .webp'); process.exit(1); }
...
new ProofReaderService(new AiClient(new AcademySiteConfig()))
  .read(dataUrl)
  .then((r) => { console.log(JSON.stringify(r, null, 2)); if (!r) process.exit(1); })
