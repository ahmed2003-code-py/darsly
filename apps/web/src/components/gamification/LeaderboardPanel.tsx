...
export function LeaderboardPanel({
  ...
  const { data, isLoading } = useQuery<LeaderboardBoard>({
    ...
    queryFn: async () =>
      (await api.get('/student/gamification/leaderboard', { params: { scope, scopeId, period } })).data,
    staleTime: 30_000,
  });
  ...
  return (
    <section className="card">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h2 className="font-heading text-lg font-extrabold">{t('gamification.leaderboard.title')}</h2>
          ...
          {(['WEEKLY', 'MONTHLY', 'ALLTIME'] as Period[]).map((p) => (
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-bold transition ${
                period === p ? 'bg-surface-container-lowest text-primary shadow-hairline' : 'text-on-surface-variant'
              }`}
            >
      ...
      {isLoading ? (
        ...
        <p className="py-8 text-center text-sm text-on-surface-variant">{t('gamification.leaderboard.empty')}</p>
      ) : (
      ...
      {period === 'WEEKLY' && !compact && (
        <p className="mt-3 text-center text-xs text-outline">{t('gamification.leaderboard.resetsWeekly')}</p>
      )}
    </section>
  );
}
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-xl border px-3 py-2 ${
        r.isMe ? 'border-primary bg-primary-fixed/50' : 'border-transparent bg-surface-container-low'
      }`}
    >
      <span
      ...
      <span className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-full bg-surface-container-high font-heading text-sm font-bold text-on-surface-variant">
        {r.avatarUrl ? <img src={r.avatarUrl} alt="" className="h-full w-full object-cover" /> : r.name.trim().charAt(0)}
      </span>
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm font-bold">
          {r.name}
          {r.isMe && <span className="ms-1.5 text-xs font-bold text-primary">· {t('gamification.leaderboard.you')}</span>}
        </span>
