...
import { useAdminThemeCatalog, useAdminThemePreference, useSetAdminTheme } from '../../lib/adminStudio';
import { Badge, BarChart, EmptyState, PageHeader, ProgressBar, Skeleton } from '../../components/ui';
  ...
  return (
    ...
    >
      <span className="flex w-1/5 flex-col gap-1.5 p-2" style={{ backgroundColor: rgb(tokens.sidebar) }}>
        ...
        <span className="h-1 w-5/6 rounded-full" style={{ backgroundColor: rgb(tokens.textMuted), opacity: 0.35 }} />
      </span>
      <span className="flex flex-1 flex-col">
        ...
        <span className="flex flex-1 flex-col gap-1.5 p-2">
          <span className="flex gap-1.5">
            <span className="flex flex-1 flex-col gap-1 rounded-md p-1.5" style={{ backgroundColor: rgb(tokens.surface), border: `1px solid ${rgb(tokens.border)}` }}>
              <span className="h-1.5 w-1/2 rounded-full" style={{ backgroundColor: rgb(tokens.text), opacity: 0.85 }} />
              <span className="h-1 w-1/3 rounded-full" style={{ backgroundColor: rgb(tokens.textMuted), opacity: 0.7 }} />
            </span>
            <span className="flex flex-1 flex-col gap-1 rounded-md p-1.5" style={{ backgroundColor: rgb(tokens.surfaceElevated), border: `1px solid ${rgb(tokens.border)}` }}>
              <span className="h-1.5 w-2/3 rounded-full" style={{ backgroundColor: rgb(tokens.text), opacity: 0.85 }} />
              <span className="h-1 w-1/4 rounded-full" style={{ backgroundColor: rgb(tokens.success) }} />
            </span>
          </span>
          <span className="mt-auto flex items-end gap-1">
            ...
            <span className="ms-auto flex items-end gap-0.5">
              {[5, 8, 4, 9, 6].map((h, i) => (
                <span key={i} className="w-1 rounded-t-sm" style={{ height: h * 1.5, backgroundColor: rgb([tokens.chart1, tokens.chart2, tokens.chart3][i % 3]) }} />
              ))}
            </span>
          </span>
        </span>
...
function LookCard({
  entry, active, previewing, busy, onPreview, onApply, t,
}: {
  ...
  return (
    <article className={`studio-card card flex flex-col p-4 transition ${active ? 'ring-2 ring-primary' : previewing ? 'ring-2 ring-outline' : ''}`}>
      <span className={entry.meta.rarity === 'LEGENDARY' ? 'studio-sheen block rounded-xl' : 'block'}>
        <LookPreview tokens={entry.tokens} />
      </span>
      <div className="mt-3 flex items-start gap-2">
        {entry.meta.logoUrl ? (
          <img src={entry.meta.logoUrl} alt="" className="h-8 w-8 shrink-0 rounded-lg object-cover" />
        ) : (
          <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg" style={{ backgroundColor: rgb(entry.tokens.primary) }}>
            <span className="material-symbols-outlined text-[18px]" style={{ color: rgb(entry.tokens.background) }}>
              {shelf === 'PRESET' ? 'palette' : shelf === 'COSMETIC' ? 'storefront' : shelf === 'CENTER' ? 'apartment' : 'school'}
            </span>
          </span>
        )}
        <div className="min-w-0 flex-1">
          <p className="truncate font-heading font-bold">{entry.name}</p>
          <p className="truncate text-xs text-on-surface-variant">{entry.subtitle ?? t(`adminControlStudio.shelf.${shelf}`)}</p>
        </div>
        {entry.meta.rarity ? (
          <span className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-bold ${RARITY_TONE[entry.meta.rarity] ?? RARITY_TONE.COMMON}`}>
      ...
      <div className="mt-2 flex items-center gap-2 text-xs text-on-surface-variant">
        <span className="h-3.5 w-3.5 rounded-full border border-outline-variant/40" style={{ backgroundColor: rgb(entry.tokens.primary) }} />
        <span className="h-3.5 w-3.5 rounded-full border border-outline-variant/40" style={{ backgroundColor: rgb(entry.tokens.accent) }} />
        <span className="h-3.5 w-3.5 rounded-full border border-outline-variant/40" style={{ backgroundColor: rgb(entry.tokens.background) }} />
        <span>{t(entry.mode === 'dark' ? 'adminControlStudio.theme.dark' : 'adminControlStudio.theme.light')}</span>
        {entry.meta.slug && <span className="ms-auto truncate" dir="ltr">/{shelf === 'CENTER' ? 'a' : 't'}/{entry.meta.slug}</span>}
      </div>
      {active && (
      ...
      <div className="mt-auto flex items-center gap-2 pt-3">
        <button type="button" onClick={onPreview} className="rounded-xl border border-outline-variant px-3 py-2 text-sm font-bold transition hover:border-outline">
          {previewing ? t('adminControlStudio.theme.stopPreview') : t('adminControlStudio.theme.preview')}
        </button>
        {!active && (
          <button type="button" disabled={busy} onClick={onApply} className="btn-primary ms-auto px-4 py-2 text-sm">
...
export default function AdminStudioPage() {
  ...
  const all = useMemo<AdminThemeEntry[]>(
    () => (catalog.data ? [...catalog.data.presets, ...catalog.data.academies, ...catalog.data.cosmetics] : []),
    [catalog.data],
  );
  ...
  const shown = useMemo(() => {
    ...
    return all.filter((e) => (shelf === 'ALL' || shelfOf(e) === shelf) && (!needle || `${e.name} ${e.subtitle ?? ''} ${e.meta.slug ?? ''}`.toLowerCase().includes(needle)));
  }, [all, shelf, q]);
  ...
  const apply = (entry: AdminThemeEntry) => {
    setPreviewId(null);
    setTheme.mutate(entry.id, { onSuccess: (data) => applyAdminTheme(data.theme ?? DEFAULT_ADMIN_THEME) });
  };
  const reset = () => {
    setPreviewId(null);
  ...
  return (
    <div className="page">
      ...
      <section className="card mb-6 flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
        <div className="sm:w-64"><LookPreview tokens={(previewing ?? saved).tokens} tall /></div>
        <div className="min-w-0 flex-1">
          <p className="text-xs font-bold uppercase tracking-wide text-on-surface-variant">
            {previewing ? t('adminControlStudio.theme.previewingLabel') : t('adminControlStudio.theme.currentLabel')}
          </p>
          <p className="mt-1 font-heading text-2xl font-extrabold">{(previewing ?? saved).name}</p>
          <p className="text-sm text-on-surface-variant">
            {(previewing ?? saved).subtitle ?? t(`adminControlStudio.shelf.${shelfOf(previewing ?? saved)}`)}
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {previewing && (
              <>
                <button type="button" className="btn-primary px-4 py-2 text-sm" disabled={setTheme.isPending} onClick={() => apply(previewing)}>
                  {t('adminControlStudio.theme.applyThis')}
                </button>
                <button type="button" className="btn-secondary px-4 py-2 text-sm" onClick={() => setPreviewId(null)}>
                  {t('adminControlStudio.theme.revert')}
                </button>
              </>
            )}
            {!previewing && activeId && (
              <button type="button" className="btn-secondary px-4 py-2 text-sm" disabled={setTheme.isPending} onClick={reset}>
            ...
            {SHELVES.map((s) => (
              <button
                ...
                className={`whitespace-nowrap rounded-xl border px-4 py-2 text-sm font-bold transition ${
                  shelf === s ? 'border-primary bg-primary text-on-primary' : 'border-outline-variant text-on-surface-variant hover:border-outline'
                }`}
              >
      ...
      {loading ? (
        <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="h-64 rounded-2xl" />)}</div>
      ) : shown.length === 0 ? (
        <div className="mb-8"><EmptyState icon="palette" title={t('adminControlStudio.theme.empty')} /></div>
      ) : (
