...
export default function ChallengePlayPage() {
  ...
  const answer = useMutation({
    mutationFn: async ({ questionId, optionIds }: { questionId: string; optionIds: string[] }) =>
      (
        await api.post(`/challenges/${id}/attempts/${attempt!.attemptId}/answers`, {
          questionId,
  ...
  const complete = useMutation({
    mutationFn: async () =>
      (await api.post(`/challenges/${id}/attempts/${attempt!.attemptId}/complete`))
  ...
  const retryMistakes = useMutation({
    mutationFn: async () =>
      (await api.post(`/challenges/${id}/attempts/${result!.attemptId}/retry-mistakes`))
        .data as AttemptState,
    onSuccess: (data) => {
      setAttempt(data);
      setIndex(0);
      setResult(null);
  ...
  useEffect(() => {
    if (!question?.timeLimitSec || feedback) {
      setQMsLeft(null);
      return;
    }
  ...
  useEffect(() => {
    if (qMsLeft === 0 && question && !feedback && !answer.isPending)
      answer.mutate({ questionId: question.id, optionIds: selected });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [qMsLeft]);
  ...
  const openLeaderboard = () => {
    ...
    if (!leaderboard)
      api.get(`/challenges/${id}/leaderboard`).then(({ data }) => setLeaderboard(data));
  };
      ...
      <div className="mx-auto max-w-xl px-6 py-8 sm:px-8">
        ...
        >
          ...
          <m.p
            ...
            animate={{ scale: 1, opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.1, ease: [0.2, 1.3, 0.4, 1] }}
            className="font-heading text-5xl font-extrabold text-primary"
          >
            {result.score}
          </m.p>
          <p className="mt-1 text-sm text-on-surface-variant">
            {t('challenges.result.correctOf', {
              correct: result.correctCount,
              total: result.correctCount + result.wrongCount,
            })}
          </p>
          ...
          <div className="mt-5 grid grid-cols-3 gap-3 border-t border-outline-variant/50 pt-4 text-center">
            {[
              ...
              {
                value: result.speedPct != null ? `${result.speedPct}%` : '—',
                label: t('challenges.result.speed'),
              },
              ...(result.rank != null
                ? [{ value: `#${result.rank}`, label: t('challenges.result.rank') }]
                : []),
            ].map((s, i) => (
              <m.div
                key={s.label}
            ...
            {!leaderboard ? (
              ...
              <div className="space-y-1.5">
                {leaderboard.map((r) => (
                  <div
                    key={r.studentId}
                    className={`flex items-center gap-3 rounded-xl px-3 py-2 text-sm ${r.isMe ? 'border border-primary bg-primary-fixed/50' : 'bg-surface-container-low'}`}
                  >
                    <span className="w-6 text-center font-bold text-outline">{r.rank}</span>
                    <span className="flex-1 truncate font-semibold">
                      {r.name}
                      {r.isMe && (
                        <span className="text-primary"> · {t('challenges.leaderboard.you')}</span>
                      )}
                    </span>
        ...
        <div className="card">
          ...
          {!result.mistakes.length ? (
            <p className="py-4 text-center text-sm text-on-surface-variant">
              {t('challenges.result.noMistakes')}
            </p>
          ) : (
            <>
              <div className="space-y-3">
                {result.mistakes.map((m) => (
                  <div
                    key={m.questionId}
                    className="rounded-xl border border-error/15 bg-error-container/20 p-3 text-sm"
                    dir="auto"
                  >
                    <p className="font-semibold">{m.prompt}</p>
                    {m.explanation && (
                      <p className="mt-1 text-xs text-on-surface-variant">{m.explanation}</p>
                    )}
                  </div>
                ))}
              </div>
              <button
                ...
                disabled={retryMistakes.isPending}
                onClick={() => retryMistakes.mutate()}
              >
        ...
        <Link to="/challenges" className="btn-ghost mt-6 block w-full text-center">
          {t('challenges.result.backToChallenges')}
        </Link>
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-md px-6 py-8 sm:px-8">
        ...
        <div className="card text-center">
          ...
          {detail.description && (
            <p className="mt-2 text-sm text-on-surface-variant">{detail.description}</p>
          )}
          ...
          <p className="mt-4 text-sm text-on-surface-variant">
            {t('challenges.detail.attempts', {
              used: detail.attemptsUsed,
              max:
                detail.maxAttempts === 0 ? t('challenges.detail.unlimitedMax') : detail.maxAttempts,
            })}
          </p>
          ...
          <button
            ...
            disabled={
              start.isPending || (detail.attemptsRemaining != null && detail.attemptsRemaining <= 0)
            }
  ...
  return (
    <div className="mx-auto max-w-xl px-6 py-8 sm:px-8">
      <div className="mb-4 flex items-center justify-between text-sm text-on-surface-variant">
        <span>
          {t('challenges.play.question', { n: index + 1, total: attempt.totalQuestions })}
        </span>
        {msLeft != null && (
          <span
            className={`flex items-center gap-1 font-bold tabular-nums ${msLeft <= 30_000 ? 'text-error' : ''}`}
          >
      ...
      <div className="mb-5 h-1.5 w-full overflow-hidden rounded-full bg-surface-container-high">
        <m.div
          ...
          animate={{ width: `${((index + (answered ? 1 : 0)) / attempt.totalQuestions) * 100}%` }}
          transition={{ duration: 0.35, ease: EASE }}
        />
      </div>
      ...
      {liveXp != null && !answered && (
        <div className="mb-4 text-center">
          ...
          <div
            className="mx-auto mt-1.5 h-1.5 w-full max-w-xs overflow-hidden rounded-full bg-surface-container-high"
            dir="ltr"
          >
            <div
              className={`h-full rounded-full ${qMsLeft != null && qMsLeft <= 3000 ? 'animate-pulse bg-error' : 'bg-student-secondary'}`}
              style={{
                width: `${(qMsLeft! / (question.timeLimitSec! * 1000)) * 100}%`,
      ...
      <AnimatePresence mode="wait">
        <m.div
          ...
          className="card"
        >
          <p className="mb-5 text-center font-heading text-xl font-extrabold" dir="auto">
            {question.prompt}
          </p>
          <div className="space-y-3">
                ...
                >
                  ...
                  {isCorrectOpt && (
                    <m.span
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      transition={{ duration: 0.25, ease: [0.2, 1.3, 0.4, 1] }}
            ...
            {answered && feedback && (
              ...
              >
                ...
                {feedback.xpAwarded > 0 && (
                  <span className="s-rise ms-2 inline-block text-student-gold-ink">
                    +{feedback.xpAwarded} {t('gamification.xp')}
                  </span>
                )}
                {feedback.explanation && (
                  <p className="mt-1 text-xs font-normal text-on-surface-variant" dir="auto">
                    {feedback.explanation}
                  </p>
                )}
                <button
                  type="button"
                  onClick={goNext}
                  className="mt-2 block w-full text-xs font-bold text-primary hover:underline"
                >
