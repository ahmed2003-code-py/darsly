...
export default function QuizBuilderPage() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const saved = (
        await api.put(`/teacher/lessons/${lessonId}/quiz/questions`, {
          questions: payload,
          aiGrading,
        })
      ).data;
  ...
  return (
    <div className="page">
      ...
      <PageHeader
        title={t('assess.builder.quizTitle')}
        subtitle={t('assess.builder.quizSubtitle')}
      />
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_20rem]">
        ...
        <div className="space-y-4">
          {questions.map((q, i) => (
            <div key={i} className="card">
              <div className="mb-3 flex items-center justify-between">
                <span className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setQuestions((qs) =>
                        qs.map((qq, j) => (j === i ? blankQ(e.target.value as Q['type']) : qq)),
                      )
                    }
                  >
              ...
              <textarea
                ...
                placeholder={t('assess.q.promptPlaceholder')}
                dir="auto"
                value={q.prompt}
              ...
              {q.type !== 'SHORT_ANSWER' ? (
                <div className="space-y-2">
                    ...
                    <label className="mb-1 flex items-center gap-2 text-sm text-on-surface-variant">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setQ(i, {
                            maxSelections: n,
                            correctOptionIds: q.correctOptionIds.slice(0, n),
                          });
                        }}
                      >
                        {Array.from({ length: q.options.length }, (_, n) => n + 1).map((n) => (
                          <option key={n} value={n}>
                            {n === 1
                              ? t('assess.q.howManyOne')
                              : t('assess.q.howManyN', { count: n })}
                          </option>
                        ))}
                      </select>
                    </label>
                  ...
                  <p className="text-xs text-outline">
                    {q.maxSelections > 1
                      ? t('assess.q.pickCorrectMulti')
                      : t('assess.q.pickCorrect')}
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
                  <p className="rounded-lg bg-surface-container-low px-3 py-2 text-xs text-outline">
                    {t('assess.q.manualNote')}
                  </p>
              ...
              <div className="mt-3 flex items-center gap-4">
                <label className="flex items-center gap-2 text-sm">
                  ...
                  <input
                    ...
                    value={q.points}
                    onChange={(e) =>
                      setQ(i, { points: e.target.value.replace(/\D/g, '').slice(0, 4) })
                    }
          ...
          <div className="flex flex-wrap gap-2">
            <button
              className="btn-ghost"
              onClick={() => setQuestions((qs) => [...qs, blankQ('MCQ')])}
            >
              + {t('assess.q.mcq')}
            </button>
            <button
              className="btn-ghost"
              onClick={() => setQuestions((qs) => [...qs, blankQ('TRUE_FALSE')])}
            >
              + {t('assess.q.trueFalse')}
            </button>
            <button
              className="btn-ghost"
              onClick={() => setQuestions((qs) => [...qs, blankQ('SHORT_ANSWER')])}
            >
              + {t('assess.q.short')}
            </button>
          </div>
        ...
        <aside className="space-y-4">
          <div className="card">
            ...
            <input
              ...
              value={passingScore}
              onChange={(e) =>
                setPassingScore(Math.min(100, Number(e.target.value.replace(/\D/g, '')) || 0))
              }
            />
