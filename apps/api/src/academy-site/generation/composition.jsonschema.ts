...
const hex = (description: string) => ({
  type: 'string',
  description: `${description} Six-digit hex, #RRGGBB.`,
});
...
const designSchema = object({
  ...
  typography: object({
    headingFamily: enumOf(
      HEADING_FAMILIES,
      'condensed and display are loud; serif is scholarly; mono is technical.',
    ),
    bodyFamily: enumOf(BODY_FAMILIES),
    scale: enumOf(
      TYPE_SCALES,
      'How far headlines dominate. monumental is a real design choice, not a default.',
    ),
  ...
  geometry: object({
    radius: {
      ...
      minimum: 0,
      maximum: 32,
      description: '0–4 architectural, 10–16 contemporary, 24–32 friendly.',
    },
    radiusStyle: enumOf(RADIUS_STYLES),
    border: enumOf(BORDER_WEIGHTS),
    shadow: enumOf(
      SHADOW_DEPTHS,
      'brutal is a hard offset with no blur; it needs a border and a small radius.',
    ),
    grain: {
      type: 'boolean',
      description: 'Film grain. Expensive on a dark page; it is removed from light ones.',
    },
  }),
  rhythm: object({
    density: enumOf(DENSITIES),
    sectionRhythm: enumOf(
      SECTION_RHYTHMS,
      'even, alternating bands, or opening up as the page scrolls.',
    ),
  ...
  motion: object({
    intensity: enumOf(
      MOTION_INTENSITIES,
      'How far the page moves. All three animate; they differ in amplitude.',
    ),
  ...
  decoration: object({
    backdrop: enumOf(
      BACKDROPS,
      'The atmosphere behind the whole page. The biggest single lever on how it feels.',
    ),
    accents: {
      type: 'array',
      maxItems: 3,
      items: enumOf(ACCENT_MARKS),
      description: 'Recurring marks that give the page a signature. At most three.',
    },
...
const sectionSchema = object({
  ...
  pattern: {
    type: 'string',
    description: 'A pattern id from the catalogue in the brief, e.g. "hero.split-portrait".',
  },
  emphasis: enumOf(SECTION_EMPHASIS, 'quiet shrinks the section, feature enlarges it.'),
  width: enumOf(CONTAINER_WIDTHS),
  surface: enumOf(
    SECTION_SURFACES,
    'The band this section sits on. Vary it — a page of identical bands reads as one long scroll.',
  ),
...
export const compositionJsonSchema: Record<string, unknown> = object({
  ...
  sections: {
    ...
    items: sectionSchema,
    description:
      'The page, in order — 8 to 12 sections. The first must be the hero, the last contact. about, courses, reviews and faq belong on essentially every page.',
  },
  content: object({
    statCount: {
      type: 'integer',
      minimum: 0,
      maximum: 6,
      description: 'Figures to write. 0 if the facts contain no real numbers.',
    },
  ...
  rationale: {
    type: 'string',
    description: 'One sentence on why this design suits this teacher. Under 300 characters.',
  },
});
