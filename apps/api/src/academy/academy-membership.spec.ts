...
const liveMembership = (over: Record<string, unknown> = {}) => ({
  id: 'm1', userId: 'u1', academyId: 'a1', role: 'TEACHER', status: 'ACTIVE', permissions: [], deletedAt: null,
...
describe('AcademyService.buildContext — membership is necessary, not sufficient', () => {
  ...
  it('queries only ACTIVE, non-soft-deleted rows (findFirst, never findUnique)', async () => {
    ...
    expect(where).toMatchObject({ userId: 'u1', academyId: 'a1', status: 'ACTIVE', deletedAt: null });
  });

  it.each([
    ['inactive user', liveMembership({ user: { isActive: false, role: 'TEACHER', teacherProfile: { status: 'APPROVED' } } })],
    ...
    ['soft-deleted academy', liveMembership({ academy: { status: 'ACTIVE', deletedAt: new Date() } })],
    ['suspended teacher identity', liveMembership({ user: { isActive: true, role: 'TEACHER', teacherProfile: { status: 'SUSPENDED' } } })],
    ['teacher identity with no profile', liveMembership({ user: { isActive: true, role: 'TEACHER', teacherProfile: null } })],
  ...
  it('a PENDING academy (teacher awaiting approval, mirrored) is not itself a blocker', async () => {
    const prisma = makePrisma();
    prisma.academyMembership.findFirst.mockResolvedValue(liveMembership({ academy: { status: 'PENDING', deletedAt: null } }));
  ...
  it('a STUDENT-role membership yields a context with no capabilities', async () => {
    const prisma = makePrisma();
    prisma.academyMembership.findFirst.mockResolvedValue(liveMembership({
      role: 'STUDENT', permissions: ['member.manage'], user: { isActive: true, role: 'STUDENT', teacherProfile: null },
    }));
...
describe('AcademyService.addMember — identity eligibility', () => {
  ...
  it('refuses a STUDENT identity for any staff role', async () => {
    ...
    await expect(new AcademyService(prisma).addMember('a1', { email: 'x@y.z', role: 'TEACHER' }))
  ...
  it('refuses an unapproved teacher', async () => {
    const prisma = setup({ role: 'TEACHER', isActive: true, teacherProfile: { status: 'PENDING' } });
    await expect(new AcademyService(prisma).addMember('a1', { email: 'x@y.z', role: 'ASSISTANT' }))
      .rejects.toMatchObject({ response: { code: 'TEACHER_NOT_APPROVED' } });
  });

  it('refuses a disabled account', async () => {
    const prisma = setup({ role: 'TEACHER', isActive: false, teacherProfile: { status: 'APPROVED' } });
    await expect(new AcademyService(prisma).addMember('a1', { email: 'x@y.z', role: 'TEACHER' }))
  ...
  it('accepts an approved teacher and creates the row INVITED, never ACTIVE', async () => {
    const prisma = setup({ role: 'TEACHER', isActive: true, teacherProfile: { status: 'APPROVED' } });
    await new AcademyService(prisma).addMember('a1', { email: 'x@y.z', role: 'TEACHER' });
    expect(prisma.academyMembership.upsert.mock.calls[0][0].create).toMatchObject({ status: 'INVITED', role: 'TEACHER' });
  });
});

describe('AcademyService — membership lifecycle', () => {
  const manageable = (status: string) => ({ id: 'm1', userId: 'u1', academyId: 'a1', role: 'TEACHER', status });
  const setup = (row: Record<string, unknown>) => {
    const prisma = makePrisma();
    prisma.academyMembership.findFirst.mockResolvedValue(row);
    prisma.academy.findUnique.mockResolvedValue({ ownerUserId: 'owner' });
    prisma.academyMembership.update.mockResolvedValue({ id: 'm1', role: 'TEACHER', status: 'ACTIVE', userId: 'u1' });
    return prisma;
  };

  it.each(['LEFT', 'INVITED'])('updateMember cannot resurrect a %s row to ACTIVE', async (status) => {
    const prisma = setup(manageable(status));
    await expect(new AcademyService(prisma).updateMember('a1', 'm1', { status: 'ACTIVE' }))
  ...
  it('updateMember may un-suspend (SUSPENDED → ACTIVE)', async () => {
    ...
    await expect(new AcademyService(prisma).updateMember('a1', 'm1', { status: 'ACTIVE' })).resolves.toMatchObject({ status: 'ACTIVE' });
  });

  it('suspending a member revokes their group scope', async () => {
    const prisma = setup(manageable('ACTIVE'));
    prisma.academyMembership.update.mockResolvedValue({ id: 'm1', role: 'TEACHER', status: 'SUSPENDED', userId: 'u1' });
    await new AcademyService(prisma).updateMember('a1', 'm1', { status: 'SUSPENDED' });
    expect(prisma.groupAssignment.deleteMany).toHaveBeenCalledWith({ where: { academyId: 'a1', userId: 'u1' } });
  ...
  it('removing a member sets LEFT and cleans assignments + future sessions in that academy only', async () => {
    ...
    expect(prisma.academyMembership.update).toHaveBeenCalledWith({ where: { id: 'm1' }, data: { status: 'LEFT' } });
    expect(prisma.groupAssignment.deleteMany).toHaveBeenCalledWith({ where: { academyId: 'a1', userId: 'u1' } });
  ...
  it('the owner can never be removed or downgraded', async () => {
    ...
    await expect(new AcademyService(prisma).updateMember('a1', 'm1', { role: 'ASSISTANT' })).rejects.toBeDefined();
