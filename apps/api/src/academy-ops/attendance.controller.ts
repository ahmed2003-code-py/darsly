...
function assertDate(date?: string): string {
  if (!date || !DATE_RE.test(date))
    throw new BadRequestException({ message: 'date must be YYYY-MM-DD', code: 'INVALID_DATE' });
  return date;
}
...
export class AttendanceController {
  ...
  @ApiOperation({
    summary: '[academy] Attendance for a group on one date — full roster, marked or not',
  })
  sessionFor(
