...
// ── Payment scope ──
report(
  'Payment whose academyId is neither its own tenantId nor a real Academy',
...
report(
  'CENTER payment (organisation is a Center) with no revenue split configured at settlement time yet marked PAID',
  await prisma.$queryRaw`
...
// ── Cash lifecycle ──
report(
  'CASH payment with no cashOrigin (should be set by every cash-creating path)',
  await prisma.$queryRaw`
  SELECT id FROM "Payment" WHERE method = 'CASH' AND "cashOrigin" IS NULL AND "deletedAt" IS NULL`,
);
report(
  'CASH payment PAID with no confirmer recorded (verifiedById)',
  await prisma.$queryRaw`
  SELECT id FROM "Payment" WHERE method = 'CASH' AND status = 'PAID' AND "verifiedById" IS NULL`,
);
report(
  'CASH payment PAID but not settled (cash confirmation must settle in the same step)',
  await prisma.$queryRaw`
  SELECT id FROM "Payment" WHERE method = 'CASH' AND status = 'PAID' AND "settledAt" IS NULL`,
);
report(
  'CASH payment with cashReceiver CENTER on a PERSONAL organisation (invalid combination)',
  await prisma.$queryRaw`
...
report(
  'TEACHER_RECORDED or CENTER_RECORDED cash with no recordedByUserId',
  await prisma.$queryRaw`
...
report(
  'PAID + settled Payment with no LedgerTransaction (money moved with nothing booked)',
  await prisma.$queryRaw`
...
report(
  'LedgerEntry on a teacher:/academy: account with no academyId (Phase 7 backfill gap)',
  await prisma.$queryRaw`
...
report(
  'Ledger credit on a Center/teacher balance whose academyId disagrees with the account it names',
  await prisma.$queryRaw`
...
report(
  'PayoutRequest whose academyId is not a real Academy',
  await prisma.$queryRaw`
...
report(
  'PayoutMethodSaved on a CENTER with a non-null tenantId (should be organisation-owned, not author-owned)',
  await prisma.$queryRaw`
...
report(
  'COMPLETED PayoutRequest with no matching ledger debit on its account',
  await prisma.$queryRaw`
...
report(
  'Payout-eligible :balance account gone negative (should only ever be credited by sales, debited only by a matching payout)',
  await prisma.$queryRaw`
...
if (cashLiabilities.length) {
  ...
  for (const r of cashLiabilities.slice(0, 25))
    console.log('   ' + JSON.stringify(r, (_, v) => (typeof v === 'bigint' ? Number(v) : v)));
}
report(
  ':cash-liability account sitting net-positive (would mean more was remitted than was ever collected)',
  await prisma.$queryRaw`
  SELECT account, SUM(CASE WHEN direction = 'CREDIT' THEN "amountCents" ELSE -"amountCents" END) AS balance
  FROM "LedgerEntry" WHERE "deletedAt" IS NULL AND account LIKE '%:cash-liability'
  GROUP BY account HAVING SUM(CASE WHEN direction = 'CREDIT' THEN "amountCents" ELSE -"amountCents" END) > 0`,
