...
const svc = (prisma: any) => new CoursesService(prisma, { applyToMany: async (i: unknown[]) => i } as any, { hiddenTeacherIds: async () => [] } as any, none, none, none, none, none, none);

const teacherInCenter: CourseScope = { academyId: 'centerA', authorTenantId: 'teacherT', manageAll: false };
const ownerOfCenter: CourseScope = { academyId: 'centerA', authorTenantId: 'teacherX', manageAll: true };
const staffOwner: CourseScope = { academyId: 'centerA', authorTenantId: undefined, manageAll: true };
const personal: CourseScope = { academyId: 'teacherT', authorTenantId: 'teacherT', manageAll: true };
...
describe('CoursesService.create — authorship vs organisation', () => {
  ...
  it('forged tenantId / academyId in the body are overridden by the scope', async () => {
    const prisma = makePrisma('CENTER');
    prisma.academySubject.findUnique.mockResolvedValue({ isActive: true });
    const c = await svc(prisma).create(teacherInCenter, { title: 'x', priceCents: 0, tenantId: 'victim', academyId: 'centerB' } as any);
    expect(c.tenantId).toBe('teacherT');
    expect(c.academyId).toBe('centerA');
  });
  ...
  it('a STAFF owner (no TeacherProfile) cannot author — 403, nothing written', async () => {
    const prisma = makePrisma('CENTER');
    await expect(svc(prisma).create(staffOwner, { title: 'x', priceCents: 0 } as any)).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it('a Center course needs a subject the Center offers', async () => {
    ...
    await expect(svc(prisma).create(teacherInCenter, { title: 'x', priceCents: 0, subjectId: 'maths' } as any))
...
describe('CoursesService.update — Center pricing cannot be introduced later', () => {
  ...
  it('a PERSONAL course keeps its pricing behaviour', async () => {
    ...
    await expect(svc(prisma).update(personal, 'c1', { priceCents: 250 } as any)).resolves.toMatchObject({ priceCents: 250 });
  });
});
...
describe('CoursesService scope — who sees what', () => {
  ...
  it('TEACHER member: only their own courses inside that academy', async () => {
    ...
    expect(prisma.course.findMany.mock.calls[0][0].where).toEqual({ academyId: 'centerA', tenantId: 'teacherT' });
  });
  ...
  it('cross-academy: a Center B course id inside a Center A scope 404s (never reveals it)', async () => {
    ...
    await expect(svc(prisma).update(ownerOfCenter, 'courseInB', { title: 'x' } as any)).rejects.toBeInstanceOf(NotFoundException);
    await expect(svc(prisma).remove(ownerOfCenter, 'courseInB')).rejects.toBeInstanceOf(NotFoundException);
    for (const call of prisma.course.findFirst.mock.calls) expect(call[0].where.academyId).toBe('centerA');
  });
  it("a TEACHER member cannot touch a colleague's course in the same Center", async () => {
    const prisma = makePrisma();
    prisma.course.findFirst.mockResolvedValue(null);
    await expect(svc(prisma).remove(teacherInCenter, 'colleagues')).rejects.toBeInstanceOf(NotFoundException);
    expect(prisma.course.findFirst.mock.calls[0][0].where).toMatchObject({ academyId: 'centerA', tenantId: 'teacherT' });
  });
});
