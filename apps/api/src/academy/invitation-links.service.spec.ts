...
describe('InvitationLinksService.create', () => {
  it('stores only the hash, TTL defaults to 14 days, and returns the raw token exactly once', async () => {
    ...
    prisma.academyInvitationLink.create.mockResolvedValue({
      id: 'l1',
      role: 'TEACHER',
      expiresAt: future,
      createdAt: new Date(),
...
describe('InvitationLinksService.revoke', () => {
  it('revokes an unused link', async () => {
    const prisma = makePrisma();
    prisma.academyInvitationLink.findFirst.mockResolvedValue({
      id: 'l1',
      usedAt: null,
      revokedAt: null,
    });
...
describe('InvitationLinksService.preview', () => {
  ...
  it.each([
    ...
    [
      'deleted Center',
      liveRow({ academy: { name: 'C', status: 'ACTIVE', deletedAt: new Date() } }),
    ],
...
describe('InvitationLinksService.accept', () => {
  const setup = (
    ...
    prisma.academyMembership.upsert.mockResolvedValue({
      id: 'm1',
      academyId: 'c1',
      role: 'TEACHER',
      status: 'ACTIVE',
  ...
  it('an existing ACTIVE membership is a conflict, not a silent success (token still consumed)', async () => {
    const prisma = setup();
    prisma.academyMembership.findUnique.mockResolvedValue({
      id: 'm0',
      status: 'ACTIVE',
      role: 'TEACHER',
    });
  ...
  it('multi-Center: accepting into Center B only touches the (user, Center B) row — a teacher already ACTIVE in Center A, PERSONAL, or elsewhere is untouched', async () => {
    ...
    expect(prisma.academyMembership.upsert.mock.calls[0][0].where).toEqual({
      userId_academyId: { userId: 'u1', academyId: 'centerB' },
    });
...
describe('InvitationLinksService.accept — consumed links', () => {
  const consumedBy = (userId: string) => {
    const prisma = makePrisma();
    prisma.academyInvitationLink.findUnique.mockResolvedValue(
      liveRow({ usedAt: past, usedByUserId: userId }),
    );
...
describe('InvitationLinksService.claimForNewUser (inside a registration transaction)', () => {
  ...
  it("claims the exact row, then builds the membership from the ROW — academyId and role are never the caller's to say", async () => {
    ...
    t.academyInvitationLink.findUniqueOrThrow.mockResolvedValue({
      academyId: 'center-from-row',
      role: 'ASSISTANT',
    });
    ...
    expect(t.academyInvitationLink.updateMany.mock.calls[0][0].data).toMatchObject({
      usedByUserId: 'new-user',
    });
    expect(t.academyMembership.create.mock.calls[0][0].data).toEqual({
      userId: 'new-user',
      academyId: 'center-from-row',
      role: 'ASSISTANT',
      status: 'ACTIVE',
