...
function spawnApi(port, redisUrl, label) {
  ...
  child.stdout.on('data', (d) => {
    if (process.env.DEBUG_MULTI_REPLICA) process.stdout.write(`[${label}] ${d}`);
  });
  child.stderr.on('data', (d) => {
    if (process.env.DEBUG_MULTI_REPLICA) process.stderr.write(`[${label}] ${d}`);
  });
  return child;
}
...
const login = async (port, email) => {
  const r = await api(port, '/auth/login', {
    method: 'POST',
...
try {
  ...
  const student = await prisma.studentProfile.findFirst({
    where: { userId: { not: undefined }, user: { isActive: true } },
    include: { user: true },
  });
  const outsider = await prisma.studentProfile.findFirst({
    where: { id: { not: student.id }, user: { isActive: true } },
    include: { user: true },
  });
  let thread = await prisma.chatThread.findFirst({
    where: { tenantId: academy.id, studentId: student.id },
  });
  if (!thread) {
    thread = await prisma.chatThread.create({
      data: { tenantId: academy.id, studentId: student.id, type: 'DM' },
    });
  }
...

  {
    ...
    check(
      'instance A → Redis → instance B: the recipient on instance B receives the message',
      received?.body === 'from A to B — cross-instance',
      received ? 'received' : 'timed out — NOT delivered',
    );
    check(
      'an outsider socket (never authorized into the room) receives nothing',
      !outsiderReceived,
      outsiderReceived ? 'LEAKED to outsider' : 'nothing received',
    );
  }
  {
    ...
    check(
      'instance B → Redis → instance A: the recipient on instance A receives the message',
      received?.body === 'from B to A — cross-instance',
      received ? 'received' : 'timed out — NOT delivered',
    );
  }
  ...
  if (otherThread) {
    ...
    check(
      "a socket joined to a different room never receives this room's messages",
      !leaked,
      leaked ? 'LEAKED' : 'clean',
    );
    bystander.close();
  ...
  // the sender (the classic Redis-adapter local+remote double-fire bug).
  {
    ...
    await new Promise((r) => setTimeout(r, 1500));
    check(
      'a socket on the SAME instance as the sender receives the message exactly once, not twice',
      count === 1,
      `received ${count} time(s)`,
    );
  ...
  // Disconnect / reconnect
  {
    ...
    check(
      'a fresh connection after disconnect succeeds cleanly',
    ...
    check(
      'the reconnected socket still receives cross-instance messages',
      received?.body === 'after reconnect',
  ...
  console.log('\n=== 2. DISTRIBUTED RATE LIMITING (login, shared 20/min) ===');
  {
    ...
    check(
      'the 429 fires at (or immediately after) request 21 of the combined stream, not ~41',
      !!first429 && first429.i <= 23,
      first429 ? `first 429 at combined request #${first429.i + 1}` : 'never fired',
    );
    const both429 =
      results.slice(first429 ? first429.i : 25).some((r) => r.port === PORT_A) &&
      results.slice(first429 ? first429.i : 25).some((r) => r.port === PORT_B);
    check(
      'once blocked, BOTH instances honor the same block (not just the one that tripped it)',
      !!first429 && both429,
      'checked post-block responses on both ports',
    );
  }
} catch (e) {
  console.error('\nERROR:', e.message);
