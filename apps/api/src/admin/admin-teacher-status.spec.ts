...
describe('AdminService.setTeacherStatus — suspension evicts live sessions', () => {
  it.each(['SUSPENDED', 'REJECTED'] as const)(
    ...
    async (status) => {
      ...
      await new AdminService(
        prisma,
        {} as any,
        d.notifications as any,
        d.mail as any,
      ).setTeacherStatus('t1', status as any, 'admin');
  ...
  it('APPROVED does not touch sessions', async () => {
    ...
    await new AdminService(
      ...
      d.notifications as any,
      d.mail as any,
    ).setTeacherStatus('t1', 'APPROVED' as any, 'admin');
