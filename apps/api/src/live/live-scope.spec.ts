...
const academyMock = () => ({ assertAssignableTeacher: jest.fn(async (_a: string, userId: string) => ({ userId, teacherProfileId: `tp_${userId}` })) }) as any;
const svc = (prisma: any, academy = academyMock()) => new LiveService(prisma, { create: jest.fn() } as any, {} as any, {} as any, {} as any, {} as any, academy);
const ownerCtx: LiveScope = { academyId: 'centerA', userId: 'ownerU', manageAll: true, role: 'OWNER' };
const teacherCtx: LiveScope = { academyId: 'centerA', userId: 'tA', manageAll: false, role: 'TEACHER' };
const staffCtx: LiveScope = { academyId: 'centerA', userId: 'staffU', manageAll: true, role: 'OWNER' };
...
describe('LiveService.create — organisation, teacher and group scope', () => {
  it('a teacher creating in a Center: tenantId = own profile (authorship), academyId = Center, teacherUserId = self', async () => {
    ...
    expect(s).toMatchObject({ tenantId: 'tp_tA', academyId: 'centerA', teacherUserId: 'tA', groupId: null });
  ...
  it('STAFF without a named teacher is refused by the identity check (self is not a teacher)', async () => {
    const prisma = makePrisma();
    const academy = academyMock();
    academy.assertAssignableTeacher.mockRejectedValue(new BadRequestException({ code: 'TEACHER_NOT_ASSIGNABLE' }));
    await expect(svc(prisma, academy).create(staffCtx, dto)).rejects.toMatchObject({ response: { code: 'TEACHER_NOT_ASSIGNABLE' } });
  ...
  it('a forged teacherUserId from another Center is refused', async () => {
    ...
    await expect(svc(prisma, academy).create(ownerCtx, { ...dto, teacherUserId: 'teacherOfB' })).rejects.toMatchObject({ response: { code: 'TEACHER_NOT_MEMBER' } });
  });
  it('a group must belong to this Center and the teacher must be assigned to it', async () => {
    const prisma = makePrisma();
    prisma.group.findFirst.mockResolvedValue(null);
    await expect(svc(prisma).create(teacherCtx, { ...dto, groupId: 'groupOfB' })).rejects.toBeInstanceOf(NotFoundException);
    expect(prisma.group.findFirst.mock.calls[0][0].where).toEqual({ id: 'groupOfB', academyId: 'centerA' });
    prisma.group.findFirst.mockResolvedValue({ id: 'g1' });
    prisma.groupAssignment.findFirst.mockResolvedValue(null);
    await expect(svc(prisma).create(teacherCtx, { ...dto, groupId: 'g1' })).rejects.toBeInstanceOf(BadRequestException);
  });
  it('a physical slot of the same teacher in the window blocks the stream (cross-table)', async () => {
    ...
    const err = await svc(prisma).create(teacherCtx, dto).catch((e) => e);
...
describe('LiveService scope — who reaches which streams', () => {
  it('OWNER lists every stream of the academy; a TEACHER only their own', async () => {
    ...
    expect(prisma.liveSession.findMany.mock.calls[1][0].where).toEqual({ academyId: 'centerA', teacherUserId: 'tA' });
  });
  it('a stream of another Center (or a colleague, for a teacher) 404s on update/remove', async () => {
    const prisma = makePrisma();
    await expect(svc(prisma).update({ ...ownerCtx, academyId: 'centerB' }, 'ls', { title: 'x' })).rejects.toBeInstanceOf(NotFoundException);
    expect(prisma.liveSession.findFirst.mock.calls[0][0].where).toMatchObject({ id: 'ls', academyId: 'centerB' });
    await expect(svc(prisma).remove(teacherCtx, 'ls')).rejects.toBeInstanceOf(NotFoundException);
    expect(prisma.liveSession.findFirst.mock.calls[1][0].where).toMatchObject({ id: 'ls', academyId: 'centerA', teacherUserId: 'tA' });
  });
...
describe('AcademyService.assertAssignableTeacher — the one identity check', () => {
  const make = (user: any, membership: any) => new AcademyService({
  ...
  it('approved TEACHER with ACTIVE TEACHER/OWNER membership passes and yields the profile', async () => {
    await expect(make(approved, { id: 'm' }).assertAssignableTeacher('c', 'u')).resolves.toEqual({ userId: 'u', teacherProfileId: 'tp' });
  });
  it.each([
    ['STAFF (no profile)', { id: 'u', role: 'STAFF', teacherProfile: null }],
    ['STUDENT', { id: 'u', role: 'STUDENT', teacherProfile: null }],
    ['PENDING teacher', { id: 'u', role: 'TEACHER', teacherProfile: { id: 'tp', status: 'PENDING' } }],
    ['SUSPENDED teacher', { id: 'u', role: 'TEACHER', teacherProfile: { id: 'tp', status: 'SUSPENDED' } }],
    ['REJECTED teacher', { id: 'u', role: 'TEACHER', teacherProfile: { id: 'tp', status: 'REJECTED' } }],
    ['inactive / missing user', null],
  ])('%s → TEACHER_NOT_ASSIGNABLE', async (_l, user) => {
    await expect(make(user, { id: 'm' }).assertAssignableTeacher('c', 'u')).rejects.toMatchObject({ response: { code: 'TEACHER_NOT_ASSIGNABLE' } });
  });
  it('an approved teacher of ANOTHER Center (no ACTIVE membership here) → TEACHER_NOT_MEMBER', async () => {
    const s = make(approved, null);
    await expect(s.assertAssignableTeacher('centerA', 'u')).rejects.toMatchObject({ response: { code: 'TEACHER_NOT_MEMBER' } });
    expect((s as any).prisma.academyMembership.findFirst.mock.calls[0][0].where).toMatchObject({ userId: 'u', academyId: 'centerA', status: 'ACTIVE', role: { in: ['TEACHER', 'OWNER'] } });
  });
});
...
describe('membership removal — future live streams lose the teacher, history stays', () => {
  it('revokeStaffResources clears future SCHEDULED LiveSession.teacherUserId in that academy only, never tenantId', async () => {
    const prisma: any = {
      academyMembership: { findFirst: jest.fn().mockResolvedValue({ id: 'm', userId: 'tA', academyId: 'centerA', role: 'TEACHER', status: 'ACTIVE' }), update: jest.fn().mockResolvedValue({}) },
    ...
    expect(call.where).toMatchObject({ academyId: 'centerA', teacherUserId: 'tA', status: 'SCHEDULED' });
